...
public class Forsaken implements Serializable {
	...
	Boolean ruthless = false;
	int peopleInjured = 0;
	int timesHadSex = 0;
	int timesKilled = 0;
	int demonicBirths = 0;
	Boolean lustful = false;
	int orgasmsGiven = 0;
	int timesOrgasmed = 0;
	int strongestOrgasm = 0;
	Boolean hypnotized = false;
	Boolean meek = false;
	int timesTortured = 0;
	int timesHarmedSelf = 0;
	Boolean drained = false;
	Boolean debased = false;
	int timesExposed = 0;
	int timesExposedSelf = 0;
	Boolean parasitized = false;
	
	int enjoyedAnal = 0;
	
	Taker[] takers  = {Taker.NONE, Taker.NONE, Taker.NONE, Taker.NONE};
	int[] takerIDs = new int[4];
	
	Chosen[] kills = new Chosen[2];
	int defeatType = 0;
	
	Chosen formerSelf = null;
	int[][] formerRelationships = new int[3][3];
	Forsaken firstPartner = null;
	Forsaken secondPartner = null;
	Chosen firstFormerPartner = null;
	Chosen secondFormerPartner = null;
	int firstOriginalRelationship = 0;
	int secondOriginalRelationship = 0;
	
	Forsaken[] others = new Forsaken[0];
	Chosen[] otherChosen = new Chosen[0];
	int[] troublemaker = new int[0];
	Relationship[] forsakenRelations = new Relationship[0];
	Relationship[] chosenRelations = new Relationship[0];
	int forsakenID = 0;
	...
	transient SaveData save = null;
	
	Chosen.Species type = null;
	...
	public void initialize(WorldState w, Chosen c) {
		...
		if (c.isImpregnated()) {
			...
			hostility += 30;
		} else if (c.isVVirg() == false) {
			hostility += 20;
		} else if (c.isRuthless()) {
			hostility += 10;
		}
		...
		if (kills[1] != null) {
			timesKilled = 2;
		} else if (kills[0] != null) {
			timesKilled = 1;
		}
		if (c.timesSlaughtered() > 0) {
			timesKilled = c.timesSlaughtered()*10 + (int)(Math.random()*10);
		}
		if (c.isVVirg() == false) {
			timesHadSex = 100 + (int)(Math.random()*500);
			if (c.vStart == false) {
				int added = 300 + (int)(Math.random()*300);
				int addedExtra = (int)(Math.random()*50);
				timesHadSex += added;
				timesExposedSelf += added + addedExtra;
				timesExposed += added + addedExtra;
				orgasmsGiven += added + (int)(Math.random()*50);
			}
		}
		if (c.isRuthless()) {
			peopleInjured = 100 + (int)(Math.random()*100);
			ruthless = true;
		}
		if (c.isHypnotized()) {
			hypnotized = true;
		}
		if (c.timesFantasized() > 0 && (c.isCVirg() == false || c.innocence < 67)) {
			timesOrgasmed = 1000 + c.timesFantasized()*25 + (int)(Math.random()*25);
			strongestOrgasm = 1000 + (int)(Math.random()*1000);
		} else if (c.isCVirg() == false) {
			timesOrgasmed = 100 + (int)(Math.random()*800);
			strongestOrgasm = 200 + (int)(Math.random()*200);
		} else if (c.innocence < 67) {
			timesOrgasmed = (int)(Math.random()*99);
			strongestOrgasm = 100 + (int)(Math.random()*100);
		}
		if (c.isLustful()) {
			orgasmsGiven += 1000 + (int)(Math.random()*1000);
			lustful = true;
		}
		if (c.isMeek()) {
			meek = true;
		...
		if (c.isAVirg() == false) {
			timesTortured = 30 + (int)(Math.random()*30);
		}
		if (c.timesDetonated() > 0) {
			timesHarmedSelf = c.timesDetonated() + (int)(Math.random()*(c.timesDetonated()*2));
		}
		if (c.isDrained()) {
			drained = true;
		}
		...
		if (c.timesStripped() > 0) {
			int added = c.timesStripped()*20 + (int)(Math.random()*(c.timesStripped()*40));
		...
		if (gender != Gender.MALE) {
			if (c.pastTempted) {
				enjoyedAnal = 3 + (int)(Math.random()*4);
			}
			while (temptTrack < 100000) {
				enjoyedAnal = enjoyedAnal*3/2 + (int)(Math.random()*3);
				temptTrack = temptTrack*10/9;
			}
		}
		...
		} else if (w.getGenders()[c.getNumber()].equals("female")) {
	...
	public void philosophyTalk(JTextPane t) {
		//takes all other corruptions into account - leads into training talk
		say(t, "\"");
		if (hostility < 20) {
			if (innocence > 66) {
			...
			if (innocence > 66) {
				...
				if (disgrace > 66) {
					say(t, "Ever since I fell from grace, people have been showing me their true colors, and they aren't pretty.  ");
				} else if (disgrace > 33) {
	...
	public void trainingTalk(JTextPane t) {
		//takes past training into account (simple for now) - leads into life talk
		if (deviancy < 20) {
	...
	public void lifeTalk(JTextPane t) {
		//maybe have this one take personality into account more heavily, since it leads into nothing?
		//or have it bring up relationships
		//or have it bring up current stamina/motivation
		if (disgrace < 20) {
	...
	public void othersTalk(WorldState w, JTextPane t, SaveData s) {
		...
		for (int i = 0; i < w.getHarem().length; i++) {
			if (w.getHarem()[i] != this) {
				Boolean killedPartner = false;
				Boolean otherKilledPartner = false;
				int opinionOfDead = 0;
				int otherOpinionOfDead = 0;
				int formerRelationship = 0;
				Forsaken x = w.getHarem()[i];
				Boolean history = false;
				if (firstFormerPartner != null && secondFormerPartner != null && formerSelf != null && x.firstFormerPartner != null && x.secondFormerPartner != null && x.formerSelf != null) {
				...
				if (formerSelf != null && x.formerSelf != null && x.firstFormerPartner != null) {
					if (kills != null && x.kills != null) {
						if (kills.length > 0 && x.kills.length > 0) {
							...
							} else if (kills[0] != null && (kills[0].equals(firstFormerPartner) || kills[0].equals(secondFormerPartner))) {
								killedPartner = true;
								if (ownID < i) {
	...
	public void describeSacrifice(JTextPane t, WorldState w) {
		...
		if (obedience > 66) {
			if (disgrace > 66) {
				append(w, t, "{mainName} doesn't voice even a word of complaint as your Thralls lead {himHer} to the breeding chamber (not that {heShe} retains enough strength to resist even if {heShe} wanted to).  ");
			} else if (disgrace > 33) {
				append(w, t, "{mainName} gladly cooperates with the Thralls who tell {himHer} that it's {hisHer} turn to be sent to the breeding chamber.  ");
			} else {
				append(w, t, "{mainName} is still quite strong, perhaps even strong enough to resist if {heShe} wanted to, but {heShe} still happily accompanies the Thralls sent to bring {himHer} to the breeding chamber.  ");
			}
			if (hostility > 66) {
				append(w, t, "Madness swims in {hisHer} eyes as {heShe} eagerly welcomes the tentacles wrapping around {himHer}.\n\n");
				say(t, "\"");
				if (confidence > 66) {
				...
				append(w, t, "{HisHer} face shines with pure happiness right up until the end.\n\n");
				say(t, "\"");
				if (innocence > 66) {
				...
				append(w, t, "{HeShe} holds {hisHer} head high, smiling triumphantly as the tentacles approach.\n\n");
				say(t, "\"");
				if (morality > 66) {
			...
			say(t, "\"\n\n");
			if (deviancy > 66) {
				append(w, t, "{HeShe} screams in ecstasy as a tentacle rams into {hisHer} " + hole + ", climaxing instantly.  {HisHer} voice rises in pitch and volume, the orgasm being drawn out by the spurts of slime being shot into {himHer}, only growing more and more intense with time.  ");
			} else if (deviancy > 33) {
				append(w, t, "{HeShe} sighs with satisfaction at the feeling of one of your tentacles entering {hisHer} " + hole + ", then moans and shudders at the pleasure radiating through {hisHer} body from the slime being pumped inside.  ");
			} else {
				append(w, t, "When one of your tentacles forces its way into {hisHer} " + hole + ", {heShe} winces in discomfort at first.  But {heShe} concentrates on relaxing and accepting it into {himHer}, and soon {heShe}'s rewarded by several spurts of slime into {hisHer} belly.  {HisHer} senses are assaulted by sudden overwhelming pleasure, and all pain is forgotten.  ");
			}
		} else if (obedience > 33) {
			if (disgrace > 66) {
				append(w, t, "When the Thralls come for {mainName}, {hisHer} face goes pale with fear, but {heShe} knows that there's no point in resisting with force.  Instead, as {heShe}'s led to the breeding chamber, {heShe} pleads and bargains, grasping for any reason at all for you to spare {himHer}.  ");
			} else if (disgrace > 33) {
				append(w, t, "As the Thralls lead {mainName} to the breeding chamber, {heShe} protests that {heShe}'s still strong enough to be of use to you, but {heShe} doesn't actually fight back, hoping against hope that you'll have a change of heart if {heShe} acts obediently enough.  ");
			} else {
				append(w, t, "{mainName} is still strong enough that {heShe} could put up some serious resistance, but {heShe}'s paralyzed by the fear you've put into {hisHer} heart, and {heShe} allows {himHer}self to be led to the breeding chamber.  ");
			}
			if (hostility > 66) {
				append(w, t, "As the tentacles encircle {himHer} and it becomes clear that {heShe}'s truly going to be converted into breeding stock, {heShe} breaks out into terrified, insane laughter.\n\n");
				say(t, "\"");
				if (confidence > 66) {
				...
				append(w, t, "Even when the tentacles begin to entwine {hisHer} limbs, {heShe} still doesn't fight back, {hisHer} fear giving way to resignation.\n\n");
				say(t, "\"");
				if (morality > 66) {
				...
				append(w, t, "{HeShe} hangs {hisHer} head, so consumed by {hisHer} self-loathing that {heShe} barely notices the tentacles wrapping around {hisHer} wrists and ankles.\n\n");
				say(t, "\"");
				if (innocence > 66) {
			...
			say(t, "\"\n\n");
			if (deviancy > 66) {
				append(w, t, "{HisHer} reluctance quickly fades when {heShe} sees the aphrodisiac-laced slime dripping from the surrounding tentacles.  {HeShe} eagerly takes one into {hisHer} mouth, then moans around it as {heShe} feels another slip into {hisHer} " + hole + ".  ");
			} else if (deviancy > 33) {
				append(w, t, "But {hisHer} breathing quickens and {heShe} instinctively spreads {hisHer} legs wider when {heShe} sees a tentacle approaching {hisHer} " + hole + ".  It pushes inside, and {mainName}'s last traces of terror fade away as the slime pumped inside {himHer} fills {hisHer} head with a sexual arousal too thick to think through.  ");
			} else {
				append(w, t, "A spike of terror shoots through {hisHer} mind when {heShe} feels a tentacle pushing against {hisHer} " + hole + ", and {heShe} groans in mingled fear and discomfort when it penetrates {himHer}.  However, {hisHer} body soon begins to feel hot, a side-effect of the slime being pumped into {himHer}, and {hisHer} reluctance gradually fades until {heShe}'s openly moaning in pleasure, unable to think of anything other than how good it feels.  ");
			}
		} else {
			if (disgrace > 66) {
				append(w, t, "{mainName} struggles with all {hisHer} might, kicking and screaming as {heShe}'s dragged to the breeding chamber, but {heShe}'s not significantly stronger than a regular human anymore, and it takes only three Thralls to subdue {himHer}.  ");
			} else if (disgrace > 33) {
				append(w, t, "{mainName} does {hisHer} best to stop you from bringing {himHer} to the breeding chamber, but {heShe} can't access {hisHer} full power anymore without your help, and it's not long before {heShe}'s defeated and thrown inside.  ");
			} else {
				append(w, t, "Enough residual psychic energy remains within {mainName} that {heShe}'s able to make an impressive last stand, killing the Demons you send after {himHer} and collapsing several chambers of the hive.  But eventually, {heShe} exhausts {himHer}self, and {heShe}'s dragged to the breeding chamber.  ");
			}
			if (hostility > 66) {
				append(w, t, "Even then, {heShe} snarls and shrieks, screaming bloody vengeance loud enough to be heard several chambers away.\n\n");
				say(t, "\"");
				if (innocence > 66) {
				...
				append(w, t, "The tentacles close in, and {heShe} shouts wild denials, unable to accept {hisHer} fate.\n\n");
				say(t, "\"");
				if (confidence > 66) {
				...
				append(w, t, "More and more tentacles surround {himHer}, piling on, wrapping around {himHer} so thickly that {heShe} can hardly been seen, but {heShe} puts all {hisHer} strength into mounting as much resistance as {heShe} can.\n\n");
				say(t, "\"");
				if (morality > 66) {
	...
	public void trainingMenu(final JTextPane t, final JPanel p, final JFrame f, final WorldState w, final SaveData s, final Boolean[] currentTraining, final int page, final Boolean consenting) {
		...
		for (int i = 0; i < currentTraining.length; i++) {
			...
			if (i == 0) {
				...
				trainingMotivations[i] = 20;
				trainingExpertises[i] = 800;
			} else if (i == 5) {
				trainingIntensities[i][2] = 20;
				trainingIntensities[i][3] = 20;
				trainingStaminas[i] = 140;
				trainingMotivations[i] = 30;
				trainingExpertises[i] = 1000;
			} else if (i == 6) {
				trainingIntensities[i][0] = 30;
				trainingIntensities[i][1] = 30;
				trainingStaminas[i] = 130;
				trainingMotivations[i] = 40;
				trainingExpertises[i] = 3100;
			} else if (i == 7) {
				trainingIntensities[i][0] = 40;
				trainingIntensities[i][2] = 35;
				trainingStaminas[i] = 140;
				trainingMotivations[i] = 60;
				trainingExpertises[i] = 3300;
			} else if (i == 8) {
				trainingIntensities[i][0] = 35;
				trainingIntensities[i][3] = 40;
				trainingStaminas[i] = 100;
				trainingMotivations[i] = 50;
				trainingExpertises[i] = 2900;
			} else if (i == 9) {
				trainingIntensities[i][1] = 35;
				trainingIntensities[i][2] = 30;
				trainingStaminas[i] = 110;
				trainingMotivations[i] = 40;
				trainingExpertises[i] = 2500;
			} else if (i == 10) {
				trainingIntensities[i][1] = 40;
				trainingIntensities[i][3] = 30;
				trainingStaminas[i] = 120;
				trainingMotivations[i] = 50;
				trainingExpertises[i] = 2700;
			} else if (i == 11) {
				trainingIntensities[i][2] = 40;
				trainingIntensities[i][3] = 35;
				trainingStaminas[i] = 150;
				trainingMotivations[i] = 60;
				trainingExpertises[i] = 3500;
			} else if (i == 12) {
				trainingIntensities[i][0] = 50;
				trainingIntensities[i][1] = 50;
				trainingStaminas[i] = 95;
				trainingMotivations[i] = 60;
				trainingExpertises[i] = 7000;
			} else if (i == 13) {
				trainingIntensities[i][0] = 60;
				trainingIntensities[i][2] = 50;
				trainingStaminas[i] = 105;
				trainingMotivations[i] = 80;
				trainingExpertises[i] = 8000;
			} else if (i == 14) {
				trainingIntensities[i][0] = 70;
				trainingIntensities[i][3] = 70;
				trainingStaminas[i] = 75;
				trainingMotivations[i] = 70;
				trainingExpertises[i] = 13000;
			} else if (i == 15) {
				trainingIntensities[i][1] = 70;
				trainingIntensities[i][2] = 60;
				trainingStaminas[i] = 115;
				trainingMotivations[i] = 100;
				trainingExpertises[i] = 11000;
			} else if (i == 16) {
				trainingIntensities[i][1] = 60;
				trainingIntensities[i][3] = 60;
				trainingStaminas[i] = 85;
				trainingMotivations[i] = 90;
				trainingExpertises[i] = 9000;
			} else if (i == 17) {
				trainingIntensities[i][2] = 70;
				trainingIntensities[i][3] = 50;
				trainingStaminas[i] = 125;
				trainingMotivations[i] = 80;
				trainingExpertises[i] = 12000;
			}
			if (currentTraining[i]) {
				...
				if (numerator > denominator) {
					...
					if (numerator/denominator < 10) {
						...
						trainingExpertises[i] = trainingExpertises[i]*10;
					}
				}
				if (trainingExpertises[i] >= hateExp && i % 6 >= 3) {
					trainingExpertises[i] = hateExp-1;
					penalty[i] = true;
				}
				if (trainingExpertises[i] >= pleaExp && (i % 6 == 1 || i % 6 == 2 || i % 6 == 5)) {
					trainingExpertises[i] = pleaExp-1;
					penalty[i] = true;
				}
				if (trainingExpertises[i] >= injuExp && i % 2 == 0) {
					trainingExpertises[i] = injuExp-1;
					penalty[i] = true;
				}
				if (trainingExpertises[i] >= expoExp && (i % 6 == 0 || i % 6 == 1 || i % 6 == 3)) {
					trainingExpertises[i] = expoExp-1;
					penalty[i] = true;
				}
			}
		}
		int[] currentIntensity = new int[]{currentHostility, currentDeviancy, currentObedience, currentDisgrace};
		int[] trainingCounts = new int[]{hostilityCount, deviancyCount, obedienceCount, disgraceCount};
		append(w, t, "\n\n" + w.getSeparator() + "\n\n{mainName}\n");
		w.append(t, "Stamina: " + stamina/10 + "." + stamina % 10 + "%\nMotivation: " + motivation/10 + "." + motivation % 10 + "%");
		w.append(t, "\nExpertise\nHATE: " + condensedFormat(hateExp) + "\nPLEA: " + condensedFormat(pleaExp));
		if (w.tickleOn) {
			w.append(t, "\nANTI: ");
		} else {
			w.append(t, "\nINJU: ");
		...
		if (types == 0) {
			...
			if (hostilityCount != deviancyCount) {
				w.append(t, "\nGained Expertise ");
				if (hostilityCount > deviancyCount) {
					w.append(t, "+" + (hostilityCount-deviancyCount)*20 + "%");
				} else {
					w.append(t, "-" + (deviancyCount-hostilityCount)*20 + "%");
				}
				for (int i = 0; i < trainingExpertises.length; i++) {
					trainingExpertises[i] = trainingExpertises[i]*(5 + hostilityCount - deviancyCount)/5;
					if (trainingExpertises[i] < 0) {
						trainingExpertises[i] = 0;
					}
				}
			}
		...
		for (int i = 0; i < trainingExpertises.length; i++) {
			trainingExpertises[i] = trainingExpertises[i]*expModifier()/1000;
		}
		...
		trainingNames[8] = "Psychic Transmission";
		trainingNames[9] = "Apply Aphrodisiac";
		trainingNames[10] = "Service Thralls";
		trainingNames[11] = "Rigged Fight";
		trainingNames[12] = "Inseminate";
		trainingNames[13] = "Empower Thralls";
			...
			if (currentTraining[trainingType] == false) {
				...
				if (reqsMet) {
					...
					for (int j = 0; j < 4; j++) {
						...
						if (threatenedIntensity[j] != currentIntensity[j]) {
							if (threatenedIntensity[j] != trainingIntensities[trainingType][j]) {
								w.append(t, "+" + (threatenedIntensity[j] - currentIntensity[j]) + "% ");
							} else {
								w.append(t, threatenedIntensity[j] + "% ");
							}
							if (j == 0) {
					...
					if (trainingType == 6 && ruthless == false) {
						...
						w.redAppend(t, "sex");
					} else if (trainingType == 14 && timesExposed < 300000) {
						w.append(t, " - ");
						w.redAppend(t, "first worldwide exposure");
					} else if (trainingType == 15 && strongestOrgasm < 200) {
						w.append(t, " - ");
						w.redAppend(t, "first high-intensity orgasm");
					} else if (trainingType == 17 && timesTortured == 0) {
						w.append(t, " - ");
						w.redAppend(t, "first high-intensity torture");
					}
					w.append(t, "\n  -" + trainingStaminas[trainingType]/10);
					if (trainingStaminas[trainingType] % 10 != 0) {
						w.append(t, "." + trainingStaminas[trainingType] % 10);
					}
					w.append(t, "% Stamina, +" + trainingMotivations[trainingType]/10);
		...
		if (types == 0) {
			...
			w.trainedForsaken = new Forsaken[]{this};
	...
	public void trainingDescription(final JTextPane t, final JPanel p, final JFrame f, final WorldState w, final SaveData s, final Boolean[] currentTraining, final int nextTraining, int[] threatenedIntensity, final Boolean consenting, final int lostStamina, final int gainedMotivation, final long gainedExpertise) {
		...
		Boolean inPublic = currentTraining[0] || currentTraining[2] || currentTraining[4] || currentTraining[5] || currentTraining[6] || currentTraining[7] || currentTraining[8] || currentTraining[10] || currentTraining[11];
		Boolean tiedUp = (currentTraining[3] && consenting == false) || currentTraining[5];
		if (nextTraining == 0) {
			append(w, t, "{mainName} will be placed under a curse which renders {himHer} sexually irresistible to everyone else in your domain.  ");
			if (inPublic) {
				append(w, t, "The attention {heShe}'s getting from the public will become even more intense, more depraved, and less concerned with {hisHer} own comfort.");
			} else {
				append(w, t, "Every time {heShe} goes out in public, {heShe}'ll be the subject of constant sexual advances - and {hisHer} suitors won't take 'no' for an answer.");
			}
		} else if (nextTraining == 1) {
			append(w, t, "{mainName} will be given unfiltered access to the Demonic hivemind, whether {heShe} wants it or not.  ");
			if (inPublic) {
				append(w, t, "Being able to experience the thoughts and emotions of all the people around {himHer} will give {himHer} a deeper appreciation of humanity's innate sadism.");
			} else if (tiedUp) {
				append(w, t, "The overwhelming flood of thoughts, emotions, and sensations that aren't {hisHer} own will render {himHer} even more helpless.");
			} else {
				append(w, t, "The overwhelming flood of thoughts, emotions, and sensations that aren't {hisHer} own will render {himHer} completely helpless to resist your will.");
			}
		} else if (nextTraining == 2) {
			append(w, t, "{mainName} will ");
			if (inPublic) {
				append(w, t, "submit {himHer}self to the people around {himHer}, inviting them to ridicule and abuse {himHer}.  ");
			} else {
				append(w, t, "go out in public and then submit {himHer}self to everyone's ridicule and abuse.  ");
			}
			append(w, t, "Even the people that don't have any reason to hate {himHer} will be tempted to join in on the fun.");
		} else if (nextTraining == 3) {
			append(w, t, "Various telekinetically-manipulated sex toys will attach themselves to {mainName}'s body and begin tormenting {himHer} with pleasure.  ");
			if (consenting) {
				append(w, t, "If {heShe} tries to take them off, then restraints and bindings will arrive to prevent {himHer} from doing so.");
			} else if (tiedUp) {
				append(w, t, "{HeShe}'s in no position to refuse.");
			} else {
				append(w, t, "Ropes and cuffs will immobilize {hisHer} limbs so that {heShe} can't take them off.");
			}
		} else if (nextTraining == 4) {
			append(w, t, "{mainName} will be granted the psychic energy to transform and call on {hisHer} Forsaken powers, but the clothes that come with {hisHer} transformation will be modified to humiliate {himHer}.  ");
			if (timesExposedSelf == 0) {
				append(w, t, "{HeShe} won't be able to maintain any degree of modesty.");
			} else {
				append(w, t, "Exposing {hisHer} body doesn't bother {himHer} much anymore, but ");
				if (deviancy < 34) {
					w.append(t, "the blatantly sexual design will be even worse than just being seen naked.");
				} else if (obedience < 34) {
					append(w, t, "being forced to wear an outfit of your choosing will rankle {himHer}.");
				} else if (hostility < 34) {
					append(w, t, "the Demonic features incorporated into {hisHer} garb will highlight how far {heShe}'s fallen.");
				} else {
					append(w, t, "the fact that it's being forced upon {himHer} will take {himHer} out of {hisHer} comfort zone.");
				}
			}
		} else if (nextTraining == 5) {
			if (inPublic) {
				if (tiedUp) {
					append(w, t, "{mainName}'s restraints will be anchored to the ground so that there's time for a larger crowd to gather and witness {hisHer} humiliation.  ");
				} else {
					append(w, t, "{mainName} will be immobilized in stationary restraints so that {heShe} can no longer flee or fight back against the surrounding public.  ");
				}
			} else {
				if (tiedUp) {
					append(w, t, "{mainName} will be dragged out into public so that a larger number of people can witness {hisHer} humiliation.  ");
				} else {
					append(w, t, "{mainName} will be locked in outdoors stationary restraints so that a large number of people can see how helpless {heShe} is.  ");
				}
			}
			append(w, t, "The fact that {heShe} can't do anything to resist will be obvious to everyone, including {himHer}.");
		} else if (nextTraining == 6) {
			if (tiedUp) {
				append(w, t, "{mainName}'s restraints will be loosened up just enough to let {himHer} fight a little.  Then, {heShe} ");
			} else {
				append(w, t, "{mainName} ");
			}
			if (consenting) {
				append(w, t, "will beat up some Thralls who have disobeyed you, and {heShe}'ll be encouraged to sexually satisfy {himHer}self in the process.  If {heShe} refuses, then {heShe} ");
			}
			w.append(t, "will be assaulted by Thralls ");
			if (timesHadSex == 0) {
				append(w, t, "intent on stealing {hisHer} ");
			...
			append(w, t, ", and {heShe}'ll be forced to get violent in order to defend {himHer}self.");
			if (ruthless == false) {
				w.redAppend(t, "  Afterward, " + heShe() + " won't be able to deny " + hisHer() + " capacity to cause suffering.");
			}
		} else if (nextTraining == 7) {
			append(w, t, "The Thralls will be allowed to abuse {mainName} however they want.  ");
			if (tiedUp) {
				append(w, t, "Considering {hisHer} helplessness, this should be especially painful for {himHer}.");
			} else {
				append(w, t, "{HeShe} in turn will be allowed to flee or fight back if {heShe} can, but the experience of being hunted should prove educational either way.");
			}
		} else if (nextTraining == 8) {
			append(w, t, "{mainName} will begin to unwillingly broadcast {hisHer} surface thoughts in a large area around {himHer}.  ");
			if (inPublic) {
				append(w, t, "{HeShe} won't be able to hide {hisHer} reactions and feelings from the gathered crowd.");
			} else {
				append(w, t, "Many of those who hear {himHer} will take the opportunity to come find {himHer} in hopes of having a chance to participate in {hisHer} training.");
			}
			if (debased == false) {
		...
		} else if (nextTraining == 9) {
			if (consenting) {
				append(w, t, "{mainName} will be instructed to consume an extra-potent version of the aphrodisiac slime you pump into your human breeding stock.  If {heShe} refuses, then you'll have your Thralls ensure that {heShe} gets dosed.  ");
			} else {
				append(w, t, "You will have your Thralls dose {mainName} with an extra-potent version of the aphrodisiac slime you pump into your human breeding stock.  ");
			}
			append(w, t, "{HeShe}'ll soon be helpless with lust.");
		} else if (nextTraining == 10) {
			if (consenting == false) {
				w.append(t, "The helpless ");
			}
			append(w, t, "{mainName} will be used as an outlet for the Thralls' lust, and you'll place notices throughout your domain that anyone who's interested can come and join in on the fun.");
			if (lustful == false) {
		...
		} else if (nextTraining == 11) {
			append(w, t, "{mainName} will be attacked by several Demons in a makeshift gladiatorial match for the public's enjoyment.  ");
			if (tiedUp) {
				append(w, t, "Obviously, in {hisHer} current state, it will be nothing close to a fair fight, and it will only stop once {heShe} admits defeat.");
			} else {
				append(w, t, "More Demons will continue to arrive until {mainName} recognizes the futility of fighting and admits defeat.");
			}
			if (meek == false) {
				w.redAppend(t, "  Being forced to explicitly give up will help to break what's left of " + hisHer() + " pride.");
			}
		} else if (nextTraining == 12) {
			append(w, t, "You will create a short-term Commander body equipped with a penis-like organ which will penetrate {mainName} and infuse {hisHer} body with potent slime, tying {himHer} more closely to the Demonic hivemind.");
			if (timesHadSex == 0) {
				if (gender == Gender.MALE) {
					w.redAppend(t, "  This will be " + hisHer() + " first anal penetration, ");
				} else {
		...
		} else if (nextTraining == 13) {
			append(w, t, "You will infuse a large group of Thrall volunteers with pure Demonic energy, temporarily turning them into lesser versions of your Demon Commanders.  They will then hunt down {mainName} and abuse {himHer} in ways more extreme than a regular human could ever imagine.");
		} else if (nextTraining == 14) {
			append(w, t, "You will form a supernatural connection with televisions and computers all across the world, then broadcast images of {mainName} being humiliated to the unsuspecting civilians who happen to be nearby.");
			if (timesExposed < 300000) {
				w.redAppend(t, "  This will damage " + hisHer() + " reputation even among those who don't live in your domain.");
			}
		} else if (nextTraining == 15) {
			append(w, t, "You will subject {mainName} to a mass of tentacles specialized for the purpose of putting the human body in a state of continuous orgasm, far beyond what's normally possible for human hands and tools alone.");
			if (strongestOrgasm < 200) {
				w.redAppend(t, "  This will be " + hisHer() + " first time feeling such intense sexual pleasure.");
			}
		} else if (nextTraining == 16) {
			append(w, t, "You will have {mainName} demonstrate sexual techniques, whether willingly or unwillingly, on a tentacled body made for this purpose.  ");
			if (inPublic) {
				w.append(t, "The assembled crowd will be encouraged to watch and learn.");
			} else {
				w.append(t, "A crowd will be assembled to observe.");
			}
		} else if (nextTraining == 17) {
			if (w.tickle()) {
		...
		int[] corruptionComparison = new int[]{hostility, deviancy, obedience, disgrace};
		Boolean consentBlock = true;
		Boolean lacksConsent = (consenting == false);
		int relationTotal = 0;
		...
		if (consenting) {
			...
			if (consentModifier() < 0) {
				w.append(t, consentModifier() + " (low obedience)");
			} else {
				w.append(t, "+" + consentModifier() + " (obedience)");
			}
			w.append(t, "\n");
			if (intensityFactor < 0) {
				w.append(t, intensityFactor + " (intensity exceeds current corruption)");
			} else {
				w.append(t, "+" + intensityFactor + " (accustomed to similar circumstances)");
			}
			w.append(t, "\n");
			int breakFactor = 0;
			if (nextTraining == 6 && ruthless == false) {
				breakFactor = -10;
				w.redAppend(t, "-10 (first violence toward humans)");
			} else if (nextTraining == 8 && debased == false) {
				breakFactor = -10;
				w.redAppend(t, "-10 (first large-scale humiliation)");
			} else if (nextTraining == 10 && lustful == false) {
				breakFactor = -10;
				w.redAppend(t, "-10 (first experience with public sexual use)");
			} else if (nextTraining == 11 && meek == false) {
				breakFactor = -10;
				w.redAppend(t, "-10 (first time being forced to beg)");
			} else if (nextTraining == 12 && timesHadSex == 0) {
				breakFactor = -20;
			...
			} else if (nextTraining == 14 && timesExposed < 300000) {
				breakFactor = -20;
				w.redAppend(t, "-20 (first worldwide exposure)");
			} else if (nextTraining == 15 && strongestOrgasm < 200) {
				breakFactor = -20;
				w.redAppend(t, "-20 (first high-intensity orgasm)");
			} else if (nextTraining == 17 && timesTortured == 0) {
				breakFactor = -20;
				w.redAppend(t, "-20 (first high-intensity torture)");
			}
			if (breakFactor != 0) {
				w.append(t, "\n");
			}
			for (int i = 0; i < others.length; i++) {
				if (forsakenRelations[i] == Relationship.PARTNER) {
					if (opinion(others[i]) > -100) {
						int opinionClamp = opinion(others[i]) + 100;
						if (opinionClamp > 500) {
							opinionClamp = 500;
						}
						Boolean found = false;
						for (int j = 0; j < w.getHarem().length; j++) {
							if (w.getHarem()[j].equals(others[i])) {
								...
								if (opinionClamp > 0) {
									w.append(t, "+" + opinionClamp + " (");
									if (others[i].obedience > obedience) {
										if (obedience > 80) {
											w.append(t, "wants to be trained as hard as " + others[i].mainName);
										} else if (obedience > 60) {
											w.append(t, "wants to take pressure off " + others[i].mainName);
										} else if (obedience > 40) {
											w.append(t, "worried that you'll break " + others[i].mainName + " even harder");
										} else if (obedience > 20) {
											w.append(t, "determined to take " + others[i].mainName + "'s place");
										} else {
											w.append(t, "trying to distract you from " + others[i].mainName);
										}
									} else {
										if (obedience > 80) {
											w.append(t, "happy to serve you alongside " + others[i].mainName);
										} else if (obedience > 60) {
											w.append(t, "happy with " + others[i].mainName + "'s treatment");
										} else if (obedience > 40) {
											w.append(t, "relieved that you're sparing " + others[i].mainName);
										} else if (obedience > 20) {
											w.append(t, "determined to protect " + others[i].mainName);
										} else {
											w.append(t, "wants to keep " + others[i].mainName + " safe");
										}
									}
									if (opinionClamp > reasonTotal) {
										biggestReason = others[i];
										reasonTotal = opinionClamp;
									}
								} else {
									if (obedience < 80) {
									...
									w.append(t, opinionClamp + " (");
									if (obedience > 80) {
										w.append(t, "cares more about you than about " + others[i].mainName);
									} else if (obedience > 60) {
										w.append(t, "uneasy about " + others[i].mainName + "'s extreme treatment");
									} else if (obedience > 40) {
										w.append(t, "unhappy with your abuse of " + others[i].mainName);
									} else if (obedience > 20) {
										w.append(t, "angry about how you've trained " + others[i].mainName);
									} else {
										w.append(t, "furious about what you've done to " + others[i].mainName);
									}
									if (opinionClamp < objectionTotal) {
										biggestObjection = others[i];
										objectionTotal = opinionClamp;
									}
								}
								w.append(t, ")\n");
						...
						if (found == false) {
							...
							if (obedience > 80) {
								w.append(t, "accepts your decision to sacrifice " + others[i].mainName);
							} else if (obedience > 60) {
								w.append(t, "uneasy about " + others[i].mainName + " being sacrificed");
							} else if (obedience > 40) {
								w.append(t, "resentful about you sacrificing " + others[i].mainName);
							} else if (obedience > 20) {
								w.append(t, "angry that you sacrificed " + others[i].mainName);
							} else {
								w.append(t, "determined to avenge " + others[i].mainName);
							}
							w.append(t, opinionClamp + " (");
							w.append(t, ")\n");
							relationTotal += opinionClamp;
			...
			consentTotal = consentModifier() + intensityFactor + breakFactor + relationTotal;
			if (consentTotal >= 0) {
				w.append(t, "+");
			}
			w.append(t, consentTotal + " Total");
			if (consentTotal < 0) {
				lacksConsent = true;
				for (int i = 0; i < currentTraining.length; i++) {
		...
		if (consenting && consentTotal >= 0 && consentTotal - relationTotal < 0) {
			if (flavorObedience() < 20) {
				Project.changePortrait(gender, type, Project.displayedCivilians[0], true, w, Project.displayedNames, 0, Project.Emotion.FOCUS, Project.Emotion.ANGER);
				if (innocence > 66) {
					say(t, "Yeah, that's right!  I'm gonna keep you so busy with me that you forget all about " + biggestReason.mainName + "!");
				} else if (innocence > 33) {
					say(t, "This is how I will protect " + biggestReason.mainName + "!");
				} else {
					say(t, "My will is stronger than " + biggestReason.mainName + "'s!  If it will keep you from doing the same to " + biggestReason.himHer() + ", then I can endure anything!");
				}
			} else if (flavorObedience() < 40) {
				if (innocence > 66) {
				...
				if (innocence > 66) {
					say(t, "I'm scared... but I have to be strong for " + biggestReason.mainName + "!");
				} else if (innocence > 33) {
					say(t, "You have " + biggestReason.mainName + ", so... I can't really refuse."); 
				} else {
					say(t, "I can only hope that you'll remember my cooperation when it comes time to decide what to do with " + biggestReason.mainName + ".");
				}
			} else if (flavorObedience() < 81) {
				...
				if (innocence > 66) {
					say(t, "When I think of " + biggestReason.mainName + ", it makes me wanna try extra hard!");
				} else if (innocence > 33) {
					say(t, "I wouldn't normally do this, but I owe you for " + biggestReason.mainName + ".");
				} else {
					say(t, "You've been taking such good care of " + biggestReason.mainName + "...  I suppose I can put forth some extra effort.");
				}
			} else {
				Project.changePortrait(gender, type, Project.displayedCivilians[0], true, w, Project.displayedNames, 0, Project.Emotion.FOCUS, Project.Emotion.JOY);
				if (innocence > 66) {
					say(t, "I wanna tell " + biggestReason.mainName + " what we did together!");
				} else if (innocence > 33) {
					say(t, "I... I'll show " + biggestReason.mainName + " how it's done!");
				} else {
					say(t, "Of course, I love you more than I love " + biggestReason.mainName + "... but... I must admit that " + biggestReason.heShe() + " helps to give me strength.");
				}
			}
		} else if (consenting && consentTotal < 0 && consentTotal - relationTotal > 0) {
			if (obedience < 20) {
				...
				if (innocence > 66) {
					say(t, "I'll never forgive you for what you did to " + biggestObjection.mainName + "!  Never!");
				} else if (innocence > 33) {
					say(t, "I won't lift one finger to help the monster who hurt " + biggestObjection.mainName + "!");
				} else {
					say(t, "After what you did to " + biggestObjection.mainName + ", we have nothing to talk about.");
				}
			} else if (obedience < 40) {
				Project.changePortrait(gender, type, Project.displayedCivilians[0], true, w, Project.displayedNames, 0, Project.Emotion.ANGER, Project.Emotion.STRUGGLE);
				if (innocence > 66) {
					say(t, "I'm still mad about what you did to " + biggestObjection.mainName + "!");
				} else if (innocence > 33) {
					say(t, "Do you really think I'll work with you after seeing what you've done to " + biggestObjection.mainName + "?");
				} else {
					say(t, "Do whatever you want to me, but I refuse to cooperate.  That will be how I avenge " + biggestObjection.mainName + ".");
				}
			} else if (obedience < 61) {
				Project.changePortrait(gender, type, Project.displayedCivilians[0], true, w, Project.displayedNames, 0, Project.Emotion.ANGER, Project.Emotion.FEAR);
				if (innocence > 66) {
					say(t, "If you want me to go along with that, th-then make " + biggestObjection.mainName + " go back to the way " + biggestObjection.heShe() + " used to be!");
				} else if (innocence > 33) {
					say(t, "There's no point in cooperating.  You'll just do the same to me as you did to " + biggestObjection.mainName + "...");
				} else {
					say(t, "Considering your treatment of " + biggestObjection.mainName + ", I simply don't trust you to show any mercy to me, even if I do cooperate.");
				}
			} else if (obedience < 81) {
				...
				if (innocence > 66) {
					say(t, "I'm sorry... but I'm missing " + biggestObjection.mainName + " and I can't really get into it...");
				} else if (innocence > 33) {
					say(t, "Are you just going to push me harder and harder until I end up like " + biggestObjection.mainName + "?  I... can't accept that.");
				} else {
					say(t, "I cared about " + biggestObjection.mainName + "... perhaps as much as I care about you.  I'm not going to push myself, not until I can forgive you for what you did to " + biggestObjection.himHer() + ".");
				}
			} else {
				Project.changePortrait(gender, type, Project.displayedCivilians[0], true, w, Project.displayedNames, 0, Project.Emotion.SHAME, Project.Emotion.STRUGGLE);
				if (innocence > 66) {
					...
					say(t, "Memories of " + biggestObjection.mainName + "... keep distracting me...!");
				}
			}
		} else if (consenting == false || (lacksConsent && (nextTraining % 2 == 1))) {
			if (nextTraining == 0) {
				if (obedience < 20) {
					Project.changePortrait(gender, type, Project.displayedCivilians[0], true, w, Project.displayedNames, 0, Project.Emotion.ANGER, Project.Emotion.STRUGGLE);
					if (innocence > 66) {
					...
					Project.changePortrait(gender, type, Project.displayedCivilians[0], true, w, Project.displayedNames, 0, Project.Emotion.SHAME, Project.Emotion.LEWD);
					if (innocence > 66) {
					...
					Project.changePortrait(gender, type, Project.displayedCivilians[0], true, w, Project.displayedNames, 0, Project.Emotion.FEAR, Project.Emotion.JOY);
					if (innocence > 66) {
				...
				if (obedience < 20) {
					...
					Project.changePortrait(gender, type, Project.displayedCivilians[0], true, w, Project.displayedNames, 0, Project.Emotion.STRUGGLE, Project.Emotion.ANGER);
					if (innocence > 66) {
					...
					Project.changePortrait(gender, type, Project.displayedCivilians[0], true, w, Project.displayedNames, 0, Project.Emotion.LEWD, Project.Emotion.JOY);
					if (innocence > 66) {
				...
				if (obedience < 20) {
					...
					Project.changePortrait(gender, type, Project.displayedCivilians[0], true, w, Project.displayedNames, 0, Project.Emotion.NEUTRAL, Project.Emotion.STRUGGLE);
					if (morality > 66) {
				...
				if (obedience < 20) {
					...
					Project.changePortrait(gender, type, Project.displayedCivilians[0], true, w, Project.displayedNames, 0, Project.Emotion.JOY, Project.Emotion.FOCUS);
					if (dignity > 66) {
				...
				if (obedience < 20) {
					...
					Project.changePortrait(gender, type, Project.displayedCivilians[0], true, w, Project.displayedNames, 0, Project.Emotion.NEUTRAL, Project.Emotion.LEWD);
					if (morality > 66) {
				...
				if (obedience < 20) {
					Project.changePortrait(gender, type, Project.displayedCivilians[0], true, w, Project.displayedNames, 0, Project.Emotion.ANGER, Project.Emotion.FOCUS);
					if (morality > 66) {
						say(t, "You won't be able to break my will with mere slime!");
					} else if (morality > 33) {
						say(t, "No matter what you put inside me, I won't give in!");
					} else {
						say(t, "I'll kill you if you put that disgusting slime inside me!");
					}
				} else if (obedience < 40) {
					Project.changePortrait(gender, type, Project.displayedCivilians[0], true, w, Project.displayedNames, 0, Project.Emotion.FEAR, Project.Emotion.ANGER);
					if (innocence > 66) {
						say(t, "I... I don't want your stuff inside me!  Stop it!");
					} else if (innocence > 33) {
						say(t, "Th-This is just rape, isn't it!?");
					} else {
						say(t, "Slime injected directly by the Demon Lord...  This may be challenging...");
					}
				} else if (obedience < 61) {
					Project.changePortrait(gender, type, Project.displayedCivilians[0], true, w, Project.displayedNames, 0, Project.Emotion.FEAR, Project.Emotion.SHAME);
					if (innocence > 66) {
						say(t, "I-It won't fit!  You're just going to break me!");
					} else if (innocence > 33) {
						say(t, "Y-You're joking, right?  Just tell me what you want from me, there's no need to put that inside!");
					} else {
						say(t, "Is it truly necessary... to penetrate me with something so large...?");
					}
				} else if (obedience < 81) {
					Project.changePortrait(gender, type, Project.displayedCivilians[0], true, w, Project.displayedNames, 0, Project.Emotion.FEAR, Project.Emotion.SHAME);
					if (dignity > 66) {
						say(t, "I'm ready for you to put it inside me, Demon Lord...  Or at least I think I am...");
					} else if (dignity > 33) {
						say(t, "Oh no...  It's going to... stretch me open...");
					} else {
						say(t, "It's too big!  I'm scared!");
					}
				} else {
					Project.changePortrait(gender, type, Project.displayedCivilians[0], true, w, Project.displayedNames, 0, Project.Emotion.LEWD, Project.Emotion.JOY);
					if (dignity > 66) {
						say(t, "I'm honored... to become one with the Demon Lord...");
					} else if (dignity > 33) {
						say(t, "I promise to make you feel good, Demon Lord.");
					} else {
						say(t, "Yes!  Please!  Put it inside me!");
					}
				}
			} else if (nextTraining == 13) {
				Project.changePortrait(gender, type, Project.displayedCivilians[0], true, w, Project.displayedNames, 0, Project.Emotion.FOCUS, Project.Emotion.ANGER);
				if (obedience < 20) {
					if (morality > 66) {
						say(t, "I will never give up, even against enemies like these!");
					} else if (morality > 33) {
						say(t, "Empowered Thralls are still just Thralls.");
					} else {
						say(t, "If they still bleed, I'll still kill them.");
					}
				} else if (obedience < 40) {
					Project.changePortrait(gender, type, Project.displayedCivilians[0], true, w, Project.displayedNames, 0, Project.Emotion.SHAME, Project.Emotion.FEAR);
					if (innocence > 66) {
						say(t, "Um, they're just Thralls, so I shouldn't have anything to worry about!  Right...?");
					} else if (innocence > 33) {
						say(t, "These ones might be too strong for me...");
					} else {
						say(t, "I see.  Forcing me to face human foes with inhuman power...");
					}
				} else if (obedience < 61) {
					Project.changePortrait(gender, type, Project.displayedCivilians[0], true, w, Project.displayedNames, 0, Project.Emotion.FEAR, Project.Emotion.STRUGGLE);
					if (dignity > 66) {
						say(t, "I... I'll surrender right away...");
					} else if (dignity > 33) {
						say(t, "I'll do whatever you want, just don't let them have me!");
					} else {
						say(t, "No!  Stop!  Don't make them any stronger!");
					}
				} else if (obedience < 81) {
					Project.changePortrait(gender, type, Project.displayedCivilians[0], true, w, Project.displayedNames, 0, Project.Emotion.SHAME, Project.Emotion.FEAR);
					if (innocence > 66) {
						say(t, "I'm afraid...  They might kill me...");
					} else if (innocence > 33) {
						say(t, "If you want them to hurt me... then that's your right...");
					} else {
						say(t, "I... I will submit to their punishment...");
					}
				} else {
					Project.changePortrait(gender, type, Project.displayedCivilians[0], true, w, Project.displayedNames, 0, Project.Emotion.JOY, Project.Emotion.FOCUS);
					if (innocence > 66) {
						say(t, "It's okay if you let them play with me all day!");
					} else if (innocence > 33) {
						say(t, "Aah, they're going to hurt me so much...!");
					} else {
						say(t, "To be punished by mere humans...  How delightfully humiliating...!");
					}
				}
			} else if (nextTraining == 14) {
				if (obedience < 20) {
					...
					if (dignity > 66) {
						Project.changePortrait(gender, type, Project.displayedCivilians[0], true, w, Project.displayedNames, 0, Project.Emotion.SHAME, Project.Emotion.STRUGGLE);
						say(t, "This...  This hurts, Demon Lord... but I can't stop you...");
					} else if (dignity > 33) {
						Project.changePortrait(gender, type, Project.displayedCivilians[0], true, w, Project.displayedNames, 0, Project.Emotion.NEUTRAL, Project.Emotion.JOY);
					...
					Project.changePortrait(gender, type, Project.displayedCivilians[0], true, w, Project.displayedNames, 0, Project.Emotion.FOCUS, Project.Emotion.JOY);
					if (dignity > 66) {
						say(t, "Yes!  Yes!  I want the whole world to know that I'm yours!");
					} else if (dignity > 33) {
						say(t, "I'm happy that you're going to the effort of showing this to people around the world.");
					} else {
						say(t, "This is great, everyone will be so shocked when they see this!");
					}
				}
			} else if (nextTraining == 15) {
				if (obedience < 20) {
					Project.changePortrait(gender, type, Project.displayedCivilians[0], true, w, Project.displayedNames, 0, Project.Emotion.ANGER, Project.Emotion.NEUTRAL);
					if (innocence > 66) {
						say(t, "I-I'm not gonna feel good no matter what you do!  Watch me!");
					} else if (innocence > 33) {
						say(t, "You think you can break me by making me cum!?  Just how weak do you think I am!?");
					} else {
						say(t, "Hmph.  I doubt that you can inflict the level of pleasure required to break my mind.");
					}
				} else if (obedience < 40) {
					if (innocence > 66) {
						Project.changePortrait(gender, type, Project.displayedCivilians[0], true, w, Project.displayedNames, 0, Project.Emotion.FEAR, Project.Emotion.STRUGGLE);
					...
					Project.changePortrait(gender, type, Project.displayedCivilians[0], true, w, Project.displayedNames, 0, Project.Emotion.FEAR, Project.Emotion.SHAME);
					if (dignity > 66) {
						say(t, "I-If you want to pleasure me right now... then I can't really resist...");
					} else if (dignity > 33) {
						say(t, "This is going to drive me crazy...!");
					} else {
						say(t, "W-Wait!  If you touch me there right now, I'll...!");
					}
				} else if (obedience < 81) {
					Project.changePortrait(gender, type, Project.displayedCivilians[0], true, w, Project.displayedNames, 0, Project.Emotion.NEUTRAL, Project.Emotion.JOY);
					if (morality > 66) {
						say(t, "Well, I'd rather be serving you, but I suppose this is what you want from me right now...");
					} else if (morality > 33) {
						say(t, "Is this... a reward...?");
					} else {
						say(t, "Heh...  I can't really complain about feeling good...");
					}
				} else {
					Project.changePortrait(gender, type, Project.displayedCivilians[0], true, w, Project.displayedNames, 0, Project.Emotion.JOY, Project.Emotion.FOCUS);
					if (dignity > 66) {
						say(t, "Yes!  Yes!  Thank you, Demon Lord, thank you!");
					} else if (dignity > 33) {
						say(t, "I've been dreaming of doing this with the Demon Lord...");
					} else {
						say(t, "Aaah, feeling good with the Demon Lord...!");
					}
				}
			} else if (nextTraining == 16) {
				if (obedience < 20) {
					Project.changePortrait(gender, type, Project.displayedCivilians[0], true, w, Project.displayedNames, 0, Project.Emotion.ANGER, Project.Emotion.STRUGGLE);
					if (innocence > 66) {
						say(t, "Stop messing with me!  I'm not a toy!");
					} else if (innocence > 33) {
						say(t, "I'm not going to be your fucking entertainment!");
					} else {
						say(t, "I refuse to cooperate with this farce!");
					}
				} else if (obedience < 40) {
					if (dignity > 66) {
						Project.changePortrait(gender, type, Project.displayedCivilians[0], true, w, Project.displayedNames, 0, Project.Emotion.FEAR, Project.Emotion.STRUGGLE);
					...
					if (dignity > 66) {
						Project.changePortrait(gender, type, Project.displayedCivilians[0], true, w, Project.displayedNames, 0, Project.Emotion.SHAME, Project.Emotion.NEUTRAL);
						say(t, "I-I'm alright, Demon Lord.  Just... a part of me still isn't used to this...");
					} else if (dignity > 33) {
						Project.changePortrait(gender, type, Project.displayedCivilians[0], true, w, Project.displayedNames, 0, Project.Emotion.FEAR, Project.Emotion.SHAME);
						say(t, "This sounds... pretty extreme, Demon Lord.  Um, not that I'm refusing, of course!");
					} else {
						Project.changePortrait(gender, type, Project.displayedCivilians[0], true, w, Project.displayedNames, 0, Project.Emotion.FOCUS, Project.Emotion.JOY);
						say(t, "It's alright, Demon Lord, I don't really mind serving you like this!");
					}
				} else {
					Project.changePortrait(gender, type, Project.displayedCivilians[0], true, w, Project.displayedNames, 0, Project.Emotion.FOCUS, Project.Emotion.JOY);
					if (dignity > 66) {
						say(t, "I'll help you put on an amazing show for them, Demon Lord!");
					} else if (dignity > 33) {
						say(t, "Yes, please use my body to give them an incredible show!");
					} else {
						say(t, "Hahahah, I can't wait to see the look on their faces when I'm filled with your tentacles!");
					}
				}
			} else if (nextTraining == 17) {
				if (obedience < 20) {
					Project.changePortrait(gender, type, Project.displayedCivilians[0], true, w, Project.displayedNames, 0, Project.Emotion.ANGER, Project.Emotion.STRUGGLE);
					if (morality > 66) {
						say(t, "I'll keep fighting you until my last breath!");
					} else if (morality > 33) {
						say(t, "Graaah!  Bring it on!");
					} else {
						say(t, "I'll do ten times worse to you once I break free!");
					}
				} else if (obedience < 40) {
					Project.changePortrait(gender, type, Project.displayedCivilians[0], true, w, Project.displayedNames, 0, Project.Emotion.FEAR, Project.Emotion.SHAME);
					if (dignity > 66) {
					...
					Project.changePortrait(gender, type, Project.displayedCivilians[0], true, w, Project.displayedNames, 0, Project.Emotion.FEAR, Project.Emotion.STRUGGLE);
					if (dignity > 66) {
						say(t, "Aah, w-wait, there's no need to go that far!");
					} else if (dignity > 33) {
						say(t, "Oh no, no no no!");
					} else {
						say(t, "No, no!  I'm scared!  Help!");
					}
				} else if (obedience < 81) {
					Project.changePortrait(gender, type, Project.displayedCivilians[0], true, w, Project.displayedNames, 0, Project.Emotion.SHAME, Project.Emotion.STRUGGLE);
					if (morality > 66) {
						say(t, "D-Did I do something wrong?  If this is my punishment, then...");
					} else if (morality > 33) {
						say(t, "Did I do something you didn't like!?  P-Please, tell me!");
					} else {
						say(t, "I... I don't like this, Demon Lord.  You'd better let me hurt some more people to make up for it...");
					}
				} else {
					Project.changePortrait(gender, type, Project.displayedCivilians[0], true, w, Project.displayedNames, 0, Project.Emotion.FOCUS, Project.Emotion.JOY);
					if (confidence > 66) {
						say(t, "This training helps toughen my body for service to the Demon Lord!");
					} else if (confidence > 33) {
						say(t, "Thank you for showing me firsthand how to make people suffer!");
					} else {
						say(t, "Yes...  Make me suffer...!");
					}
				}
			}
		} else if (lacksConsent && consentBlock) {
			if (nextTraining == 0) {
				Project.changePortrait(gender, type, Project.displayedCivilians[0], true, w, Project.displayedNames, 0, Project.Emotion.ANGER, Project.Emotion.NEUTRAL);
				if (deviancy < 20) {
					if (morality > 66) {
				...
				} else if (hostility < 35) {
					Project.changePortrait(gender, type, Project.displayedCivilians[0], true, w, Project.displayedNames, 0, Project.Emotion.NEUTRAL, Project.Emotion.SHAME);
					if (morality > 66) {
				...
				if (deviancy < 50) {
					if (innocence > 66) {
						...
						say(t, "That sounds too extreme to actually feel good.  ");
					} else {
					...
					if (morality > 66) {
						...
						say(t, "Isn't there a chance of getting pregnant if I do this with you?  ");
					} else {
						say(t, "I don't feel like getting pregnant with your child.  ");
					}
				} else {
					if (confidence > 66) {
						say(t, "That would normally sound like fun, but I'm not letting you take me for granted.  ");
					} else if (confidence > 33) {
						say(t, "If I feel like having sex with you, I'll be the one to let you know.  ");
					} else {
						say(t, "I-I'm not broken to the point that I'd be willing to have sex with the Demon Lord...  ");
					}
				}
				Project.changePortrait(gender, type, Project.displayedCivilians[0], true, w, Project.displayedNames, 0, Project.Emotion.NEUTRAL, Project.Emotion.FOCUS);
				if (innocence > 66) {
					say(t, "If I see your body, I'm gonna run as fast as I can!");
				} else if (innocence > 33) {
					say(t, "If it's a body like that, I'm pretty sure I'll be able to get away before you can catch me.");
				} else {
					say(t, "I doubt that an insemination-type body is capable of catching me if I focus on fleeing.");
				}
			} else if (nextTraining == 14) {
				if (disgrace < 70) {
					if (innocence > 66) {
						say(t, "That would be like asking the whole world to make fun of me!  ");
					} else if (innocence > 33) {
						say(t, "Humiliating myself in front of the entire world is where I draw the line.  ");
					} else {
						say(t, "Such a large-scale humiliation would have the potential to completely sever the flow of psychic energy to my body.  ");
					}
				} else if (hostility < 70) {
					if (dignity > 66) {
						say(t, "If I let this happen, then even my old fans outside the Demonic hive will know that I was never a true hero.  ");
					} else if (dignity > 33) {
						say(t, "This would be like declaring war on the entire human world...  ");
					} else {
						say(t, "Having fun in the hive is one thing, but I don't want to be a propaganda tool for Demons worldwide.  ");
					}
				} else {
					if (dignity > 66) {
						say(t, "Hmph.  It's not like I enjoy being humiliated.  ");
					} else if (dignity > 33) {
						say(t, "If I'm going to send a message out to the rest of the world, I want it to be on my own terms.  ");
					} else {
						say(t, "Pointless.  I'll pass.  ");
					}
				}
				Project.changePortrait(gender, type, Project.displayedCivilians[0], true, w, Project.displayedNames, 0, Project.Emotion.NEUTRAL, Project.Emotion.FOCUS);
				if (confidence > 66) {
					say(t, "A Demon body loaded down with all that communication equipment shouldn't be able to hold me for long.");
				} else if (confidence > 33) {
					say(t, "I'll fight back.  Or at least I should be able to get away.");
				} else {
					say(t, "I might not be able to fight back much, but at least I can smash the body enough to stop the broadcast...");
				}
			} else if (nextTraining == 16) {
				if (deviancy < 60) {
					if (confidence > 66) {
						...
						say(t, "... No, this sounds like too much for me.  ");
					} else {
						say(t, "Th-This sounds like it'd just... break me.  ");
					}
				} else if (disgrace < 60) {
					if (dignity > 66) {
						say(t, "I really don't want anyone knowing how much I enjoy your tentacles...  ");
					} else if (dignity > 33) {
						say(t, "It might feel good... but it's too extreme to just let everyone watch.  ");
					} else {
						say(t, "I don't see any point in bringing a whole crowd of people into this.  ");
					}
				} else {
					if (confidence > 66) {
						say(t, "You think you can make me work for you just by giving me pleasure?  Keep your tentacles away or I'll cut them off!  ");
					} else if (confidence > 33) {
						say(t, "If you want to show off what you can do with your tentacles, then just use some random person.  ");
					} else {
						say(t, "Nnng...  I can still... resist this kind of temptation!  ");
					}
				}
				Project.changePortrait(gender, type, Project.displayedCivilians[0], true, w, Project.displayedNames, 0, Project.Emotion.NEUTRAL, Project.Emotion.FOCUS);
				if (innocence > 66) {
					say(t, "Your squishy arms won't be able to hold me!");
				} else if (innocence > 33) {
					say(t, "I won't hold still and let you grab me.");
				} else {
					say(t, "A noncombat form like this should be trivial to defeat or evade.");
				}
			}
		} else if (lacksConsent) {
			if (nextTraining == 0) {
				if (obedience < 20) {
					Project.changePortrait(gender, type, Project.displayedCivilians[0], true, w, Project.displayedNames, 0, Project.Emotion.ANGER, Project.Emotion.NEUTRAL);
					if (hostility < 20) {
				...
				if (obedience < 20) {
					...
					if (deviancy < 10) {
						...
						Project.changePortrait(gender, type, Project.displayedCivilians[0], true, w, Project.displayedNames, 0, Project.Emotion.FEAR, Project.Emotion.NEUTRAL);
						if (dignity > 66) {
				...
				if (obedience < 60) {
					if (deviancy < 50) {
						if (innocence > 66) {
							...
							say(t, "I don't want to have sex with you in the middle of all this!  ");
						} else {
							say(t, "We may have gone this far, but I have no interest in sexual relations with you right now.  ");
						}
					} else {
						if (innocence > 66) {
							say(t, "That sounds like it would feel pretty good...  Ah!  But I can't!  ");
						} else if (innocence > 33) {
							say(t, "If it were just having sex, I wouldn't mind.  But...  ");
						} else {
					...
					if (hostility < 50) {
						if (morality > 66) {
							...
							say(t, "It would be bad to get your slime in me right now...");
						} else {
							say(t, "I'm still not on your side.  If you want my body, you'll just have to rape me.");
						}
					} else {
						if (confidence > 66) {
							say(t, "I'd rather be fighting than doing something like... this.");
						} else if (confidence > 33) {
							say(t, "I'm still determined to resist at least this much!");
						} else {
					...
					if (hostility < 50) {
						if (morality > 66) {
							...
							say(t, "I really do want your slime inside me!  ");
						} else {
							say(t, "Ah... I want to feel the Demons inside of me...  ");
						}
					} else {
						if (morality > 66) {
							say(t, "Yes...  Whether it's for killing or for pleasure, I want you to use my body however you like...  ");
						} else if (morality > 33) {
							say(t, "I want to carry your babies, Demon Lord.  ");
						} else {
					...
					if (deviancy < 50) {
						if (innocence > 66) {
							...
							say(t, "Break me into the shape of your cock until I can't fight back anymore...");
						} else {
							say(t, "I doubt I'll be able to retain my sanity, though...  At least until I get used to this.");
						}
					} else {
						if (confidence > 66) {
							say(t, "I'm going to struggle...  I like it rough.");
						} else if (confidence > 33) {
							say(t, "The pleasure is going to make me go crazy...");
						} else {
							say(t, "I-If I start going crazy... I'm sorry, but it just feels too good...");
						}
					}
				}
			} else if (nextTraining == 14) {
				if (obedience < 60) {
					if (hostility < 70) {
						if (innocence > 66) {
							...
							say(t, "You can make me go into battle, but you can't make me go along with something this big.  ");
						} else {
							say(t, "To be seen doing this would be aiding the Demonic war effort on a truly worldwide scale...  I refuse!  ");
						}
					} else {
						if (morality > 66) {
							say(t, "I'm not going to give entertainment to the evil people who enjoy this sort of thing!  ");
						} else if (morality > 33) {
							say(t, "I'll happily kill for you, but I'm not going to become some sort of clown.  ");
						} else {
							say(t, "Anyone who would enjoy this is someone I hate.  I won't give them the satisfaction.  ");
						}
					}
					if (disgrace < 70) {
						Project.changePortrait(gender, type, Project.displayedCivilians[0], true, w, Project.displayedNames, 0, Project.Emotion.ANGER, Project.Emotion.FOCUS);
						if (innocence > 66) {
							say(t, "Even if you let the whole world see this, I'll impress them with how hard I resist!");
						} else if (innocence > 33) {
							say(t, "I'll give everyone my own message, and you won't like what I have to say...");
						} else {
							say(t, "If you try to make one of your own tools look weak in front of the whole world, you're the one who will suffer for it.");
						}
					} else {
						Project.changePortrait(gender, type, Project.displayedCivilians[0], true, w, Project.displayedNames, 0, Project.Emotion.SHAME, Project.Emotion.STRUGGLE);
						if (dignity > 66) {
							say(t, "Everyone already thinks badly enough of me...");
						} else if (dignity > 33) {
							say(t, "I'm so tired of being a laughingstock...");
						} else {
							say(t, "I can at least resist you this much...");
						}
					}
				} else {
					if (disgrace < 70) {
						if (innocence > 66) {
							say(t, "I-In front of the whole world?  That's... a lot of people!  ");
						} else if (innocence > 33) {
							say(t, "A worldwide broadcast is... a lot of pressure...  ");
						} else {
							say(t, "W-Wait, if you let everyone see me in this situation, won't my usefulness to you diminish?  ");
						}
					} else {
						if (dignity > 66) {
							say(t, "I'd love to show everyone how much I love doing this!  But...  ");
						} else if (dignity > 33) {
							say(t, "I'm not bothered by being humiliated anymore, Demon Lord.  But...  ");
						} else {
					...
					if (hostility < 70) {
						if (confidence > 66) {
							...
							say(t, "I'm afraid I'll disappoint you, and the more afraid I get, the more I start to panic...");
						} else {
							say(t, "I get stage fright.  I hope I don't panic...");
						}
					} else {
						if (innocence > 66) {
							say(t, "When I think of everyone laughing at me, I start to get mad and go crazy...");
						} else if (innocence > 33) {
							say(t, "It's hard to resist the urge to get angry and go off-script...");
						} else {
							say(t, "With so many people... my hateful urges begin to grow out of control... and I begin to act foolishly.");
						}
					}
				}
			} else if (nextTraining == 16) {
				if (obedience < 60) {
					if (deviancy < 60) {
						if (innocence > 66) {
							...
							say(t, "I don't think I want to deal with any more of your tentacles right now.  ");
						} else {
							say(t, "To turn this into such a depraved show would be going too far.  ");
						}
					} else {
						if (innocence > 66) {
							say(t, "If it were just the tentacles...  Well, yeah, it would feel pretty good.  But...  ");
						} else if (innocence > 33) {
							say(t, "If you just want me to do stuff with your tentacles later, then you have a deal.  But otherwise...  ");
						} else {
							say(t, "Do not mistake my enjoyment of your tentacles for a willingness to coopeate fully with your plans.  ");
						}
					}
					Project.changePortrait(gender, type, Project.displayedCivilians[0], true, w, Project.displayedNames, 0, Project.Emotion.NEUTRAL, Project.Emotion.ANGER);
					if (disgrace < 60) {
						if (innocence > 66) {
							say(t, "Letting everyone see all that would just feel too weird.");
						} else if (innocence > 33) {
							say(t, "You can demonstrate on someone else if you want.");
						} else {
							say(t, "I have too much to lose by putting on such a shameful display.");
						}
					} else {
						if (confidence > 66) {
							say(t, "I let you go too far in the past, and I'm not about to repeat that.");
						} else if (confidence > 33) {
							say(t, "People look down on me enough as it is.");
						} else {
							say(t, "I've already been humiliated enough...");
						}
					}
				} else {
					if (disgrace < 60) {
						if (dignity > 66) {
							say(t, "If people see me doing this, then they'll definitely know how devoted I am to you.  ");
						} else if (dignity > 33) {
							say(t, "It could be fun to show people how far I'm really willing to go for you...  ");
						} else {
							say(t, "I won't fight back, of course.  ");
						}
					} else {
						if (innocence > 66) {
							say(t, "I wanna do something even more shocking...  No one is surprised anymore...  ");
						} else if (innocence > 33) {
					...
					if (deviancy < 60) {
						Project.changePortrait(gender, type, Project.displayedCivilians[0], true, w, Project.displayedNames, 0, Project.Emotion.NEUTRAL, Project.Emotion.JOY);
						if (innocence > 66) {
							say(t, "It's just kinda intense-sounding, so don't be surprised if I pass out or go crazy or something.");
						} else if (innocence > 33) {
							say(t, "This is getting pretty extreme, though.  I'll try my best to keep my wits about me, but...");
						} else {
							say(t, "I suspect that I lack the mental fortitude to remain sane until the end of this demonstration, but please do not stop it on my account.");
						}
					} else {
						Project.changePortrait(gender, type, Project.displayedCivilians[0], true, w, Project.displayedNames, 0, Project.Emotion.LEWD, Project.Emotion.JOY);
						if (confidence > 66) {
							say(t, "If you do all this at once, then the pleasure might be enough to break even my mind...");
						} else if (confidence > 33) {
							say(t, "I'll try to cooperate with the show, but the more pleasure I feel, the harder it'll get...");
						} else {
							say(t, "I, um, p-probably won't be able to actually cooperate, though.  The pleasure is going to be too much...");
						}
					}
				}
			}
			...
			if (nextTraining == 0) {
				...
				if (flavorObedience() < 20) {
					...
					if (hostility < 10) {
						...
						Project.changePortrait(gender, type, Project.displayedCivilians[0], true, w, Project.displayedNames, 0, Project.Emotion.FOCUS, Project.Emotion.STRUGGLE);
					} else {
						if (confidence > 66) {
				...
				if (flavorObedience() < 60) {
					if (deviancy < 50) {
						if (innocence > 66) {
							...
							say(t, "Even if I turn you down, you'll still find a way to rape me.  ");
						} else {
							say(t, "I'm hardly in a position to refuse sex with you.  ");
						}
					} else {
						if (confidence > 66) {
							say(t, "Hmph.  I guess you can be my dildo.  ");
						} else if (confidence > 33) {
					...
					if (hostility < 50) {
						if (morality > 66) {
							...
							say(t, "Even now, your slime shouldn't be able to turn me into a Thrall, right?");
						} else {
							say(t, "I'm not afraid of your slime.");
						}
					} else {
						if (confidence > 66) {
							say(t, "Even under the influence of your slime, I doubt I can get much crueler.");
						} else if (confidence > 33) {
							say(t, "I doubt your slime can corrupt me more than it already has.");
						} else {
							say(t, "It's not like your slime can make me fall much further...");
						}
					}
					Project.changePortrait(gender, type, Project.displayedCivilians[0], true, w, Project.displayedNames, 0, Project.Emotion.NEUTRAL, Project.Emotion.FOCUS);
				} else {
					if (hostility < 50) {
						if (morality > 66) {
							...
							say(t, "Yes, I want your slime to change me... so that I won't feel so bad when I hurt people for your sake.  ");
						} else {
							say(t, "Yes...  Pour your hatred into my deepest parts... Make me even more evil...!  ");
						}
					} else {
						if (confidence > 66) {
							say(t, "Heh, I don't need your slime to make me obedient anymore.  But I won't turn it down.  ");
						} else if (confidence > 33) {
					...
					if (deviancy < 50) {
						Project.changePortrait(gender, type, Project.displayedCivilians[0], true, w, Project.displayedNames, 0, Project.Emotion.JOY, Project.Emotion.FOCUS);
						if (innocence > 66) {
							say(t, "Train me more in how naughty stuff works, okay?");
						} else if (innocence > 33) {
							say(t, "I'm so happy to become one with you.");
						} else {
							say(t, "I am not sure whether you feel pleasure in the same way as humans, but I certainly intend to try to make you feel good as well.");
						}
					} else {
						Project.changePortrait(gender, type, Project.displayedCivilians[0], true, w, Project.displayedNames, 0, Project.Emotion.LEWD, Project.Emotion.JOY);
						if (innocence > 66) {
							say(t, "This is gonna feel so good...");
						} else if (innocence > 33) {
							say(t, "I really, really want you to fill me up...");
						} else {
							say(t, "I must admit that the prospect of having you inside me is very... motivating...");
						}
					}
				}
			} else if (nextTraining == 13) {
				if (obedience < 50) {
					if (hostility < 60) {
						if (innocence > 66) {
							...
							say(t, "Once they've had their fun, they'll leave me alone.  ");
						} else {
							say(t, "Even if they've been empowered, mere humans can't match the cruelty of the Demons I've faced.  ");
						}
					} else {
						if (confidence > 66) {
							say(t, "If they come after me, I'll kill them all!  ");
						} else if (confidence > 33) {
							say(t, "I wonder how many I'll be able to kill.  ");
						} else {
							say(t, "H-Heh...  If they think I'll just let them hurt me, they've got something else coming...  ");
						}
					}
					Project.changePortrait(gender, type, Project.displayedCivilians[0], true, w, Project.displayedNames, 0, Project.Emotion.FOCUS, Project.Emotion.NEUTRAL);
					if (confidence > 66) {
						...
						say(t, "I... I won't run away!");
					}
				} else {
					if (confidence > 66) {
						say(t, "Heh, I'm pretty tough.  Tell them not to hold back.  ");
					} else if (confidence > 33) {
					...
					if (hostility < 60) {
						if (morality > 66) {
							...
							say(t, "I wonder what they'll do to me...");
						} else {
							say(t, "Maybe they can teach me a thing or two about hurting people...");
						}
					} else {
						if (innocence > 66) {
							say(t, "I know how mean people can be.");
						} else if (innocence > 33) {
							say(t, "I won't expect any mercy.");
						} else {
							say(t, "It will be a good reminder of how cruel all humans are at their core.");
						}
					}
					Project.changePortrait(gender, type, Project.displayedCivilians[0], true, w, Project.displayedNames, 0, Project.Emotion.NEUTRAL, Project.Emotion.FOCUS);
				}
				...
				if (obedience < 60) {
					if (disgrace < 70) {
						if (dignity > 66) {
							say(t, "Regardless of what you do to me, it means I can appear on a worldwide stage again.  ");
						} else if (dignity > 33) {
							say(t, "For the chance to tell the world the truth about you... it's worth it!  ");
						} else {
							say(t, "Heh.  You think I'm worried about my reputation?  ");
						}
					} else {
						if (confidence > 66) {
							say(t, "You've already convinced everyone that I'm a weakling, so seeing the real me can only make things better.  ");
						} else if (confidence > 33) {
					...
					if (hostility < 70) {
						if (innocence > 66) {
							...
							say(t, "I can deal with whatever abuse they throw at me.");
						} else {
							say(t, "I will find a way to turn this to my advantage.");
						}
					} else {
						if (innocence > 66) {
							say(t, "Yeah, it'll be fun to scare everyone and tell them they're gonna die!");
						} else if (innocence > 33) {
							say(t, "They should know how hopeless their situation is.");
						} else {
							say(t, "Let's give them all a taste of my despair...");
						}
					}
					Project.changePortrait(gender, type, Project.displayedCivilians[0], true, w, Project.displayedNames, 0, Project.Emotion.FOCUS, Project.Emotion.JOY);
				} else {
					if (hostility < 70) {
						if (morality > 66) {
							...
							say(t, "Everyone will know that I'm on the Demons' side...  No, no, that's fine.  ");
						} else {
							say(t, "If you want to show me to everyone, then I'll happily obey.  ");
						}
					} else {
						if (morality > 66) {
							say(t, "Since they oppose the Demon Lord, they deserve to be given some fear.  ");
						} else if (morality > 33) {
							say(t, "Do you want this to be a recruitment video?  Or should I just let them know how hopeless it is to oppose you?  ");
						} else {
							say(t, "Use me to show them the terror of the Demon Lord!  ");
						}
					}
					Project.changePortrait(gender, type, Project.displayedCivilians[0], true, w, Project.displayedNames, 0, Project.Emotion.FOCUS, Project.Emotion.JOY);
					if (disgrace < 70) {
						if (dignity > 66) {
							say(t, "I want the whole world to know that I serve you.");
						} else if (dignity > 33) {
							say(t, "If it helps bring more people to your side, then I'm happy to help.");
						} else {
							say(t, "I don't care what anyone else thinks.  Serving you is what's important.");
						}
					} else {
						if (confidence > 66) {
							say(t, "I'm proud of how much shame I can take!");
						} else if (confidence > 33) {
							say(t, "You can humiliate me as much as you want...");
						} else {
							say(t, "This is all I can really do for you now...");
						}
					}
				}
			} else if (nextTraining == 15) {
				if (obedience < 60) {
					if (deviancy < 70) {
						Project.changePortrait(gender, type, Project.displayedCivilians[0], true, w, Project.displayedNames, 0, Project.Emotion.NEUTRAL, Project.Emotion.SHAME);
						if (innocence > 66) {
							say(t, "This sounds... kinda weird.  Still, ");
						} else if (innocence > 33) {
							say(t, "You're going to keep going even after I ask you to stop, aren't you?  Well, ");
						} else {
							say(t, "Past a certain point, excessive pleasure begins to feel like pain.  Nonetheless, ");
						}
					} else {
						Project.changePortrait(gender, type, Project.displayedCivilians[0], true, w, Project.displayedNames, 0, Project.Emotion.FOCUS, Project.Emotion.JOY);
						if (innocence > 66) {
							say(t, "Aah, yes, yes, I wanna feel good even more!  Yeah, ");
						} else if (innocence > 33) {
							say(t, "Why would I refuse the chance to feel some pleasure?  Yeah, ");
						} else {
							say(t, "This sort of thing is why I stilll deign to serve you.  Yes, ");
						}
					}
					if (confidence > 66) {
						say(t, "let's get started already!");
					} else if (confidence > 33) {
						say(t, "I won't stop you.");
					} else {
						say(t, "I suppose I can give in.");
					}
				} else {
					if (morality > 66) {
						say(t, "Do I really deserve a reward?  ");
					} else if (morality > 33) {
						say(t, "You must be happy with me, right?  ");
					} else {
					...
					if (deviancy < 70) {
						if (confidence > 66) {
							...
							say(t, "Please train me until I can handle it without flinching!");
						} else {
							say(t, "Well, my body might not be able to take it, but I'm still grateful!");
						}
					} else {
						if (innocence > 66) {
							say(t, "I like... being able to feel happy and forget everything...");
						} else if (innocence > 33) {
							say(t, "I'm going to cum until I can't even remember my own name...");
						} else {
							say(t, "My body is enslaved to your pleasure...");
						}
					}
				}
			} else if (nextTraining == 16) {
				if (obedience < 60) {
					if (disgrace < 60) {
						if (innocence > 66) {
							say(t, "Isn't this just an excuse to have everyone watch me do naughty things?  ");
						} else if (innocence > 33) {
							say(t, "You really want me to become a laughingstock, don't you?  ");
						} else {
							say(t, "This is an especially obvious attempt to sabotage my reputation.  ");
						}
					} else {
						if (confidence > 66) {
							say(t, "Putting on this kind of show is beneath me, no matter what everyone says.  ");
						} else if (confidence > 33) {
							say(t, "Even if you've convinced everyone that I'm only good for my body, that doesn't mean I have to go along with it.  ");
						} else {
					...
					if (deviancy < 60) {
						if (dignity > 66) {
							...
							say(t, "I'll go along with this, but I'm not happy about it.");
						} else {
							say(t, "Well, whatever, it's not important enough to make me refuse.");
						}
					} else {
						if (innocence > 66) {
							say(t, "Um, but as long as it feels good, I guess it's alright.");
						} else if (innocence > 33) {
							say(t, "We can keep going until I cum a few times, but we're stopping after that.");
						} else {
							say(t, "You had better give me plenty of pleasure in return.");
						}
					}
				} else {
					Project.changePortrait(gender, type, Project.displayedCivilians[0], true, w, Project.displayedNames, 0, Project.Emotion.FOCUS, Project.Emotion.JOY);
					if (deviancy < 60) {
						if (innocence > 66) {
							...
							say(t, "I'll do my best, even if it seems pretty extreme.  ");
						} else {
							say(t, "Yes, my body certainly needs further training.  ");
						}
					} else {
						if (innocence > 66) {
							say(t, "This is gonna be so much fun!  ");
						} else if (innocence > 33) {
							say(t, "This sounds like a good time to me!  ");
						} else {
							say(t, "It may prove difficult to assist with the show, given that I'll be paralyzed by continuous orgasm, but I will certainly try.  ");
						}
					}
					if (disgrace < 60) {
						if (dignity > 66) {
							say(t, "I want to show everyone what I'm actually good at.");
						} else if (dignity > 33) {
							say(t, "I need to build a new reputation as the Demon Lord's plaything.");
						} else {
							say(t, "I'm happy that I can do this sort of thing now.");
						}
					} else {
						if (confidence > 66) {
							say(t, "I'll show them that I'm an even bigger pervert than they thought I was!");
						} else if (confidence > 33) {
							say(t, "This is what I'm supposed to be good at.");
						} else {
							say(t, "Everyone knows that I'm just the Demon Lord's sex toy, after all...");
						}
					}
				}
			} else if (nextTraining == 17) {
				if (obedience < 70) {
					if (confidence > 66) {
						say(t, "I... I can take it.  ");
					} else if (confidence > 33) {
						say(t, "No point in resisting.  ");
					} else {
						say(t, "Fine.  Do what you want with me...  ");
					}
					if (disgrace < 50) {
						if (dignity > 66) {
							say(t, "I'm still going to try to keep my voice in, though...");
						} else if (dignity > 33) {
							say(t, "People are going to be really surprised when they see me like this...");
						} else {
							say(t, "I just wish you weren't using this as entertainment for the Thralls...");
						}
					} else {
						if (dignity > 66) {
							say(t, "I guess this is what I'm known for now...");
						} else if (dignity > 33) {
							say(t, "I've become a disgrace...");
						} else {
							say(t, "I'm gonna scream really loud, though...");
						}
					}
					Project.changePortrait(gender, type, Project.displayedCivilians[0], true, w, Project.displayedNames, 0, Project.Emotion.SHAME, Project.Emotion.STRUGGLE);
				} else {
					if (disgrace < 50) {
						if (dignity > 66) {
							say(t, "Yes, this should help everyone learn that I'm nothing more than your toy.  ");
						} else if (dignity > 33) {
							say(t, "I'll scream as loud as you want me to.  ");
						} else {
							say(t, "Hah, everyone's going to be surprised when they see how weak I actually am.  ");
						}
					} else {
						if (innocence > 66) {
							say(t, "I'm just happy to be spending time with the Demon Lord!  ");
						} else if (innocence > 33) {
							say(t, "Yes, please train me hard.  ");
						} else {
							say(t, "I'm honored that you're going to the effort of disciplining me.  ");
						}
					}
					if (morality > 66) {
						say(t, "I deserve this.");
					} else if (morality > 33) {
						say(t, "Torture me until none of my original personality is left...");
					} else {
						say(t, "This just proves I'm your favorite.");
					}
					Project.changePortrait(gender, type, Project.displayedCivilians[0], true, w, Project.displayedNames, 0, Project.Emotion.JOY, Project.Emotion.NEUTRAL);
				}
			}
		}
		say(t, "\"");
		...
		if (consenting == false || lacksConsent == false || consentBlock == false) {
			...
			if (consenting == false || lacksConsent) {
				for (int i = 0; i < 4; i++) {
					if (i != 2) {
		...
		for (int i = 1; i < currentTraining.length; i = i+2) {
			if (currentTraining[i]) {
				helpless = true;
			}
		}
		if (consenting == false || helpless) {
			append(w, t, "\n\n{mainName} is helpless, so you can proceed without {hisHer} consent.");
		} else if (lacksConsent && consentBlock == false) {
			append(w, t, "\n\nThis type of training doesn't require {mainName}'s cooperation, so you can proceed without {hisHer} consent.");
		} else if (lacksConsent) {
			append(w, t, "\n\nThis type of training requires {mainName}'s cooperation, so you cannot continue.");
		}
	...
	public void executeTraining(final JTextPane t, final JPanel p, final JFrame f, final WorldState w, final SaveData s, final Boolean[] currentTraining, int nextTraining, final Boolean consenting, long gainedExpertise) {
		...
		if (currentTraining[3] && consenting == false) {
			tied = true;
		}
		...
		Boolean inPublic = currentTraining[0] || currentTraining[2] || currentTraining[4] || currentTraining[5] || currentTraining[6] || currentTraining[7] || currentTraining[10] || currentTraining[11] || currentTraining[13] || currentTraining[16] || currentTraining[17];
		Boolean stimulated = currentTraining[3] || currentTraining[9] || (currentTraining[6] && orgasmsGiven >= 1000) || currentTraining[12] || currentTraining[15] || currentTraining[16];
		Boolean servicing = (currentTraining[0] && (helpless || orgasmsGiven >= 1000)) || currentTraining[10] || (currentTraining[6] && (orgasmsGiven >= 1000 || consenting == false));
		Boolean bodyPresent = currentTraining[12] || currentTraining[14] || currentTraining[15] || currentTraining[16] || currentTraining[17];
		Boolean penile = (stimulated && gender != Gender.FEMALE);
		Boolean clitoral = (stimulated && gender == Gender.FEMALE);
		Boolean anal = false;
		Boolean vaginal = false;
		int currentTypes = 0;
		String organ = "penis";
		...
		Boolean orgasming = (stimulated || vaginal || anal) && timesOrgasmed > 0;
		Boolean fighting = (consenting == false && peopleInjured > 0);
		Boolean oral = servicing && (fighting == false || obedience >= 20);
		int penetrations = 0;
		if (oral) {
			penetrations++;
		}
		...
		Boolean pained = currentTraining[1] || currentTraining[7] || currentTraining[11];
		if (((currentTraining[0] || nextTraining == 0) && (servicing || stimulated)) || currentTraining[4] || currentTraining[5] || nextTraining == 4 || nextTraining == 5 || ((currentTraining[6] || nextTraining == 6) && (consenting == false || orgasmsGiven >= 1000)) || currentTraining[10]) {
			int added = 15 + (int)(Math.random()*15);
			timesExposed += added;
			if (consenting) {
				timesExposedSelf += added;
			}
		...
		if (nextTraining == 0) {
			//can affect inPublic, servicing, body parts, orgasming, penetrations
			if (currentTraining[11]) {
				append(w, t, "The Demon whose tentacles surround {mainName} begins to release pheromones to entice the watching crowd to grow even more lustful.  ");
			} else if (inPublic) {
				append(w, t, "A shudder runs through the crowd of people surrounding {mainName}.  ");
			} else if (helpless) {
				append(w, t, "Subconsciously drawn in by the potent pheromones wafting off {hisHer} body, it's not long before several people loitering in the slums come across the room where {mainName} lays helpless.  ");
			} else {
				append(w, t, "{mainName} knows the effect the pheromones will have on those around {himHer}, but {heShe} still goes out in public.  It isn't long before {heShe}'s attracted a crowd.  ");
			}
			if (penetrations > 0) {
				if (fighting && obedience < 20) {
					append(w, t, "One attacker, succumbing to his amplified lust, tries again to shove his cock into {mainName}'s mouth, but {mainName} bares {hisHer} teeth in warning, and the attacker wisely chooses to back off rather than press the issue.  ");
				} else {
					if (consenting && currentTraining[6]) {
						append(w, t, "{mainName} finds {himHer}self smirking slightly around {hisHer} mouthful of cock, enjoying the pitifully pleasured expression on its owner's face as it becomes almost painfully sensitive.  ");
					} else if (penetrations > 1) {
						append(w, t, "The people fucking {mainName} begin to thrust with renewed intensity, causing {himHer} to groan with every impact deep inside {himHer}.  ");
					} else if (oral) {
						append(w, t, "{HeShe} whimpers around {hisHer} mouthful of cock as {heShe} feels a pair of hands roughly grabbing {hisHer} hair, and the man {heShe}'s servicing begins to fuck {hisHer} throat with urgent intensity.  ");
					} else {
						append(w, t, "{HisHer} eyes go wide with anticipation as the man fucking {himHer} abruptly seizes {hisHer} hips, and then {heShe}'s crying out with every thrust into {hisHer} ");
						if (vaginal) {
							w.append(t, "pussy.  ");
						} else {
							w.append(t, "ass.  ");
				...
				if (fighting && helpless && (obedience < 20)) {
					append(w, t, "One of the men immediately pulls out his cock and grabs {mainName} by the hair, but {mainName} snaps {hisHer} teeth shut, and the would-be attacker quickly rethinks his plan.  ");
					servicing = true;
				} else if (helpless) {
					append(w, t, "One of the men grabs {mainName} by the hair and lifts {hisHer} face up so that it's level with the man's cock.  When {mainName} feels the shaft pressing against {hisHer} lips, {heShe} reflexively opens them and allows it inside.  ");
					oral = true;
					penetrations++;
					servicing = true;
				} else if (currentTraining[6]) {
					append(w, t, "Some of the men start desperately playing with themselves, while others get down on their knees and beg {mainName} to service them.  One man tries to grab {himHer}, but {heShe} just kicks the attacker between the legs, then glares down at his crumpled form with obvious disgust.  ");
				} else if (orgasmsGiven >= 1000 && (fighting == false || obedience >= 20)) {
					append(w, t, "Several of the men pull out their cocks, and {mainName} obediently gets down on {hisHer} knees and opens {hisHer} mouth for the closest of the group, allowing him to slide it inside and down {hisHer} throat.  ");
					oral = true;
					penetrations++;
					servicing = true;
				} else {
					append(w, t, "Several of the men pull out their cocks, but {mainName} backs away warily, taking a fighting stance.  None of the would-be attackers are brave enough to approach, but they do start to masturbate, unable to contain themselves as they leer at {hisHer} body.  ");
				}
				if (servicing) {
					if (currentTraining[3]) {
						if (vaginal && anal) {
							append(w, t, "Meanwhile, {mainName} is briefly relieved from the stimulation down below as two other men remove the vibrators from {hisHer} pussy and ass - but then {heShe}'s violently filled up again by a pair of cocks.  ");
						} else if (vaginal) {
							append(w, t, "Meanwhile, {mainName} is briefly relieved from the stimulation down below as another man removes the vibrator from {hisHer} pussy - only to shove his cock inside in its place.  ");
						} else if (anal) {
							append(w, t, "Meanwhile, {mainName} is briefly relieved from the stimulation down below as another man removes the anal vibrator from {hisHer} chastity belt - only to shove his cock inside in its place.  ");
						} else {
							append(w, t, "{mainName}'s chastity belt prevents any of the other men from penetrating {himHer}, which means that they can only try to relieve their lust by stroking themselves with handfuls of {hisHer} hair, grinding against {hisHer} limbs and torso, and masturbating close to {hisHer} face.  ");
						}
					} else {
						if (timesHadSex > 0 || (timesTortured > 0 && gender != Gender.MALE)) {
							append(w, t, "Meanwhile, the other men jostle for position behind {mainName}, with one of them finally pushing his way to the front of the pack and lining his cock up against {mainName}'s ");
							if (gender != Gender.MALE || timesHadSex == 0) {
								vaginal = true;
								append(w, t, "lower lips.  He forces his way inside, disregarding {mainName}'s pleasure, ramming {hisHer} deepest parts with wild abandon.  ");
								penetrations++;
								if (timesTortured > 0) {
									anal = true;
									append(w, t, "Noticing the free hole, yet another man gives the same treatment to {hisHer} ass.  ");
									penetrations++;
								}
							} else {
								anal = true;
								append(w, t, "tightly-puckered asshole.  He forces his way inside, quickly stretching the orifice open and ramming deep into {mainName}'s bowels with wild abandon.  ");
								penetrations++;
							}
						} else {
							append(w, t, "The other men grow bolder and bolder, stroking themselves with handfuls of {hisHer} hair, grinding against {hisHer} limbs and torso, and masturbating close to {hisHer} face.  ");
						}
					}
				}
			}
			if (vaginal || anal) {
				if (deviancy > 33) {
					if (innocence > 66) {
						append(w, t, "The pleasure soon overwhelms {mainName}, leaving {himHer} moaning and bucking {hisHer} hips like an animal in heat.");
					} else if (innocence > 33) {
						append(w, t, "{mainName}'s well-trained body welcomes the pleasure that grows with every impact deep inside {himHer}, and soon enough {heShe}'s cooperating wholeheartedly.");
					} else {
						append(w, t, "{mainName} surrenders {himHer}self to the pleasure, pursuing the sweet release that {heShe} feels growing closer and closer with every thrust inside.");
					}
				} else if (consenting) {
					if (innocence > 66) {
						append(w, t, "{mainName} still isn't entirely comfortable with the feeling of getting fucked, but {hisHer} discomfort slowly melts away as the pleasure builds.");
					} else if (innocence > 33) {
						append(w, t, "Some part of {mainName} is still mortified that {heShe}'s enjoying a gangbang this much, but that doesn't stop {himHer} from bucking {hisHer} hips in time with each thrust.");
					} else {
						append(w, t, "{mainName} tries as best {heShe} can to reciprocate as {heShe}'s fucked, but {hisHer} movements gradually become jerky and erratic as {heShe} approaches {hisHer} peak.");
					}
				} else {
					if (innocence > 66) {
						append(w, t, "{mainName} is scared of the pleasure growing inside {himHer}, but {hisHer} thrashing efforts to escape only succeed at impaling {himHer} even further.");
					} else if (innocence > 33) {
						append(w, t, "{mainName} is disgusted to recognize the signs that {hisHer} body is beginning to welcome being raped.");
					} else {
						append(w, t, "{mainName} closes {hisHer} eyes and tries to turn {hisHer} mind away from the feeling of being so roughly penetrated, but no matter what {heShe} does, the pleasure continues to grow, causing {hisHer} face to flush and {hisHer} breathing to grow ragged.");
					}
				}
				w.append(t, "\n\n");
				if (timesOrgasmed == 0) {
					if (vaginal && anal) {
						append(w, t, "The feeling of the two cocks inside {himHer} beginning to spurt their cum into {himHer} ");
					} else {
						append(w, t, "The feeling of the cock inside {himHer} beginning to spurt its cum into {hisHer} deepest place ");
					}
					append(w, t, "causes {himHer} to shudder briefly with a small peak of pleasure - not quite an orgasm, but close enough that {heShe} can't help but wonder what kinds of experiences {heShe}'s been missing.  ");
				} else if (dignity > 66) {
					append(w, t, "When {mainName} cums, {heShe} instinctively tries to hide it, but ");
					if (vaginal && anal) {
						append(w, t, "{hisHer} partners can feel the muscles in {hisHer} pussy and anus squeeze down on them, and they laugh and comment on how nice and tight {heShe} is as they cum inside.  ");
					} else if (vaginal) {
						append(w, t, "{hisHer} pussy still squeezes down on the penetrating cock, pushing it over the edge so that it cums deep into {hisHer} womb.  ");
					} else {
						append(w, t, "{hisHer} ass still squeezes down on the penetrating cock, pushing it over the edge so that it cums deep into {hisHer} bowels.  ");
					}
					timesOrgasmed++;
				} else if (dignity > 33) {
					append(w, t, "{mainName} can't hide {hisHer} orgasm, shuddering and spasming as {heShe}'s filled and covered with another layer of cum.  ");
					timesOrgasmed++;
				} else {
					append(w, t, "{mainName} thrashes wildly as {heShe} cums, {hisHer} orgasmic spasms ");
					if (vaginal && anal) {
						append(w, t, "milking the cocks inside {himHer} until they spurt out their loads into {hisHer} pussy and asshole both.  ");
					} else if (vaginal) {
						append(w, t, "milking {hisHer} partner's cock of every drop of semen until {hisHer} womb is completely full.  ");
					} else {
						append(w, t, "milking {hisHer} partner's cock of every drop of semen until {hisHer} ass has been completely filled.  ");
					}
					timesOrgasmed++;
				}
				if (hostility < 20) {
					if (morality > 66) {
						if (oral) {
							append(w, t, "{mainName} starts to ");
						} else {
							append(w, t, "{mainName} tries to pull the cock out of {hisHer} mouth for a moment to ");
						}
						if (consenting && currentTraining[6]) {
						...
						append(w, t, "but before {heShe} has a chance, ");
						if (vaginal && anal) {
							w.append(t, "another pair of cocks has been thrusted ");
						} else {
							w.append(t, "another cock is thrust ");
						}
						append(w, t, "inside {himHer} with complete disregard for {hisHer} comfort.");
					} else {
						if (vaginal && anal) {
						...
						append(w, t, "out, {mainName} is ready to heave a sigh of relief, but {heShe}'s horrified to be abruptly penetrated again, {hisHer} aching body being given no chance to enjoy the afterglow.");
					}
				} else {
					if (morality > 66) {
						append(w, t, "Exhausted from the climax, {mainName} wants to take a break, but {heShe} finds {himHer}self spreading {hisHer} legs again, knowing that it's pointless to expect the crowd to care about {hisHer} wishes.");
					} else if (morality > 33) {
						append(w, t, "{mainName}'s body is especially sensitive in the afterglow, but {heShe} knows that the crowd won't spare any thought to {hisHer} own desires.  All {heShe} can do is prepare {himHer}self to be taken again.");
					} else {
						append(w, t, "When the next man steps forward to fuck {himHer}, {mainName} plants {hisHer} foot against {hisHer} prospective partner's chest, holding {himHer} at bay for a moment while {heShe} tries to recover.");
					}
				}
				...
				if (currentTraining[8]) {
					append(w, t, "  With the telepathy informing them of just how intensely the penetration is affecting {himHer}, the men grow bolder and bolder.");
				}
				...
				if (oral) {
					if (currentTraining[3]) {
						if (deviancy > 33) {
							if (innocence > 66) {
								append(w, t, "When the vibrators attached to {mainName}'s nipples and " + target + " abruptly turn up to a higher intensity, {hisHer} squeak of surprise is muffled by the cock in {hisHer} mouth.");
							} else if (innocence > 33) {
								append(w, t, "{mainName} loses {himHer}self in the pleasure of sucking a cock, giving {himHer}self over to the sensation of the vibrators against {hisHer} nipples and " + target + ".");
							} else {
								append(w, t, "The vibrators against {hisHer} nipples and " + target + " alone aren't enough to push {mainName} over the edge, but the fact that {heShe}'s sucking a cock turns {himHer} on more and more.");
							}
						} else if (consenting) {
							if (innocence > 66) {
								append(w, t, "{mainName} still isn't entirely comfortable with the feeling of a cock in {hisHer} mouth, but {heShe} can't deny that the vibrators against {hisHer} nipples and " + target + " feel really, really good.");
							} else if (innocence > 33) {
								append(w, t, "{mainName} is reluctant at first, but with the vibrators against {hisHer} nipples and " + target + " spurring {himHer} on, {heShe} starts to take the cock deeper and deeper into {hisHer} throat.");
							} else {
								append(w, t, "At first, {mainName} is entirely focused on servicing the cock as quickly as possible in order to get this over with, but the relentless pulsing of the vibrators against {hisHer} nipples and " + target + " soon cause {hisHer} movements to become more eager and passionate.");
							}
						} else {
							if (innocence > 66) {
								append(w, t, "The unwashed cock tastes awful, and {mainName} just wants to get it out of {hisHer} mouth as soon as possible, but the vibrators against {hisHer} nipples and " + target + " are making it harder and harder to focus.");
							} else if (innocence > 33) {
								append(w, t, "{mainName} stubbornly refuses to actively service the cock in {hisHer} mouth, but it makes little difference, as the breathy moans and involuntary shudders elicited by the vibrators against {hisHer} nipples and " + target + " cause {himHer} to unwillingly stimulate the shaft.");
							} else {
								append(w, t, "{mainName} attempts to retreat into {hisHer} own mind in order to avoid thinking about what {heShe}'s being forced to do, but the pleasure from the vibrators against {hisHer} nipples and " + target + " continues to build, and {heShe} enjoys it on some level.");
							}
						}
					}
					if (currentTraining[9]) {
						append(w, t, "  The influence of the aphrodisiac causes even the inside of {hisHer} mouth to feel like an erogenous zone.");
					}
				} else {
					if (currentTraining[3]) {
						if (innocence > 66) {
							append(w, t, "As if in response, the vibrators against {mainName}'s nipples and " + target + " begin to stimulate {himHer} even more intensely, and {hisHer} reflexive struggling to get them off just makes it even worse.");
						} else if (innocence > 33) {
							append(w, t, "Even as {heShe} stubbornly denies them the use of {hisHer} mouth, the pleasure of the vibrators against {hisHer} nipples and " + target + " continues to build, and soon {heShe}'s reduced to desperately clenching {hisHer} teeth as {heShe} feels the peak approach.");
						} else {
							append(w, t, "When the stimulation from the vibrators against {hisHer} nipples and " + target + " grows too strong, {mainName} turns {hisHer} face toward the floor before opening {hisHer} mouth to moan, knowing that {hisHer} attackers will be looking for any chance to force a gag onto {himHer}.");
						}
					} else if (currentTraining[9]) {
						if (innocence > 66) {
							append(w, t, "However, even as {heShe} refuses to couple with the men, the lust caused by the aphrodisiac is unbearable, and {mainName} tries to discreetly reach underneath {hisHer} clothes, utterly failing to hide the fact that {heShe}'s started to rub {himHer}self down there.");
						} else if (innocence > 33) {
							append(w, t, "With the effects of the aphrodisiac, however, even the rubbing of {hisHer} clothes against {hisHer} sensitive places is too much to bear.");
						} else {
							append(w, t, "But even without the men touching {himHer}, the aphrodisiac causes {hisHer} pleasure to mount higher and higher.  No direct stimulation is necessary.");
						}
					}
				}
				w.append(t, "\n\n");
				if (timesOrgasmed == 0) {
					append(w, t, "Even for {mainName}'s inexperienced body, the stimulation doesn't take long to produce a reaction.  {HeShe} shudders and moans softly, then blinks and looks around in confusion, uncertain about what {heShe} just experienced.  It was too small to be called a true orgasm, but the memory of the pleasure will stay with {himHer}.  ");
				} else if (dignity > 66) {
					append(w, t, "{mainName} tries to hold back {hisHer} orgasm until ");
					if (oral) {
						append(w, t, "the man fucking {hisHer} throat finishes, ");
					} else {
						append(w, t, "{hisHer} would-be attackers aren't looking, ");
					}
					append(w, t, "but that just makes it all the more spectacular when it finally overwhelms {himHer}.  {HeShe} coughs and sputters as ");
					if (gender == Gender.FEMALE) {
						append(w, t, "{hisHer} juices squirt down {hisHer} thighs");
					} else {
						append(w, t, "{hisHer} cum paints the ground");
					}
					w.append(t, ", prompting raucous laughter from the crowd.  ");
					timesOrgasmed++;
				} else if (dignity > 33) {
					append(w, t, "When the inevitable orgasm finally comes, {mainName} tries to take it in stride, {hisHer} thighs squeezing together as ");
					if (gender == Gender.FEMALE) {
						append(w, t, "{hisHer} juices drip down between them.  ");
					} else {
						append(w, t, "{hisHer} cum spurts onto the ground.  ");
					}
					timesOrgasmed++;
				} else {
					append(w, t, "Finally, {heShe} cums hard, ");
					if (oral) {
						append(w, t, "groaning loudly around {hisHer} partner's cock");
					} else {
						append(w, t, "clenching {hisHer} teeth and groaning out loud");
					}
					if (gender == Gender.FEMALE) {
						append(w, t, " as {hisHer} juices stream down {hisHer} thighs.  ");
					}
					timesOrgasmed++;
				}
				if (currentTraining[8]) {
					append(w, t, "The telepathic broadcast of the orgasm hits the men all at once, and the ones who haven't been satisfied yet jeer at {mainName} for putting {hisHer} own satisfaction ahead of theirs.  ");
				}
				if (hostility < 20) {
					if (morality > 66) {
						append(w, t, "{HisHer} head is swimming in the afterglow, and a part of {himHer} assumes that {heShe}'ll get to rest, but {heShe}'s shocked when {heShe} feels ");
						if (oral) {
							append(w, t, "cum suddenly spurting down {hisHer} throat.  It pulls out, only to be replaced immediately by another man who facefucks {himHer} even harder.");
						} else {
							append(w, t, "several men taking advantage of {hisHer} distraction to try to pry {hisHer} mouth open, while others seize {hisHer} hands and try to make {himHer} stroke their cocks.");
						}
					} else if (morality > 33) {
						append(w, t, "A wave of exhaustion hits {himHer}, and {heShe} desperately tries to signal for a break, but the lustful crowd ignores {himHer}, trying to force {hisHer} hands around their cocks with complete disregard for {hisHer} own desires.");
					}
				} else {
					if (morality > 66) {
						append(w, t, "{HeShe} knows now that the sight of {himHer} orgasming will only make the rest of the crowd more eager to pursue their own, and {heShe}'s already warily looking between the lustful faces around {himHer} to see who will be wanting to use {himHer} next.");
					} else if (morality > 33) {
						append(w, t, "{HeShe} tries to fight through {hisHer} growing exhaustion, knowing that the crowd won't let up.");
					} else {
						append(w, t, "With {hisHer} body satisfied and {hisHer} head feeling more clear, {heShe} tries to make {hisHer} exit, but much to {hisHer} annoyance, {heShe}'s boxed in on all sides by more people thrusting their cocks at {himHer} and demanding service.");
					}
				}
				Project.changePortrait(gender, type, Project.displayedCivilians[0], true, w, Project.displayedNames, 0, Project.Emotion.LEWD, Project.Emotion.STRUGGLE);
			} else if (oral) {
				if (hostility < 20) {
					if (currentTraining[6] && consenting) {
						append(w, t, "{mainName} is far too gentle for this to be considered part of the punishment, but {heShe} still can't help but take satisfaction in the sense of power {heShe} has over the Thrall who desperately wants to orgasm inside {hisHer} mouth.");
					} else if (morality > 66) {
						append(w, t, "{mainName} endures having {hisHer} throat violently raped until {hisHer} partner finishes, then tries to use the brief period while {hisHer} mouth is empty to ask for gentler treatment.  However, the next man cuts {himHer} off by thrusting inside.");
					} else if (morality > 33) {
						append(w, t, "The pain of having {hisHer} throat fucked causes {mainName} to wince and choke.  {HeShe} desperately tries to signal for {hisHer} partner to slow down, but {hisHer} frantic movements accomplish nothing more than drawing a few chuckles from the watching crowd.");
					}
				} else {
					if (currentTraining[6] && consenting) {
						append(w, t, "{mainName} moves {hisHer} lips more and more slowly across the cock in {hisHer} mouth, refusing to push its owner over the edge into orgasm.");
					}else if (morality > 66) {
						append(w, t, "With the cock pushing down {hisHer} throat, {mainName} is starting to have trouble breathing.  {HeShe} frantically tries to bob {hisHer} head and work {hisHer} tongue along the shaft, desperate for the man to finish so that {heShe} can have a chance to come up for air.");
					} else if (morality > 33) {
						append(w, t, "Having a cock repeatedly thrust down {hisHer} throat doesn't feel good at all, but {mainName} just endures it, {hisHer} eyes glazed over.  {HeShe} knows that there's no point in protesting.");
					} else {
						append(w, t, "The lust amplification causes the man thrusting down {mainName}'s throat to be exceptionally rough and merciless, prompting {mainName} to glare up at him with resentful fury.  {mainName} briefly considers biting down, but {heShe} knows that it would only make things much worse for {himHer}.");
					}
				}
				Project.changePortrait(gender, type, Project.displayedCivilians[0], true, w, Project.displayedNames, 0, Project.Emotion.STRUGGLE, Project.Emotion.FEAR);
			} else {
				append(w, t, "Tension hangs in the air as the men grow more and more irritated at {hisHer} refusal to service them.\n\n");
				if (currentTraining[4]) {
					append(w, t, "{mainName} tries to put on an intimidating presence and scare the crowd away, but {hisHer} efforts are sabotaged from the start by {hisHer} ridiculous appearance.  ");
					if (dignity > 66) {
						append(w, t, "{HisHer} cheeks begin to turn red as {heShe} realizes how ridiculous {heShe} must look, and the men catch on, beginning to catcall and taunt {himHer} just to see {hisHer} reactions.  ");
					} else if (dignity > 33) {
						append(w, t, "The more {heShe} tries to scare them off, the more they laugh and crowd around {himHer} even more.  ");
					} else {
						append(w, t, "Even though {heShe} normally wouldn't care how people see {himHer}, the fact that it's now putting {himHer} in danger causes {himHer} to clench {hisHer} fists in annoyance.  ");
					}
				} else {
					if (dignity > 66) {
						append(w, t, "{mainName} tries to intimidate them into leaving {himHer} alone, shouting threats and reminding them all of {hisHer} mighty reputation, but they no longer pay any heed to {himHer}, chatting amongst themselves as they make plans to forcibly tie {himHer} up.  ");
					} else if (dignity > 33) {
						append(w, t, "{mainName} tries to convince them to go away, alternating between promises and threats, but no one in the crowd is afraid of {himHer} anymore.  ");
					} else {
						append(w, t, "{mainName} soon lapses into sullen silence, glaring at the assembled crowd as it grows larger and larger.  They know that {heShe} can't truly stop them if they all decide to take {himHer} together, and {heShe} knows that {heShe} knows.  ");
					}
				}
				if (hostility < 20) {
					if (morality > 66) {
						append(w, t, "In the end, all {heShe} can do is appeal to their sense of right and wrong, and {hisHer} heart breaks a little when {heShe} sees how ineffective it is.");
					} else if (morality > 33) {
						append(w, t, "Although {heShe} tries to tell {himHer}self that these people are only acting like this because of the circumstances, {heShe} can't stop {himHer}self from feeling some anger at them.");
					}
				} else {
					if (morality > 66) {
						append(w, t, "In the end, all {heShe} can do is speak out loud directly to the Demon Lord, asking you to end this here before things get any worse.");
					} else if (morality > 33) {
						append(w, t, "Resentment boils in {hisHer} eyes.");
					} else {
						append(w, t, "{HeShe} tries to content {himHer}self with the knowledge that if they try to go any further, {heShe}'ll probably be able to hurt a few of them for it.");
					}
				}
				if (currentTraining[8]) {
					append(w, t, "  The telepathic broadcast tells everyone exactly how afraid {heShe} is that they'll press the issue.");
				}
				Project.changePortrait(gender, type, Project.displayedCivilians[0], true, w, Project.displayedNames, 0, Project.Emotion.ANGER, Project.Emotion.STRUGGLE);
			}
			if (vaginal) {
				int added = 3 + (int)(Math.random()*3);
				timesHadSex += added;
				orgasmsGiven += added;
			}
			if (anal) {
			...
			if (oral) {
				orgasmsGiven += 3 + (int)(Math.random()*3);
			}
		} else if (nextTraining == 1) {
			//can affect helpless, servicing, body parts, orgasming, pain
			if (helpless || (consenting && orgasmsGiven >= 1000)) {
				if (consenting == false) {
					if (confidence > 66) {
						append(w, t, "{mainName}'s struggles grow increasingly frantic and disjointed as {hisHer} mind buckles under the weight of dozens upon dozens of other people's thoughts and perceptions.  ");
					} else if (confidence > 33) {
						append(w, t, "The flood of thoughts and sensations that aren't {hisHer} own paralyzes {mainName}'s mind, and {hisHer} struggles to escape abruptly come to a complete halt.  ");
					} else {
						append(w, t, "{mainName}'s struggles, already feeble in the first place, come to a complete stop when foreign thoughts and feelings begin forcing their way into {hisHer} mind.  ");
					}
				} else if (tied) {
					if (confidence > 66) {
						append(w, t, "{mainName} reflexively strains against {hisHer} bonds as {hisHer} mind struggles to accommodate the sudden flood of new information from the new psychic linkages.  ");
					} else if (confidence > 33) {
						append(w, t, "{mainName} is already physically restrained, but {heShe} loses the ability to even think about resisting when the abrupt surge of sensations from other people makes it too hard for {himHer} to even keep track of which body belongs to {himHer}.  ");
					} else {
						append(w, t, "{mainName}'s mind puts up little resistance against the other minds suddenly intruding upon it, leaving {himHer} defenseless against the psychic flood.  ");
					}
				} else {
					if (confidence > 66) {
						append(w, t, "{mainName} spasms wildly as {hisHer} mind reflexively resists the flood of information from other brains bearing down upon it, but {heShe} soon goes completely limp.  ");
					} else if (confidence > 33) {
						append(w, t, "{mainName} immediately seizes up, unable to process the information rushing through {hisHer} mind as {heShe}'s connected to dozens of other people's thoughts and feelings.  ");
					} else {
						append(w, t, "{mainName} whimpers and clutches {hisHer} head, shaking it back and forth as intrusive thoughts and sensations seem to tear {himHer} out of {hisHer} own body.  ");
					}
				}
				if (currentTraining[6]) {
					append(w, t, "The disruption to {hisHer} consciousness causes {himHer} to lose any influence over the crowd, which continues to eagerly make use of {hisHer} body.");
				} else if (servicing) {
					append(w, t, "Unconcerned with whatever {heShe}'s going through, the crowd of people surrounding {himHer} continues to enjoy {hisHer} body.");
				} else if (inPublic) {
					append(w, t, "Laughter and jeers erupt from the surrounding crowd, mingled with murmured discussion as they watch what {heShe}'s going through with curiosity.");
				} else if (bodyPresent) {
					append(w, t, "{HeShe}'s completely helpless before you.");
				} else {
					append(w, t, "Even though there's no one nearby to take advantage of {hisHer} even greater vulnerability, the experiences {heShe}'s drawing from the people {heShe}'s linked with are intense enough on their own.");
				}
			} else {
				if (confidence > 66) {
					append(w, t, "{mainName} grits {hisHer} teeth as {hisHer} mind reflexively resists the flood of information from other brains bearing down upon it, but {heShe} soon collapses onto {hisHer} hands and knees.  ");
				} else if (confidence > 33) {
					append(w, t, "{mainName} immediately falls to {hisHer} knees, unable to process the information rushing through {hisHer} mind as {heShe}'s connected to dozens of other people's thoughts and feelings.  ");
				} else {
					append(w, t, "{mainName} whimpers and clutches {hisHer} head, shaking it back and forth as intrusive thoughts and sensations seem to tear {himHer} out of {hisHer} own body.  {HeShe} curls up on the ground, helpless.  ");
				}
				if (servicing) {
					append(w, t, "The crowd is disappointed that {heShe}'s stopped actively servicing them, but they content themselves by making use of {hisHer} limp, unresisting body.");
				} else if (currentTraining[0] || (currentTraining[6] && (consenting == false || orgasmsGiven >= 1000)) || currentTraining[10]) {
					servicing = true;
					if (currentTraining[6]) {
						append(w, t, "The crowd recognizes that this is their opportunity to turn the tables on {himHer}, even if only briefly.  ");
					} else {
						append(w, t, "The crowd recognizes that this is a rare opportunity, surging forward to take advantage of {hisHer} sudden helplessness.  ");
					}
					if (timesHadSex > 0) {
						if (gender == Gender.MALE) {
							anal = true;
					...
					} else if (gender == Gender.FEMALE && timesTortured > 0) {
						anal = true;
					}
					if (fighting == false || obedience >= 20) {
						oral = true;
					}
					if (anal) {
						append(w, t, "One attacker grabs {himHer} from behind, lifting {himHer} up with {hisHer} legs in the air.  Then, he lowers {mainName} down, ");
						if (hostility < 20) {
							if (morality > 66) {
								append(w, t, "and {mainName}'s pleas for {hisHer} attackers to stop this turn into a strangled cry as the man's cock slams into {hisHer} ass.  ");
								if (vaginal) {
									append(w, t, "{HeShe} tries to get them to at least spare {hisHer} pussy, only to be cut off once again as a second attacker rams his way in.  ");
								}
							} else if (morality > 33) {
								append(w, t, "slowly, allowing the rest of the crowd plenty of time to enjoy {mainName}'s expression of stunned disbelief as {hisHer} anus is impaled on the man's cock.  ");
								if (vaginal) {
									append(w, t, "By the time the second attacker penetrates {hisHer} pussy, {hisHer} eyes are downcast.  ");
								}
							}
						} else {
							if (morality > 66) {
								append(w, t, "enjoying the look of bitter betrayal in {hisHer} eyes as {hisHer} ass inevitably sinks onto {hisHer} attacker's cock.  ");
								if (vaginal) {
									append(w, t, "{HeShe} shudders with revulsion as a second attacker slides his cock into {hisHer} pussy.  ");
								}
							} else if (morality > 33) {
								append(w, t, "causing {mainName} to wince with resignation as {heShe} feels {hisHer} anus spread open by the waiting cock below.  ");
								if (vaginal) {
									append(w, t, "{HeShe} doesn't even react at first to a second attacker ramming himself into {hisHer} pussy.  ");
								}
							} else {
								append(w, t, "and {mainName}'s strained glare turns into an expression of shock as {hisHer} ass is forced open by {hisHer} attacker's cock.  ");
								if (vaginal) {
									append(w, t, "{HeShe} kicks angrily, only for {hisHer} legs to jerk and stiffen when a second attacker starts fucking {hisHer} pussy.  ");
								}
							}
						}
					} else if (vaginal) {
						append(w, t, "One attacker lifts {himHer} up from behind, placing his cock against {hisHer} pussy and then slowly lowering {himHer} donwward.  ");
						if (hostility < 20) {
							if (morality > 66) {
								append(w, t, "At first, {mainName}'s pleas for {hisHer} attackers to stop, but {hisHer} voice breaks with a strangled cry as the man's cock slams its way inside.  ");
							} else if (morality > 33) {
								append(w, t, "{mainName} wears an expression of stunned disbelief as {heShe}'s impaled on the man's cock.  ");
							}
						} else {
							if (morality > 66) {
								append(w, t, "A look of bitter betrayal simmers in {hisHer} eyes as {heShe} inevitably sinks onto {hisHer} attacker's cock.  ");
							} else if (morality > 33) {
								append(w, t, "{mainName} winces with resignation as {heShe} feels {hisHer} intimate folds forced open by the waiting cock below.  ");
							} else {
								append(w, t, "{mainName}'s strained glare turns into an expression of shock as {heShe}'s forced open by {hisHer} attacker's cock.  ");
							}
						}
					} else if (currentTraining[3]) {
						if (hostility < 20) {
							if (morality > 66) {
								append(w, t, "{mainName} tries to explain that the chastity belt that's been forced upon {himHer} means that they're wasting their time attacking {himHer}, but {heShe}'s surprised when they start taking out their rage on {hisHer} body.  ");
							} else if (morality > 33) {
								append(w, t, "{mainName} had expected that the chastity belt you forced on {himHer} would protect {himHer} from the worst of this kind of abuse, but it just makes the crowd angrier.  ");
							}
						} else {
							if (morality > 66) {
								append(w, t, "{mainName} holds {hisHer} head high in defiance, knowing that the chastity belt you forced on {himHer} will protect {himHer} from being raped, but {heShe} knows that the crowd won't just give up and leave {himHer} alone.  ");
							} else if (morality > 33) {
								append(w, t, "Even though {heShe}'s protected by a chastity belt, {mainName} knows that the crowd will find a way to hurt {himHer}.  ");
							} else {
								append(w, t, "{mainName} manages a cheeky smirk as {heShe} taunts them about being foiled by the chastity belt you put on {himHer}, but they're quick to punish {himHer} for {hisHer} insolence.  ");
							}
						}
					} else {
						if (hostility < 20) {
							if (morality > 66) {
								append(w, t, "{mainName} tries to keep a pure heart and a clear head, trusting {hisHer} Sexual Barrier to protect {himHer}, but {heShe}'s horrified by how determined the crowd is to abuse {himHer}.  ");
							} else {
								append(w, t, "{mainName}'s Sexual Barrier prevents {himHer} from being raped, but it begins to waver as {heShe} sees how determined the crowd is to hurt {himHer}, failing along with {hisHer} faith in humanity.  ");
							}
						} else {
							...
							if (morality > 66) {
								append(w, t, "{mainName} is forced to curl up into a ball as {heShe} desperately protects {hisHer} " + virginity + " - the last thing connecting {himHer} to the morals {heShe} once held.  ");
							} else if (morality > 33) {
								append(w, t, "{mainName}'s Sexual Barrier quickly dissolves in the heat of {hisHer} hatred for {hisHer} attackers, and the only thing protecting {hisHer} " + virginity + " is the last bit of desperate physical resistance {heShe} can muster.  ");
							} else {
								append(w, t, "{mainName} tries to fight to protect {hisHer} " + virginity + ", but the best {heShe} can do in {hisHer} current state is to clench {hisHer} hands tightly between {hisHer} thighs.  ");
							}
						}
					}
					if (oral) {
						if (w.tickle()) {
							append(w, t, "Tickling hands emerge from all directions, but when {heShe} opens {hisHer} mouth to laugh, a cock is immediately shoved inside.");
						} else {
							append(w, t, "Rough hands emerge from all directions to twist {hisHer} limbs and pummel {hisHer} body.  When {heShe} opens {hisHer} mouth to cry out in pain, a cock is immediately shoved inside, muffling {hisHer} voice.");
						}
					} else {
						if (w.tickle()) {
							append(w, t, "Tickling hands emerge from all directions, and it's all {heShe} can do to suppress {hisHer} laughter through gritted teeth, unwilling to allow {hisHer} attackers to rape {hisHer} mouth.");
						} else {
							append(w, t, "Rough hands emerge from all directions to twist {hisHer} limbs and pummel {hisHer} body.  When {heShe} opens {hisHer} mouth to cry out in pain, one of {hisHer} attackers almost manages to force his cock inside, but {mainName}'s teeth snap shut quickly enough to make him rethink his priorities.");
						}
					}
				} else if (inPublic) {
					append(w, t, "There's a smattering of laughter and conversation as the spectators enjoy seeing {himHer} subjected to this newest torment.");
				} else if (bodyPresent) {
					append(w, t, "{HeShe}'s completely helpless before you.");
				} else {
					append(w, t, "Even though there's no one nearby to take advantage of {hisHer} vulnerability, the experiences {heShe}'s drawing from the people {heShe}'s linked with are intense enough on their own.");
				}
			}
			if (currentTraining[8]) {
				append(w, t, "  Combined with the broadcast of {mainName}'s own thoughts outward, the bidirectional communication makes the experience even more intimate and horrifying.");
			}
			w.append(t, "\n\n");
			if ((vaginal || anal || oral) && servicing) {
				if (obedience < 20) {
					if (confidence > 66) {
						append(w, t, "Looking down at {himHer}self from the perspective of each man who fucks {himHer}, {mainName} is horrified and disturbed at how pathetic {heShe} looks.  ");
					} else if (confidence > 33) {
						append(w, t, "As {heShe} shares the senses of the ones fucking {himHer} in turn, {mainName} can't help but notice {hisHer} own small and fragile appearance.  ");
					}
				} else {
					if (confidence > 66) {
						append(w, t, "As {heShe} focuses on one of the people fucking {himHer}, {mainName} can feel how feeble {hisHer} reflexive struggles are, and {heShe} realizes once again that {heShe} was never as strong as {heShe} thought {heShe} was.  ");
					} else if (confidence > 33) {
						append(w, t, "By this point, {mainName} has already acknowledged {hisHer} own weakness, but feeling what it's like to fuck {hisHer} helpless body from the shared senses of the crowd drives the point home even further.  ");
					} else {
						append(w, t, "{mainName} is content to lose {himHer}self in the senses of the crowd caught up in the gangbang, taking the opportunity to dissocate {himHer}self from the weak, useless body that's never been able to put up any resistance to those who would abuse {himHer}.  ");
					}
				}
				if (deviancy > 33) {
					if (innocence > 66) {
						append(w, t, "{HeShe} quickly loses track of where {hisHer} senses end and the others' begin, simply enjoying each and every orgasm experienced by anyone in the gangbang.");
					} else if (innocence > 33) {
						append(w, t, "{HeShe} cums over and over again, overwhelmed by the amplified arousal streaming into {hisHer} brain from each and every member of the crowd.");
					} else {
						append(w, t, "{HeShe} finds {himHer}self mentally urging one of the men to fuck {himHer} harder and harder, and the sympathetic orgasm {heShe} feels as the man cums is almost strong enough to make {himHer} pass out.");
					}
					if (timesOrgasmed > 0) {
						timesOrgasmed += 5 + (int)(Math.random()*5);
					}
				} else {
					if (timesOrgasmed == 0) {
						append(w, t, "Even though {heShe}'s never orgasmed before, {heShe} still gets to feel the sensation vicariously several times over, and it's enough to leave {himHer} wanting more.");
					} else if (innocence > 66) {
						append(w, t, "The psychically-transmitted pleasure felt by the crowd is more disorienting than arousing, but {heShe}'s still dumbfounded by how much everyone seems to be enjoying {hisHer} body.");
					} else if (innocence > 33) {
						append(w, t, "With the stimulation to {hisHer} own body amplified by the arousal felt by all those around {himHer}, {mainName} is forced to cum again and again.");
					} else {
						append(w, t, "At first, {mainName}'s disgust prevents {hisHer} body from showing any signs of arousal, but the pleasure felt by all those {heShe}'s psychically connected to continues to seep into {hisHer} mind.  {HeShe} can't hold back the tide, and after the first time {heShe} cums, {heShe} starts to feel genuine arousal that makes it even easier to cum again and again.");
					}
					if (timesOrgasmed > 0) {
						timesOrgasmed += 5 + (int)(Math.random()*5);
					}
				...
				if (currentTraining[10]) {
					append(w, t, "  Yet even more than the orgasms of the relatively few men fucking {himHer} now, the built-up lust of the long line of Thralls waiting for their turn with {himHer} flows into {himHer}, and {heShe} finds it impossible to be satisfied.");
				}
				Project.changePortrait(gender, type, Project.displayedCivilians[0], true, w, Project.displayedNames, 0, Project.Emotion.LEWD, Project.Emotion.STRUGGLE);
			} else if (inPublic) {
				if (obedience < 20) {
					if (confidence > 66) {
						append(w, t, "Looking down at {himHer}self from the perspective of the crowd, {mainName} is horrified and disturbed at how pathetic {heShe} looks.  ");
					} else if (confidence > 33) {
						append(w, t, "As {heShe} shares the senses of the crowd, {mainName} can't help but notice {hisHer} own small and fragile appearance.  ");
					}
				} else {
					if (confidence > 66) {
						append(w, t, "As looks down at {himHer}self from other people's eyes, {mainName} can see how feeble {hisHer} reflexive struggles are, and {heShe} realizes once again that {heShe} was never as strong as {heShe} thought {heShe} was.  ");
					} else if (confidence > 33) {
						append(w, t, "By this point, {mainName} has already acknowledged {hisHer} own weakness, but seeing how helpless {heShe} is from an outside perspective drives the point home even further.  ");
					} else {
						append(w, t, "{mainName} is content to lose {himHer}self in the senses of the crowd watching {hisHer} torment, taking the opportunity to dissocate {himHer}self from the weak, useless body that's never been able to put up any resistance to those who would abuse {himHer}.  ");
					}
				}
				if (currentTraining[4]) {
					if (dignity > 66) {
						append(w, t, "As embarrassing as it was to be forced to wear {hisHer} current outfit, seeing it from the outside is even worse.");
					} else if (dignity > 33) {
						append(w, t, "{HeShe} was able to ignore {hisHer} embarrassing outfit easily enough before, but now that {heShe} sees {himHer}self from outside, it's far worse.");
					} else {
						append(w, t, "Forced to hear the crowd's internal monologues about how ridiculous {heShe} looks in the outfit you forced upon {himHer}, {mainName} feels even more uncomfortable.");
					}
				} else {
					if (dignity > 66) {
						append(w, t, "The sight is so shameful that {heShe} can't help but try to beg the crowd to look away, but the mental connection only goes in one direction, and {hisHer} pleas go unheard.");
					} else if (dignity > 33) {
						append(w, t, "A chorus of derogatory thoughts directed at {himHer} fills {hisHer} head, driving {himHer} insane.");
					} else {
						append(w, t, "Under the effects of your power, {heShe} can't ignore what people think about {himHer} anymore.");
					}
				}
				Project.changePortrait(gender, type, Project.displayedCivilians[0], true, w, Project.displayedNames, 0, Project.Emotion.SHAME, Project.Emotion.FEAR);
			} else {
				if (obedience < 20) {
					if (confidence > 66) {
						append(w, t, "{mainName} clenches {hisHer} teeth as {heShe} tries to cling to {hisHer} sense of self, but {heShe} feels {hisHer} sanity eroding away.  ");
					} else if (confidence > 33) {
						append(w, t, "{mainName} cries out unwillingly, blindly thrashing as {hisHer} mind loses its grasp on {hisHer} body.  ");
					}
				} else {
					if (confidence > 66) {
						append(w, t, "{mainName}'s once-strong mind is now completely porous and vulnerable to your manipulation, allowing itself to be stripped away from {hisHer} body and spread thin across many others.  ");
					} else if (confidence > 33) {
						append(w, t, "As {mainName}'s body lies abandoned, {hisHer} mind is yours to manipulate.  ");
					} else {
						append(w, t, "{mainName}'s mind is unable to put up even a token resistance, and soon it's completely at your mercy, subject to being shown whatever experiences you see fit.  ");
					}
				}
				if (hostility < 20) {
					if (morality > 66) {
						append(w, t, "{HisHer} horror at what {heShe} sees happening across your domain - violence, debauchery, and sin - causes {himHer} to recoil, and {heShe} gradually starts to return to {himHer}self.");
					} else {
						append(w, t, "{HeShe}'s forced to experience the worst of what's happening in neighboring buildings, seeing through the eyes of rapists and victims, thieves and murderers, and those who know what's happening but do nothing about it.  When {heShe} returns to {hisHer} own body, {heShe}'s gasping and covered with sweat.");
					}
				} else {
					if (morality > 66) {
						append(w, t, "{HeShe} doesn't run away from what you force {himHer} to see, taking in every detail of the horrible abuses being constantly committed around {himHer}.  {HeShe} uses the painful experience to burn away the last of {hisHer} faith in humanity before {heShe} starts to return to {hisHer} body.");
					} else if (morality > 33) {
						append(w, t, "{HeShe} tries to resist, subconsciously afraid of leaving {hisHer} body unattended, but even as {heShe} squirms and struggles on the ground, the experiences {heShe}'s forced to live through ensure that {heShe} doesn't forget the cruel things humans do to each other.");
					} else {
						append(w, t, "{HeShe} isn't surprised by what {heShe} sees, having known all along that {heShe}'s surrounded by rapists, sadists, and weaklings who allow themselves and others to be trampled upon.  But as {heShe} starts to return to {hisHer} body, {hisHer} eyes burn with renewed contempt for the rest of humanity.");
					}
				}
				Project.changePortrait(gender, type, Project.displayedCivilians[0], true, w, Project.displayedNames, 0, Project.Emotion.STRUGGLE, Project.Emotion.FEAR);
			}
			...
			if (consenting) {
				if (currentTraining[6]) {
					append(w, t, "{mainName} steps back from the Thrall {heShe} was punishing, then turns to address the crowd, telling them that it's now {hisHer} own turn to be reminded of {hisHer} place.  ");
				} else if (oral) {
					append(w, t, "{mainName} swallows a load of cum from the cock down {hisHer} throat.  When its owner pulls out, {heShe} takes the opportunity to speak up before the next man to be serviced can step forward.  ");
				} else if (inPublic && helpless) {
					append(w, t, "{mainName} lifts {hisHer} head to look up at the gathered crowd, takes a deep breath, and then begins to speak.  ");
				} else if (helpless) {
					append(w, t, "One of your Thralls goes to fetch {mainName}'s helpless form, dragging {himHer} out into public.  The Thrall calls everyone's attention to {himHer}, and then steps aside to let {himHer} speak.  ");
				} else if (inPublic) {
					append(w, t, "{mainName} gets down on {hisHer} knees before the assembled crowd, bows {hisHer} head, and then begins to speak.  ");
				} else if (bodyPresent) {
					append(w, t, "{mainName} allows you to lead {himHer} out into public, where a crowd of people quickly takes notice of {himHer}.  {HeShe} gets down on {hisHer} knees, bows {hisHer} head, and then begins to speak.  ");
				} else {
					append(w, t, "{mainName} heads out into public, where a crowd of people quickly takes notice of {himHer}.  {HeShe} gets down on {hisHer} knees, bows {hisHer} head, and then begins to speak.  ");
				}
				if (disgrace < 20) {
					if (dignity > 66) {
						append(w, t, "{HeShe} tells everyone that {heShe}'s a fraud, undeserving of {hisHer} reputation, and that {heShe} needs to be punished for deceiving everyone for so long.");
					} else if (dignity > 33) {
						append(w, t, "{HeShe} shouts that {heShe}'s actually not strong at all, that {heShe} never should have been made one of the Chosen, and that {heShe}'s at the crowd's complete mercy.");
					}
				} else {
					if (dignity > 66) {
						append(w, t, "{HeShe} tells everyone at length about how {heShe} used to be famous but is now just a playtoy for the Demon Lord.");
					} else if (dignity > 33) {
						append(w, t, "{HeShe} reminds everyone that {heShe} was a fool for ever trying to stand up to the mighty Demon Lord.");
					} else {
						append(w, t, "{HeShe} speaks freely about how weak and frightened {heShe} feels, making sure that everyone present knows that {heShe}'s at their mercy.");
					}
				}
				if (currentTraining[8]) {
					append(w, t, "  If {hisHer} words alone weren't enough, the telepathic projection of {hisHer} own thoughts into the crowd's heads leaves them with now doubt that {heShe} truly has given up on resisting in {hisHer} heart.");
				}
			} else {
				if (oral) {
					append(w, t, "{mainName} coughs and chokes as the man fucking {hisHer} throat shoots a load of cum inside, but in the brief period between that man pulling out and the next man stepping forward, {heShe} starts shouting out loud, voicing {hisHer} defiance.  ");
				} else if (inPublic) {
					append(w, t, "When you order {mainName} to humiliate {himHer}self for the crowd, {heShe} just starts struggling, attempting to do the opposite out of pure spite.  ");
				} else if (bodyPresent) {
					append(w, t, "You drag {mainName}'s weakly struggling form out into public.  A crowd quickly gathers around {himHer}, gawking curiously, but {mainName} tries to stand up for {himHer}self despite the humiliating situation.  ");
				} else {
					append(w, t, "You send your Thralls to drag {mainName}'s helpless form out into public.  A crowd quickly gathers around {himHer}, gawking curiously, but {mainName} tries to stand up for {himHer}self despite the humiliating situation.  ");
				}
				if (disgrace < 20) {
					if (dignity > 66) {
						append(w, t, "{HeShe} claims that {heShe}'s here by {hisHer} own choice, and that it's part of a plan {heShe}'s enacting as one of the Chosen, but the feeble lie doesn't convince anyone.");
					} else {
						append(w, t, "{HeShe} insists that the Demon Lord will never break {hisHer} spirit, and that this method won't work either.");
					}
				} else {
					if (dignity > 66) {
						append(w, t, "{HeShe} rants about how the Demon Lord is just trying to make everyone think {heShe}'s weak, but that {heShe}'s actually still as strong as ever.");
					} else if (dignity > 33) {
						append(w, t, "{HeShe} insults you directly, claiming that the Demon Lord isn't actually so strong, and that {heShe} only lost to you because {heShe} got unlucky.");
					} else {
						append(w, t, "{HeShe} screams wordlessly, fighting with all {hisHer} might in hopes that the raw effort will leave an impression on everyone who sees.");
					}
				}
				if (currentTraining[8]) {
					append(w, t, "  Of course, with {hisHer} innermost thoughts being broadcasted into everyone's heads, the crowd can tell exactly how helpless and defeated {heShe} truly is.");
				}
			}
			if (currentTraining[10]) {
				append(w, t, "  The crowd of Thralls waiting to take their turn with {himHer} has grown quite large, and the ones in the back are unable to make out {hisHer} exact words, but the message comes across clearly, especially with what happens next.");
			}
			w.append(t, "\n\n");
			if ((vaginal || anal) && servicing) {
				if (vaginal && anal) {
					append(w, t, "The men penetrating {mainName}'s lower holes are too occupied with their own pleasure to even bother listening to {mainName}'s words, and their pace quickens as they approach their climaxes.  ");
				} else {
					append(w, t, "The man thrusting into {mainName} from behind is too occupied with his own pleasure to even bother listening to {hisHer} words, and his pace quickens as {heShe} approaches his climax.  ");
				}
				if (deviancy > 33) {
					if (innocence > 66) {
						append(w, t, "{mainName}'s voice becomes weaker and less coherent as the pace increases, and soon {heShe}'s just squealing and moaning in {hisHer} own pitiful orgasm.");
					} else if (innocence > 33) {
						append(w, t, "With all the training {hisHer} body has experienced, {mainName} can't resist the pleasure for long, and {heShe} quickly ends up forgetting all about the crowd, entirely focused on trying to cum as much as possible.");
					} else {
						append(w, t, "The more the pleasure mounts, the more difficulty {mainName} has controlling {hisHer} voice.  At first, {heShe} falls silent in an attempt to collect {himHer}self again, but soon {heShe}'s openly crying out in orgasm.");
					}
				} else if (hostility < 20) {
					if (morality > 66) {
						append(w, t, "The sheer apathy drives {mainName} to despair, and {heShe} soon loses the will to raise {hisHer} voice, hanging {hisHer} head as {heShe} tries to ignore the shameful pleasure building inside {himHer}.");
					} else {
						append(w, t, "{mainName}'s voice is broken up by grunts and soft moans, but when {heShe} tries to ask for more gentle treatment, there's no response.");
					}
				} else {
					if (morality > 66) {
						append(w, t, "{mainName} tries to continue regardless, but the distraction is too much to bear, and {heShe} ends up just biting {hisHer} lip as {heShe} tries to endure the rough stimulation.");
					} else if (morality > 33) {
						append(w, t, "It's not long before {mainName} loses {hisHer} motivation to speak, hanging {hisHer} head in exhaustion.");
					} else {
						append(w, t, "The combined pleasure and pain of the rough treatment eventually grows to the point of annoyance, but {mainName}'s only reaction is to halfheartedly try to kick behind {himHer}.  {HeShe} knows that there's no point in asking for mercy.");
					}
				}
				Project.changePortrait(gender, type, Project.displayedCivilians[0], true, w, Project.displayedNames, 0, Project.Emotion.STRUGGLE, Project.Emotion.LEWD);
			} else {
				if (currentTraining[4]) {
					if (dignity > 66) {
						append(w, t, "{mainName}'s voice dies out when {heShe} sees how everyone in the crowd is looking at {himHer}.  {HisHer} appearance, customized to exploit {hisHer} insecurities, paralyzes {hisHer} mind.  ");
					} else if (dignity > 33) {
						append(w, t, "{mainName}'s voice is soon drowned out by the derisive laughter of the crowd as they mock {hisHer} ridiculous appearance, and {hisHer} face goes red with shame.  ");
					} else {
						append(w, t, "{mainName}'s voice breaks with a startled cry as the crowd surges forward to grope, pinch, and write derogatory messages on the skin exposed by {hisHer} ridiculous outfit.  ");
					}
				} else {
					append(w, t, "At first, the crowd is interested, but they soon grow bored and begin pelting {mainName} with garbage and shouting over {himHer}.  ");
				}
				if (hostility < 20) {
					if (morality > 66) {
						append(w, t, "{mainName} is shocked speechless by how vicious they are.");
					} else {
						append(w, t, "{mainName} tries to keep going, but it's no use, as things have become so rowdy that {heShe} has no hope of making {himHer}self heard.");
					}
				} else {
					if (morality > 66) {
						append(w, t, "Once it's obvious that there's no point in trying to continue, {mainName} hangs {hisHer} head, conserving {hisHer} energy.");
					} else if (morality > 33) {
						append(w, t, "{mainName} bites {hisHer} lip in annoyance, but doesn't bother trying to continue.  {HeShe} knows that there's no point.");
					} else { 
						append(w, t, "{mainName} falls silent, taking a sort of grim satisfaction in seeing proof that {heShe} was right not to expect any sympathy from them.");
					}
				}
				Project.changePortrait(gender, type, Project.displayedCivilians[0], true, w, Project.displayedNames, 0, Project.Emotion.SHAME, Project.Emotion.STRUGGLE);
			}
			...
			//can affect helpless, stimulated, servicing, body parts, orgasming
			String target = "penis";
			if (gender == Gender.FEMALE) {
				target = "clit";
			}
			if (tied) {
				if (confidence > 66) {
					append(w, t, "{mainName} reflexively struggles against {hisHer} bonds as {heShe} feels a sudden vibration against {hisHer} most sensitive places.  Egg-shaped vibrators attach themselves to {hisHer} nipples and " + target + ", stimulating {himHer} incessantly.  ");
				} else if (confidence > 33) {
					append(w, t, "More and more straps wrap themselves around {mainName}, anchoring {himHer} in place and holding vibrating wands against {hisHer} nipples and " + target + ".  ");
				} else {
					append(w, t, "{mainName} releases an involuntary whimper as a trio of vibrating rings form around {hisHer} nipples and " + target + ", squeezing around them so tightly that it's almost painful.  ");
				}
			} else if (consenting == false) {
				if (confidence > 66) {
					append(w, t, "Countless ribbons begin to materialize in the air around {mainName}, fragile in appearance but strong enough to bind {hisHer} wrists and ankles and to hold vibrators against {hisHer} most sensitive parts.  ");
				} else if (confidence > 33) {
					append(w, t, "Handcuffs and anklecuffs abruptly appear on {mainName}'s body, instantly immobilizing {himHer}.  A moment later, they're joined by vibrating eggs stuck against {hisHer} nipples and " + target + ", and with {hisHer} limbs trapped, {heShe} can't do anything to pull them off.  ");
				} else {
					append(w, t, "Electrified rings appear around {mainName}'s nipples and " + target + ", causing {himHer} to squeak with startled pleasure as they deliver low pulses to their targets.  When {heShe} tries to escape, the pulses become so strong that {heShe}'s unable to resist.  ");
				}
			} else {
				if (confidence > 66) {
					append(w, t, "Rough ropes wrap themselves around {mainName}'s body, then tighten up abruptly enough to force a grunt out of {himHer}.  {HeShe} briefly manages to regain control of {himHer}self, only to gasp unwillingly as vibrating clamps attach themselves to {hisHer} nipples and " + target + ".  ");
				} else if (confidence > 33) {
					append(w, t, "{mainName} is startled by how quickly {heShe}'s surrounded by heavy chains that start binding {hisHer} body, but {heShe} does {hisHer} best to calm {himHer}self, even when small egg-shaped vibrators appear to adorn {hisHer} nipples and " + target + ".  ");
				} else {
					append(w, t, "{mainName} obediently lays down with {hisHer} wrists together above {hisHer} head and {hisHer} ankles spread wide apart.  Handcuffs appear above and a spreader bar below, joined by vibrating rings attached to {hisHer} nipples and " + target + ".  ");
				}
			}
			if (timesHadSex > 0) {
				if (gender == Gender.MALE) {
			...
			if (servicing) {
				if (vaginal && anal) {
					append(w, t, "The crowd, which had begun to grow bored with {hisHer} ruined holes, watches with renewed interest as a pair of dildoes appear from underneath {himHer} and fill {himHer} completely.");
				} else if (vaginal) {
					append(w, t, "{HisHer} exhausted, dripping pussy is filled almost unbearably full with a huge dildo which forces its way inside.");
				} else if (anal) {
					if (gender == Gender.MALE) {
						append(w, t, "{HisHer} exhausted, dripping asshole is filled almost unbearably full with a huge dildo which forces its way inside.");
					} else {
						append(w, t, "Mercifully, a chastity belt locks itself onto {himHer} in order to help preserve {hisHer} virginity - but an opening in the back allows a thick dildo to force its way into {hisHer} ruined anus, stretching it even wider.");
					}
				} else {
					append(w, t, "Mercifully, a chastity belt locks itself onto {himHer} in order to make it easier for {himHer} to defend {hisHer} ");
					if (gender == Gender.MALE) {
						w.append(t, "anal ");
					}
					append(w, t, "virginity - but it also means that {heShe}'s even more helpless to stop the pleasure.");
				}
			} else if (inPublic) {
				if (vaginal && anal) {
					append(w, t, "The crowd watches with intense interest as {hisHer} pussy and asshole both are forced open by a pair of dildoes, filling {himHer} to the point that a bulge is visible in {hisHer} lower belly.");
				} else if (vaginal) {
					append(w, t, "{HisHer} slit is forced open, a huge dildo penetrating {himHer} and pushing its way deep inside while the crowd masturbates to the sight.");
				} else if (anal) {
					if (gender == Gender.MALE) {
						append(w, t, "{HisHer} tight anus is forced open, a huge dildo penetrating {himHer} and pushing its way deep inside while the crowd masturbates to the sight.");
					} else {
						append(w, t, "A chastity belt locks itself around {hisHer} hips, enticing the crowd with the reminder of {hisHer} virginity - but also leaving {hisHer} backside bare so that a thick dildo can force its way inside.");
					}
				} else {
					append(w, t, "A chastity belt locks itself around {hisHer} hips, enticing the crowd with the reminder of {hisHer} ");
					if (gender == Gender.MALE) {
						w.append(t, "anal ");
					}
					append(w, t, "virginity - and also rendering {himHer} even more helpless to stop the pleasure.");
				}
			} else {
				if (vaginal && anal) {
					append(w, t, "{HeShe} cries out as {hisHer} pussy and asshole are both forced open by a pair of dildoes, so that {heShe}'s stimulated from the outside and the inside at the same time.");
				} else if (vaginal) {
					append(w, t, "{HisHer} slit is forced open, a huge dildo penetrating {himHer} and pushing its way deep inside.");
				} else if (anal) {
					if (gender == Gender.MALE) {
						append(w, t, "{HisHer} tight anus is forced open, a huge dildo penetrating {himHer} and pushing its way deep inside.");
					} else {
						append(w, t, "A chastity belt locks itself around {hisHer} hips, ensuring that {heShe} can't prevent the stimulation - but an opening in the back allows a thick dildo to force its way into {hisHer} ass.");
					}
				} else {
					append(w, t, "A chastity belt locks itself around {hisHer} hips, ensuring that {heShe} can't prevent the stimulation.");
				}
			}
			if (currentTraining[8]) {
				append(w, t, "  Several of the Thralls clutch their own crotches, shuddering with the reverberations of lust and pleasure echoing from {mainName}'s mind.");
			}
			if (currentTraining[0] || (currentTraining[6] && (consenting == false || orgasmsGiven >= 1000)) || currentTraining[10]) {
				servicing = true;
			}
			w.append(t, "\n\n");
			if (obedience < 20) {
				if (confidence > 66) {
					append(w, t, "{mainName} still isn't used to feeling so vulnerable, and the knowledge that there's nothing {heShe} can do to stop this makes the experience that much more intense.  ");
				} else if (confidence > 33) {
					append(w, t, "{mainName} is unable to stop {himHer}self from struggling wildly, although even {heShe} isn't sure whether {heShe}'s trying to escape the vibration or to press {himHer}self against it more firmly.  ");
				}
			} else {
				if (confidence > 66) {
					append(w, t, "Even though {mainName} has come to submit {himHer}self to your will, {heShe}'s still put mentally off-balance by this kind of vulnerability.  ");
				} else if (confidence > 33) {
					append(w, t, "Even more than the vibrating toys, the fact that {heShe}'s completely powerless now turns {mainName} on.  ");
				} else {
					append(w, t, "For the naturally-submissive {mainName}, this kind of situation is inherently arousing.  ");
				}
			}
			if (inPublic) {
				if (vaginal || anal) {
					...
					if (deviancy > 33) {
						if (innocence > 66) {
							append(w, t, "{HeShe} cums over and over again, tongue hanging out of {hisHer} mouth, and when you finally remove the " + dildo + ", {heShe} arches {hisHer} back without thinking, presenting {himHer}self to be taken by the whole crowd.");
						} else if (innocence > 33) {
							append(w, t, "But with how much {hisHer} libido has grown, this alone isn't enough to satisfy {himHer}.  {HeShe} groans with disappointment at the removal of the " + dildo + ", looking back at the crowd with glazed eyes and begging them to rape {himHer}.");
						} else {
							append(w, t, "{HeShe} no longer even wants to resist the pleasure, and {heShe} fucks the " + dildo + " enthusiastically.  When you finally pull out, {heShe} smirks deliriously and provocatively waggles {hisHer} bottom for the crowd's benefit.");
						}
					} else {
						if (timesOrgasmed == 0) {
							append(w, t, "The penetration is still uncomfortable for {hisHer} inexperienced, untrained body, but {heShe}'s still breathing heavily by the time you pull out to give the crowd a clear look at {hisHer} intimate places.");
						} else if (innocence > 66) {
							append(w, t, "{HeShe} soon shudders with orgasm, then goes limp as you pull the " + dildo + " out.  {HeShe} can't even imagine feeling a stronger pleasure than that, but the crowd is eager to teach {himHer}.");
						} else if (innocence > 33) {
							append(w, t, "{HeShe} isn't comfortable with cumming in front of such a large crowd, but {heShe} soon succumbs anyway, and the sight of {himHer} writhing and moaning excites everyone who sees it - especially when you pull out and move aside the " + dildo + ".");
						} else {
							append(w, t, "{HeShe} allows {himHer}self to cum, knowing that there's no point in resistance, but then looks back in trepidation as you pull out the " + dildo + " and present {hisHer} defenseless body to the crowd.");
						}
					}
					if (currentTraining[10]) {
						append(w, t, "  Spontaneous applause erupts from the crowd of Thralls, and after the show, they're more eager than ever to have their turn with {mainName}.");
					}
				} else {
					if (deviancy > 33) {
						if (innocence > 66) {
							append(w, t, "{HeShe} cums over and over again, tongue hanging out of {hisHer} mouth, and when you give {himHer} a break from the vibration, {heShe} thoughtlessly starts trying to squirm out of {hisHer} chastity belt, unable to think of anything other than getting filled up inside.");
						} else if (innocence > 33) {
							append(w, t, "But with how much {hisHer} libido has grown, this alone isn't enough to satisfy {himHer}.  {HeShe} groans with disappointment when you shut off the vibration, begging you to unlock {hisHer} belt and let the crowd violate {himHer}.");
						} else {
							append(w, t, "{HeShe} no longer even wants to resist the pleasure, and {heShe} bucks {hisHer} hips enthusiastically whenever you pulse the vibration.  Even after the vibration dies down, {heShe} looks eager for more torment.");
						}
					} else {
						if (timesOrgasmed == 0) {
							append(w, t, "The intense vibration is still uncomfortable for {hisHer} inexperienced, untrained body, but {heShe}'s still breathing heavily once you shut off the vibration in order to avoid desensitizing {himHer}.");
						} else if (innocence > 66) {
							append(w, t, "{HeShe} soon shudders with orgasm, then goes limp as you briefly turn the vibration off.  {HeShe} can't even imagine feeling a stronger pleasure than that - at least not yet.");
						} else if (innocence > 33) {
							append(w, t, "{HeShe} isn't comfortable with cumming in front of such a large crowd, but {heShe} soon succumbs anyway, and the sight of {himHer} writhing and moaning excites everyone who sees it, prompting many of them to reach their own orgasms as well.");
						} else {
							append(w, t, "{HeShe} allows {himHer}self to cum, knowing that there's no point in resistance.  When the vibration briefly shuts off, {heShe} looks relieved at first, but {heShe} quickly realizes that you surely have more torments in store.");
						}
					}
				}
			} else {
				if (deviancy > 33) {
					if (innocence > 66) {
						append(w, t, "{HeShe} cums over and over again, tongue hanging out of {hisHer} mouth.  Babbling incoherently, {heShe} offers {himHer}self to anyone who can make {himHer} feel even better, but there's no one nearby to take {himHer}.");
					} else if (innocence > 33) {
						append(w, t, "But with how much {hisHer} libido has grown, this alone isn't enough to satisfy {himHer}.  {HeShe} whines pitifully, begging someone to rape {himHer}, but there's nobody within earshot to take {himHer} up on the offer.");
					} else {
						append(w, t, "{HeShe} no longer even wants to resist the pleasure, and {heShe} bucks {hisHer} hips enthusiastically as {heShe} cums.  The fact that {heShe}'s alone makes {himHer} even more willing to indulge in {hisHer} debauchery.");
					}
				} else {
					if (timesOrgasmed == 0) {
						append(w, t, "The intense vibration is still uncomfortable for {hisHer} inexperienced, untrained body, but {heShe}'s still breathing heavily once you shut off the vibration in order to avoid desensitizing {himHer}.");
						...
						append(w, t, "{HeShe} still reflexively denies the pleasure you're forcing upon {himHer}, but {heShe} soon succumbs to the inevitable orgasm anyway.  Over and over again, you give {himHer} bursts of intense stimulation to bring {himHer} to the peak, until {hisHer} body begins to crave it.");
					} else {
						append(w, t, "{HeShe} allows {himHer}self to cum, knowing that there's no point in resistance.  When the vibration briefly shuts off, {heShe} looks relieved at first, but {heShe} quickly realizes that you surely have more torments in store.");
					}
				}
			...
			if (currentTraining[6]) {
				if (consenting) {
					...
					append(w, t, "  The Thralls' treatment of {mainName} only grows more and more harsh as they take revenge for how {heShe} was hurting them before.");
				}
			}
			if (timesOrgasmed > 0) {
				timesOrgasmed += 5 + (int)(Math.random()*5);
			...
			Project.displayedCivilians[0] = false;
			//can affect inPublic
			if (consenting) {
				if (inPublic) {
					append(w, t, "{mainName} transforms at your command, allowing you plenty of opportunity to influence the form {hisHer} Forsaken garb will take.  ");
				} else {
					append(w, t, "{mainName} heads out into public, gathers the attention of a crowd, and then transforms.  However, under the influence of your power, not even {heShe} can predict what form {hisHer} outfit will take.  ");
				}
			} else {
				append(w, t, "In {hisHer} efforts to escape, {mainName} has transformed into {hisHer} Forsaken form, hoping to draw on whatever residual psychic energy {heShe} can.  However, under the influence of your power, the psychic energy which forms {hisHer} garb begins to shift and warp.  ");
				if (inPublic == false) {
					append(w, t, "The commotion caused by {hisHer} struggles quickly draws a curious crowd, and they're surprised by what they see.  ");
				}
			}
			if (currentTraining[3]) {
				//petplay
				append(w, t, "{HeShe}'s left practically naked, as the material of {hisHer} outfit migrates to {hisHer} limbs, becoming tight bindings that force {himHer} to crawl on all fours.  A headband resembling animal ears appears on {hisHer} head");
				if (anal) {
					if (servicing) {
						append(w, t, ", and the cum flowing out of {hisHer} ass is plugged inside by a vibrator with a tail attached to the other end.");
					} else {
						append(w, t, ", and the vibrator up {hisHer} ass sprouts a long tail.");
					}
				} else {
					w.append(t, ".");
				}
				...
				//public use
				append(w, t, "{HisHer} clothes vanish almost completely, revealing that tattoos spelling out derogatory messages have appeared across {hisHer} skin.  Arrows pointing toward {hisHer} mouth and crotch proclaim that {heShe}'s free for public use.");
			} else {
				//personality-based
				if (disgrace < 20) {
					append(w, t, "{HisHer} clothes become skimpy in the extreme, consisting of a few strings around {hisHer} chest and waist which hold thin pieces of cloth over {hisHer} nipples and crotch.  The cloth hangs down loosely, so that even moving {hisHer} body slightly to the side causes them to shift and reveal everything.  The sexual nature of the outfit makes it even more embarrassing than simple nudity.");
				} else if (obedience < 20) {
					append(w, t, "A fur-lined bodysuit spreads across {hisHer} skin, with openings over {hisHer} nipples and groin, complete with an animal ear headband, a fluffy tail, and paw-like gloves that render {hisHer} hands practically useless.  A collar with an attached leash completes the ensemble, marking {himHer} as the Demon Lord's pet.");
				} else if (deviancy < 20) {
					append(w, t, "Latex spreads across {hisHer} skin, but stops short of {hisHer} nipples and groin.  The outfit is fitted tightly enough that it squeezes and emphasizes {hisHer} chest and butt, and adhesive strips built into the material spread open {hisHer} ass ");
					if (gender == Gender.MALE == false) {
						w.append(t, "and pussy ");
					}
					w.append(t, "so that the pink insides are plainly visible.");
				} else if (hostility < 20) {
					append(w, t, "{HeShe}'s enveloped in tentacles which caress {hisHer} chest and frame {hisHer} groin in a mockery of modesty, marking {himHer} as a servant of the Demons.");
				} else {
					append(w, t, "Countless ropes wrap themselves intricately around {hisHer} torso, the crisscrossing pattern managing to avoid actually covering any of {hisHer} private parts.  It's less of an outfit and more of an ornament for {hisHer} naked body.");
				}
			}
			if (consenting && currentTraining[6]) {
				append(w, t, "  Several of the Thralls stifle snorts of laughter, but although the outfit makes it impossible to respect {mainName}, it also inflames their lust even further, and they're eager to have {himHer} continue to 'punish' them.");
			}
			if (currentTraining[10]) {
				append(w, t, "  The signs and billboards advertising the event also update themselves to account for the new attraction, showing pictures of {mainName} in {hisHer} costume and luring a new wave of curious Thralls.");
			}
			w.append(t, "\n\n");
			if (consenting) {
				if (disgrace < 20) {
					if (dignity > 66) {
						append(w, t, "{mainName}'s heart pounds, {hisHer} face turning a bright shade of red, but {heShe} forces {himHer}self to smile back at all the people leering at {himHer}.  ");
					} else if (dignity > 33) {
						append(w, t, "{mainName} reflexively tries to cover {himHer}self, but barely manages to keep {hisHer} trembling hands where they are.  ");
					}
				} else {
					if (dignity > 66) {
						append(w, t, "{mainName} feels {hisHer} heart jump when {heShe} realizes how hard everyone is staring at {himHer}, and {heShe} can't keep a delirious smile off {hisHer} face as {heShe} basks in the attention.  ");
					} else if (dignity > 33) {
						append(w, t, "The cool air on the exposed parts of {mainName}'s skin is a constant reminder of how much {heShe}'s showing off, but it doesn't even bother {himHer} anymore.  ");
					} else {
						append(w, t, "{mainName} can't help but be a bit amused at how strongly the crowd is affected by {hisHer} new appearance.  ");
					}
				}
			} else {
				if (disgrace < 20) {
					if (dignity > 66) {
						append(w, t, "{mainName} freezes up, so embarrassed that {heShe} can't even move a muscle.  ");
					} else if (dignity > 33) {
						append(w, t, "{mainName} cries out and attempts to cover {himHer}self, but {hisHer} attempts are futile.  ");
					}
				} else {
					if (dignity > 66) {
						append(w, t, "Rather than trying to cover {hisHer} body, {mainName} just turns {hisHer} face away from the crowd.  At this point, the exposure bothers {himHer} less than the show of submission itself.  ");
					} else if (dignity > 33) {
						append(w, t, "{mainName}'s face goes red with humiliation, less at the fact that {heShe}'s exposing everything and more at the particular way {heShe}'s being forced to expose {himHer}self.  ");
					} else {
						append(w, t, "{mainName} doesn't stop struggling, more motivated than embarrassed by the display {heShe}'s being forced to put on.  ");
					}
				}
			}
			if (currentTraining[8]) {
				append(w, t, "The telepathic link ensures that the Thralls understand exactly how it's affecting {himHer}.  ");
			}
			if (servicing) {
				if (oral) {
					if (anal) {
						if (currentTraining[3]) {
							append(w, t, "The crowd surges forward, pulling the tail back out of {hisHer} ass in order to ");
							if (vaginal) {
								append(w, t, "pound {himHer} in all three holes at once.  ");
							} else if (gender == Gender.MALE) {
								append(w, t, "pound {himHer} from both sides at once.  ");
							} else {
								append(w, t, "pound {hisHer} hole as another fucks {hisHer} throat.  ");
							}
						} else {
							if (vaginal) {
								append(w, t, "With its lust stoked even further, the crowd rushes in again, pounding {himHer} relentlessly in all three holes.  ");
							} else if (gender == Gender.MALE) {
								append(w, t, "With its lust stoked even further, the crowd rushes in again, pounding {himHer} in both holes.  ");
							} else {
								append(w, t, "With its lust stoked even further, the crowd rushes in again, pounding {hisHer} mouth from the front and {hisHer} ass from behind, filling both holes with their cum.  ");
							}
						}
					} else {
						if (vaginal) {
							append(w, t, "Enticed by the holes on display, the crowd grabs {himHer}, one of its members ramming himself into {hisHer} pussy, then another thrusting down {hisHer} throat when {heShe} gasps at the intense insertion.  ");
						} else {
							append(w, t, "Enticed by {hisHer} lewd appearance, the crowd forms a line, each member in turn thrusting down {hisHer} throat and shooting a load of cum inside, then moving aside for whoever's next.  ");
						}
					}
				} else {
					if (anal) {
						if (currentTraining[3]) {
							append(w, t, "The crowd surges forward, pulling the tail back out of {hisHer} ass in order to ");
							if (vaginal) {
								append(w, t, "pound both {hisHer} lower holes at once.  ");
							} else {
								append(w, t, "pound {hisHer} ass.  ");
							}
						} else {
							if (vaginal) {
								append(w, t, "With its lust stoked even further, the crowd rushes in again, pounding both {hisHer} lower holes at once.  ");
							} else {
								append(w, t, "With its lust stoked even further, the crowd rushes in again, pounding {hisHer} relentlessly.  ");
							}
						}
					} else {
						if (vaginal) {
							append(w, t, "Enticed by the hole on display, the crowd pulls {himHer} down to the ground.  Two of its members force {hisHer} legs wide apart, while the others take turns fucking {himHer}.  ");
						} else {
							append(w, t, "Enticed by the lewd display, many of the members of the crowd break out into open masturbation, aiming their ejaculations directly at {mainName}'s body.  ");
						}
					}
				}
				if (vaginal) {
			...
			if (oral || anal || vaginal) {
				if (deviancy < 20) {
					if (innocence > 66) {
						append(w, t, "{HeShe} absolutely hates it.");
					} else {
						append(w, t, "{HeShe} feels sick, all the moreso when {heShe} recognizes how good it's starting to feel.");
					}
				} else {
					if (innocence > 66) {
						append(w, t, "{HeShe} finds the experience of being put on display as a public cum receptacle oddly relaxing.");
					} else if (innocence > 33) {
						append(w, t, "The kinkiness of the situation is too much for {himHer} to deny, and {heShe} ends up enjoying {himHer}self.");
					} else {
						append(w, t, "In between being groped and fucked, {heShe} has to admire how {hisHer} appearance alterations were calculated not only to degrade {himHer}, but also to drive everyone else into a frenzy.");
					}
				}
			} else {
				if (deviancy < 20) {
					if (innocence > 66) {
						append(w, t, "{HeShe}'s confused about how they can enjoy just looking at {himHer} so much.");
					} else {
						append(w, t, "{HeShe}'s just glad that they aren't going any further.");
					}
				} else {
					if (innocence > 66) {
						append(w, t, "{HeShe} feels {hisHer} own body getting strangely heated as {heShe} sees how everyone's looking at {himHer}.");
					} else if (innocence > 33) {
						append(w, t, "A part of {himHer} wishes that the crowd would go further.");
					} else {
						append(w, t, "{HeShe} can't deny that {heShe}'s getting more and more aroused, too.");
					}
				}
			}
			Project.changePortrait(gender, type, Project.displayedCivilians[0], true, w, Project.displayedNames, 0, Project.Emotion.LEWD, Project.Emotion.STRUGGLE);
		} else if (nextTraining == 5) {
			//can affect helpless, servicing, body parts, orgasming, inPublic
			String target = "penis";
			if (gender == Gender.FEMALE) {
				target = "clit";
			}
			if (consenting && currentTraining[6]) {
				append(w, t, "{mainName} agreed to this, but {heShe}'s still surprised with the abruptness of the pillory which appears and snaps shut around {hisHer} neck and wrists, briefly immobilizing {himHer} so that the Thralls have a chance to turn the tables.  ");
			} else if (currentTraining[6]) {
				append(w, t, "More and more bindings appear around {mainName}, chaining {himHer} to the ground and rendering {hisHer} struggles less and less threatening to the Thralls, who grow bolder as a result.  ");
			} else if (currentTraining[3]) {
				//sex machine merging
				append(w, t, "{mainName}'s bindings begin to expand and merge together, encasing {himHer} in a solid block of material that leaves {himHer} completely immobile, unable to move even slightly away from the stimulation against {hisHer} " + target + ".  Only {hisHer} head ");
				if (vaginal || anal) {
					w.append(t, "and butt are exposed to the air.  ");
				} else {
					append(w, t, "is exposed to the air.  {HisHer} lower body is covered as if by a chastity belt, complete with an internal vibrator.  ");
				}
			} else {
				//pilloried
				append(w, t, "A heavy pillory appears and snaps shut around {mainName}'s neck and wrists, rendering {himHer} completely helpless.  ");
			}
			if (servicing) {
				append(w, t, "Although the crowd was already enjoying {hisHer} body, the sight of {himHer} being bound so tightly turns many of them on even more.  ");
			} else if (currentTraining[0] || (currentTraining[6] && (consenting == false || orgasmsGiven >= 1000)) || currentTraining[10]) {
				append(w, t, "And now that {heShe}'s unable to stop them, the lustful crowd wastes no time in surging forward to make use of {hisHer} body.  ");
				if (timesHadSex > 0) {
					if (timesTortured > 0) {
						anal = true;
					}
				...
				servicing = true;
				if (vaginal || anal) {
					append(w, t, "{mainName} can't even move {hisHer} hips away from the first ");
					if (vaginal && anal) {
						append(w, t, "two people who step forward to line their cocks up with {hisHer} lower holes and thrust their way inside.  ");
					} else {
						append(w, t, "person who steps forward to line his cock up with {mainName}'s ");
						if (anal) {
							w.append(t, "anus");
						} else {
					...
					if (oral) {
						append(w, t, "{HeShe} tries to look back at what's happening, but another man grabs {himHer} by the hair and forces {himHer} to start sucking his cock.  ");
					}
				} else if (oral) {
					append(w, t, "{mainName}'s eyes go wide and {heShe} shakes {hisHer} head as one man approaches with his erect cock already out, but there's nothing {mainName} can do to stop him from grabbing {himHer} by the hair and ramming it into {hisHer} mouth.  ");
				} else {
					append(w, t, "{mainName}'s clenched teeth are enough to hold at bay the members of the crowd who might be interested in fucking {hisHer} mouth, but they instead content themselves with masturbating nearby and cumming all over {hisHer} face and hair.  ");
				}
			} else if (inPublic) {
				//describe reaction
				append(w, t, "The crowd cheers, eager to see {himHer} humbled even further.  ");
			} else {
				//dragged into public
				append(w, t, "You summon some of your Thralls to wheel {himHer} out into public so that everyone can see {hisHer} predicament.  ");
			}
			if (consenting) {
				if (obedience < 20) {
					if (confidence > 66) {
						append(w, t, "{mainName} finds the treatment extremely insulting, but {heShe} stubbornly endures it.");
					} else {
						append(w, t, "{mainName} tries not to let it get to {himHer}, but the truth is that {heShe} feels weaker and more pathetic than ever.");
					}
				} else {
					if (confidence > 66) {
						append(w, t, "Far from {hisHer} once defiant nature, {mainName} just hangs limp, not even attempting to escape.");
					} else if (confidence > 33) {
						append(w, t, "Having known that this was coming, {mainName} cooperates fully, entrusting {hisHer} body to your training.");
					} else {
						append(w, t, "As much as the crowd's cruelty stings, {mainName} finds it oddly comforting that there's nothing {heShe} could do to stop this even if {heShe} tried.");
					}
				}
			} else {
				if (obedience < 20) {
					if (confidence > 66) {
						append(w, t, "{mainName} struggles with all {hisHer} considerable might, moving the contraption left and right with every shift of {hisHer} weight, but {heShe} still can't get free.");
					} else {
						append(w, t, "{mainName} struggles as best {heShe} can, grunting with futile effort.");
					}
				} else {
					if (confidence > 66) {
						append(w, t, "{mainName} glares angrily up at those surrounding {himHer}, but {heShe} knows that there's no point in resisting anymore.");
					} else if (confidence > 33) {
						append(w, t, "{mainName} just closes {hisHer} eyes and hopes that it will be over soon.");
					} else {
						append(w, t, "Tears run down {mainName}'s cheeks.");
					}
				}
			}
			if (currentTraining[8]) {
				append(w, t, "  {HeShe}'s forced to telepathically transmit {hisHer} every reaction to the crowd, and they adjust their taunting accordingly, eager to break {himHer} down even more.");
			}
			w.append(t, "\n\n");
			if (servicing) {
				append(w, t, "Within a few minutes, {mainName} is a sticky mess, unable to even wipe the cum off {hisHer} face.  ");
			} else {
				append(w, t, "The longer {mainName} spends on display, the more bold the crowd gets.  They throw garbage at {himHer}, write degrading messages on {hisHer} exposed skin, and generally enjoy themselves at {hisHer} expense.  ");
			}
			if (disgrace < 20) {
				if (dignity > 66) {
					append(w, t, "{HeShe}'s mortified by the crowd's amused chatter about how {heShe} must not have ever been very strong after all, and the worst part is that {heShe} can't do anything at all to prove them wrong.");
				} else {
					append(w, t, "The way the crowd is looking at {himHer} is already more contemptful than it was when they first saw {himHer}.");
				}
			} else {
				if (dignity > 66) {
					append(w, t, "{HeShe}'s gotten used to the lack of respect by now, but the occasional laughing comment still cuts deep enough to make {hisHer} heart sink.");
				} else if (dignity > 33) {
					append(w, t, "{HisHer} only saving grace is that {heShe}'s lost {hisHer} novelty as a target of abuse, as everyone already knows that {heShe}'s unable to fight back.");
				} else {
					append(w, t, "{HeShe} tries to ignore the crowd's amused laughter, but a part of {himHer} knows that the less they think of {himHer}, the worse {hisHer} treatment will get.");
				}
			}
			Project.changePortrait(gender, type, Project.displayedCivilians[0], true, w, Project.displayedNames, 0, Project.Emotion.SHAME, Project.Emotion.FEAR);
			if (currentTraining[6] && consenting) {
				append(w, t, "  Finally, the pillory disappears, although loose chains remain around {hisHer} wrists - a reminder that you can seize control from {himHer} again at any moment.");
			}
			if (servicing) {
				if (vaginal) {
					int added = 3 + (int)(Math.random()*3);
			...
			if (consenting) {
				if (helpless) {
					append(w, t, "You order your Thralls to clear a perimeter around {mainName} ");
					if (tied) {
						append(w, t, "while you loosen {hisHer} bindings and allow {himHer} to stand up.  Then, you ");
					} else {
						append(w, t, "while {heShe} recovers from what's been done to {himHer}.  Then, you ");
					}
				} else {
					w.append(t, "You ");
				}
				append(w, t, "order some of your misbehaving Thralls to stand in front of {mainName} while the others form a crowd to watch.  ");
				if (orgasmsGiven < 1000) {
					if (innocence > 66) {
						append(w, t, "{mainName} doesn't really understand how {heShe}'s supposed to 'punish' them, so {heShe} just ends up shoving them around and shouting verbal abuse.  Finally, {heShe} starts having them take turns being bent over {hisHer} knee and spanked.  ");
					} else if (innocence > 33) {
						append(w, t, "{mainName} starts slapping them and shoving them around, then kicking them while they're down in order to force them back onto their feet.  ");
					} else {
						append(w, t, "{mainName} punishes them coldly and methodically, having them strip their shirts off and then turn their backs so that {heShe} can lash them with the whip you provide {himHer}.  When any of them try to suggest {heShe} use a lewder method, {heShe} silences them with extra whippings.  ");
					}
					append(w, t, "Despite the non-sexual nature of the punishment, some of the Thralls enjoy it on some level, and {mainName} can't help but notice their straining erections.");
					if (currentTraining[8]) {
						append(w, t, "  They grin and stand a little straighter when they telepathically sense {hisHer} discomfort and curiosity.");
					}
				} else {
					if (innocence > 66) {
						append(w, t, "{mainName} has all the Thralls strip naked, and {heShe} giggles as {heShe} compares the sizes of their erections.  The smaller ones get kicked between the legs, while the larger ones are forced to lay on their backs, where {mainName} steps on them and enjoys the way they throb underfoot.  ");
					} else if (innocence > 33) {
						append(w, t, "{mainName} picks out the ones {heShe} finds most attractive, then has them take turns bending over for {himHer} ");
						if (gender == Gender.FEMALE) {
							append(w, t, "so {heShe} can use {hisHer} strap-on to fuck them up the ass.  ");
						} else {
							append(w, t, "so {heShe} can position the tip of {hisHer} penis against their ass and force {hisHer} way inside.  ");
						}
					} else {
						append(w, t, "{mainName} attaches various sex toys to them, promising merciless pain to those who cum first, but pleasurable rewards to those who last longest.  ");
					}
					if (currentTraining[8]) {
						append(w, t, "The Thralls are eager to comply, as the telepathy lets them know exactly how {mainName} is planning to reward those who behave best.  ");
					}
					append(w, t, "Before long, {heShe}'s overcome by {hisHer} own lust, and {heShe} allows a few of the luckier Thralls to push {himHer} down and have their way with {himHer}.");
				}
			} else {
				if (helpless) {
					if (inPublic) {
					...
					append(w, t, ", eagerly pulling {hisHer} legs apart and shoving each other aside as they compete to see who gets to violate {himHer}.  However, {heShe}'s still able to fight back, ");
				} else {
					if (inPublic) {
						append(w, t, "The surrounding Thralls mob {mainName}");
					} else {
					...
					append(w, t, ", and while {heShe}'s able to hold them off at first, their sheer numbers are too much for {himHer} to handle.  Soon, {heShe}'s reduced to ");
				}
				append(w, t, "kicking and punching with all {hisHer} strength, but that isn't enough to stop them for long.");
				if (orgasmsGiven < 1000) {
					if (currentTraining[8]) {
						append(w, t, "  The Thralls can sense how disgusted {heShe} is by all this, but they just laugh and continue anyway.");
					} else if (innocence > 66) {
						append(w, t, "  {HisHer} disgust and horror cause {himHer} to mindlessly lash out as they push {himHer} down.");
					} else if (innocence > 33) {
						append(w, t, "  {HeShe} can tell that {heShe}'s causing bruises and worse injuries, but the Thralls aren't deterred.");
					} else {
						append(w, t, "  {HeShe} knows it's futile, but {heShe} keeps struggling anyway, even as they position themselves to take {himHer}.");
					}
				} else {
					if (currentTraining[8]) {
						append(w, t, "  {HeShe} can't stop {himHer}self from thinking about how good it might feel if {heShe} fails, and the Thralls sense {hisHer} weakness.");
					} else if (innocence > 66) {
						append(w, t, "  Soon, the pleasure of the hands all over {hisHer} body has caused {himHer} to forget {hisHer} resistance.");
					} else if (innocence > 33) {
						append(w, t, "  {HisHer} body begins to spasm for a different reason, as {hisHer} desire for pleasure overcomes {hisHer} will to resist.");
					} else {
						append(w, t, "  {HeShe} might have been able to escape if {heShe} truly put {hisHer} midn to it, but lust clouds {hisHer} judgment.");
					}
				}
			}
			w.append(t, "\n\n");
			if (orgasmsGiven < 1000 && consenting) {
				if (hostility < 30) {
					if (morality > 66) {
						append(w, t, "{mainName} feels deeply conflicted about hurting people like this, but {heShe} rationalizes it by telling {himHer}self that they're bad people and so they deserve to be hurt.  The more {heShe} does it, the less it bothers {himHer}.");
					} else if (morality > 33) {
						append(w, t, "{mainName} has never considered {himHer}self a sadistic person, but the feeling of power {heShe} gets when punishing others is addictive.  As the session goes on, there's less and less hesitation in {hisHer} actions.");
					} else {
						append(w, t, "Up until recently, {mainName}'s distaste for the rest of humanity had been more of an abstract feeling, one {heShe} had rarely acted upon.  But the more {heShe} hurts other people, the more {heShe} starts to enjoy it.  It awakens a sadistic hunger in {himHer}.");
					}
				} else {
					if (morality > 66) {
						append(w, t, "{mainName} takes great joy in meting out the pain to whoever seems to deserve it the most.  However, {heShe}'s anything but fair, as {heShe}'s come to believe that everyone deserves some degree of punishment.");
					} else if (morality > 33) {
						append(w, t, "The 'punishment' is only a mockery of justice, as it quickly becomes clear that {mainName} is just doing whatever {heShe} feels like.  All of this is only an excuse to satisfy {hisHer} newly-developed sadism.");
					} else {
						append(w, t, "{mainName} pushes things far further than {heShe} has to, getting more and more vicious until {hisHer} victims don't even have the energy to scream anymore.  The Thralls only get relief when {heShe} finally gets bored.");
					}
				}
				if (currentTraining[10]) {
					append(w, t, "  A part of {himHer} is reluctant to go back to servicing them sexually.");
				}
				Project.changePortrait(gender, type, Project.displayedCivilians[0], true, w, Project.displayedNames, 0, Project.Emotion.ANGER, Project.Emotion.NEUTRAL);
			} else {
				if (timesHadSex > 0) {
				...
				append(w, t, "The Thralls are especially rough with {mainName}, getting their payback for how {heShe} hurt them.  ");
				if (currentTraining[10]) {
					append(w, t, "Those who are waiting for their own turn to be serviced cheer them on, happy to see the Forsaken being put back in {hisHer} place.  ");
				}
				if (vaginal || anal) {
					append(w, t, "There's a loud smack every time the one thrusting from behind {himHer} rams his hips forward.  ");
				} else {
					append(w, t, "Even though {heShe} doesn't let them penetrate {himHer} down below, they take turns fucking {hisHer} throat with merciless force.  ");
				}
				if (hostility < 30) {
					if (morality > 66) {
						append(w, t, "{mainName} didn't truly want to hurt them, but after seeing how cruel they can be, a part of {himHer} wishes {heShe} had punished them more severely.");
					} else if (morality > 33) {
						append(w, t, "{mainName} feels confused and betrayed - as far as {heShe} was concerned, this shouldn't have been anything to get angry about.");
					} else {
						append(w, t, "{mainName} is annoyed, and {heShe} wants to go back to punishing them, but {heShe} doesn't get the chance.");
					}
				} else {
					if (morality > 66) {
						append(w, t, "{mainName} is satisfied to know that {heShe} was right to punish them so harshly.");
					} else if (morality > 33) {
						append(w, t, "{mainName} isn't surprised by this sort of savage behavior anymore.");
					} else {
						append(w, t, "{mainName} enjoys it - {heShe} knows that this will make it feel that much more sweet when {heShe} finally gets a chance to punish them again.");
					}
				}
				orgasmsGiven += 100 + (int)(Math.random()*100);
				if (timesOrgasmed > 0) {
					timesOrgasmed += 3 + (int)(Math.random()*3);
				}
				Project.changePortrait(gender, type, Project.displayedCivilians[0], true, w, Project.displayedNames, 0, Project.Emotion.STRUGGLE, Project.Emotion.LEWD);
			}
			peopleInjured += 100 + (int)(Math.random()*100);
			...
			if (inPublic) {
				if (currentTraining[6]) {
					append(w, t, "Up until now, the Thralls have been enduring {mainName}'s violent treatment, but now they surge forward as one to immobilize {hisHer} arms and legs with the weight of their bodies, taking away every semblance of control from {himHer}.  ");
				} else if (helpless) {
					append(w, t, "As they're given license to vent their most violent urges, the Thralls bear down on the helpless {mainName}.  ");
				} else if (consenting) {
					append(w, t, "{mainName} calls out to the surrounding crowd, asking them to do their worst to {himHer}.  They're quick to comply.  ");
				} else {
					append(w, t, "The Thralls bear down on {mainName}, subduing {himHer} ");
					if (w.tickle()) {
						append(w, t, "by grabbing {hisHer} limbs and savagely tickling {hisHer} thighs and armpits.  ");
					} else {
						w.append(t, "with brutal punches and kicks.  ");
					}
					append(w, t, "{HeShe}'s quickly overwhelmed.  ");
				}
			} else {
				if (consenting && helpless == false) {
					append(w, t, "{mainName} heads out into public, knowing what awaits {himHer}.  The Thralls have already been given their orders, and they descend on {himHer} in a violent mob.  ");
				} else {
					append(w, t, "The Thralls launch a manhunt for {mainName}");
					if (helpless) {
						append(w, t, ", and they soon stumble upon {hisHer} helpless form.  Some of the Thralls head out to notify the others of {hisHer} location, while the others get an early start on tormenting {himHer}.  ");
					} else {
						append(w, t, ", and it's not long before they find {himHer} trying to flee.  They bear down on {himHer}, subduing {himHer} ");
						if (w.tickle()) {
							append(w, t, "by grabbing {hisHer} limbs and savagely tickling {hisHer} thighs and armpits");
						} else {
							w.append(t, "with brutal punches and kicks");
						}
						append(w, t, ", and {heShe}'s quickly overwhelmed.  ");
					}
				}
			}
			if (obedience < 35) {
				if (confidence > 66) {
					append(w, t, "At first, {heShe}'s confident that {heShe} can take the punishment, but then {heShe} begins to falter, and finally {heShe}'s desperate to bring it to an end, no matter what {heShe} has to do.");
				} else if (confidence > 33) {
					append(w, t, "{mainName} attempts to endure it stoically at first, but it's not long before {heShe}'s crying pitifully out loud without even trying to stifle {himHer}self.");
				} else {
					append(w, t, "{mainName} tries to soften the punishment by meekly submitting, but the Thralls have been driven into a sadistic frenzy, and {heShe}'s reduced to begging and pleading, unable to understand why nothing {heShe} does seems to help at all.");
				}
			} else {
				if (confidence > 66) {
					append(w, t, "Once it becomes clear how this is going to go, {mainName} submits {himHer}self to the punishment with masochistic pride, spreading {hisHer} arms and legs so that {heShe}'s completely without protection.");
				} else if (confidence > 33) {
					append(w, t, "{mainName} has learned to stop resisting when this sort of thing happens.  {HisHer} attempts at stoicism have been replaced by pitiful cries which {heShe} doesn't even try to hold in.");
				} else {
					append(w, t, "{mainName} goes completely limp, surrendering {himHer}self entirely to the whims of the crowd.");
				}
			}
			w.append(t, "\n\n");
			if (servicing) {
				if (currentTraining[10]) {
					append(w, t, "The signs advertising {mainName}'s service to the public now flash and change to show that the Thralls using {himHer} are allowed and encouraged to do whatever they want, no matter how extreme.  After all, ");
					if (w.tickle()) {
						w.append(t, "it's only tickling.  ");
					} else {
						append(w, t, "the Demon Lord will ensure that {heShe} lives, no matter how much they beat and stomp on {himHer}.  ");
					}
				} else {
					append(w, t, "Now that they've been given permission to indulge in their sadistic desires, the Thralls' use of {mainName}'s body becomes especially forceful and cruel.  They stab their cocks at {himHer} like weapons, hammering {himHer} again and again.  ");
				}
			} else if (currentTraining[0] || (currentTraining[6] && (consenting == false || orgasmsGiven >= 1000)) || currentTraining[10]) {
				if (timesHadSex > 0) {
					if (gender == Gender.MALE) {
				...
				if (vaginal || anal) {
					if (w.tickle()) {
						append(w, t, "The tickling is so intense and overstimulating that {mainName} hardly notices when ");
					} else {
						append(w, t, "The flurry of fists and shoes from all directions is so overwhelming that {mainName} almost doesn't notice when ");
					}
					if (anal && vaginal) {
						append(w, t, "one Thrall pushes his way into {hisHer} pussy - but {heShe} cries out wordlessly when another forces open {hisHer} asshole.  ");
					} else if (anal) {
						append(w, t, "the fingers invading {hisHer} asshole are replaced by a Thrall's thick cock.  ");
					} else {
						append(w, t, "{hisHer} pussy is spread wide open to make way for a Thrall's cock.  ");
					}
				} else if (oral) {
					append(w, t, "By locking {hisHer} arms around {hisHer} knees, {mainName} is able to keep {hisHer} legs tightly shut despite the Thralls' attempts to pry them open.  However, {heShe}'s too afraid to bite down on the cock forced past {hisHer} lips and down {hisHer} throat, moaning helplessly around it as ");
					if (w.tickle()) {
						append(w, t, "the Thralls' fingers dig into {hisHer} armpits and ribs.");
					} else {
						append(w, t, "the Thralls pummel {himHer}.");
					}
				} else {
					append(w, t, "The only form of defiance {mainName} is able to muster is to keep {hisHer} legs closed and {hisHer} mouth tightly shut, denying the Thralls any opportunity to penetrate {himHer} - and provoking them into beating {himHer} even more savagely as a result.  ");
				}
				servicing = true;
			} else {
				append(w, t, "The melee around {mainName} is so chaotic, with fists and shoes striking {himHer} again and again, that even the Thralls who would prefer to rape {himHer} don't have an opportunity to do so.  None of them are bold enough to risk exposing their most sensitive parts in this situation.  ");
			}
			if (currentTraining[8]) {
				append(w, t, "The continuing telepathy lets the Thralls know exactly how much {mainName} is suffering, but they don't show any mercy at all.  ");
			}
			if (hostility < 40) {
				if (morality > 66) {
					append(w, t, "{mainName} is in complete disbelief that people could be so cruel.  It's only later, when {heShe} has time to reflect, that {heShe}'ll start to wonder whether {hisHer} faith in humanity was misplaced all along.");
				} else if (morality > 33) {
					append(w, t, "{mainName} doesn't understand why {heShe}'s being treated this way, but {heShe} quickly begins to hate the Thralls for it.");
				} else {
					append(w, t, "Even for {mainName}, who has long since decided that all people are evil at heart, the sheer enjoyment that the Thralls take from attacking {himHer} is shocking.");
				}
				Project.changePortrait(gender, type, Project.displayedCivilians[0], true, w, Project.displayedNames, 0, Project.Emotion.STRUGGLE, Project.Emotion.SHAME);
			} else {
				if (morality > 66) {
					append(w, t, "{mainName} feels foolish for ever believing that people were all basically good inside.");
				} else if (morality > 33) {
					append(w, t, "{mainName} can't muster the effort to feel any emotions at all about {hisHer} treatment - {heShe} has already accepted that everyone is just waiting for an excuse to do things like this.");
				} else {
					append(w, t, "{mainName} distracts {himHer}self from {hisHer} torment by imagining all the ways {heShe} might be able to get revenge someday.");
				}
				Project.changePortrait(gender, type, Project.displayedCivilians[0], true, w, Project.displayedNames, 0, Project.Emotion.ANGER, Project.Emotion.STRUGGLE);
			}
			if (servicing) {
				orgasmsGiven += 15 + (int)(Math.random()*15);
			}
			if (vaginal) {
				timesHadSex += 6 + (int)(Math.random()*6);
			}
			if (anal) {
				if (gender == Gender.MALE) {
					timesHadSex += 6 + (int)(Math.random()*6);
			...
			append(w, t, "{mainName}'s mind begins broadcasting its thoughts to everyone in a large radius, ");
			if (currentTraining[10]) {
				append(w, t, "unwittingly advertising the fact that you're giving anyone who comes by a free chance to use {hisHer} body.  Many more Thralls start showing up.  ");
			} else if (inPublic) {
				append(w, t, "and the crowd around {himHer} grows even more numerous as curious passers-by come to see what's going on.  ");
			} else {
				append(w, t, "and soon {heShe}'s surrounded by men who are enticed by {hisHer} expectation that things might get even more extreme from here on.  ");
			}
			if (disgrace < 40) {
				if (dignity > 66) {
					append(w, t, "There's a wave of disbelief and dawning comprehension among many of the bystanders as they realize that {heShe}'s not nearly so fearless as they had been led to believe.");
				} else if (dignity > 33) {
					append(w, t, "Some of the bystanders are diehard fans who were willing to overlook all the failures that led {himHer} to this point, but {mainName}'s own thoughts are a proof of weakness which even they can't deny.");
				} else {
					append(w, t, "{HeShe} isn't used to managing {hisHer} words in public, let alone {hisHer} thoughts, and everyone quickly picks up on {hisHer} most intimate secrets and weaknesses.");
				}
			} else {
				if (dignity > 66) {
					append(w, t, "A part of {himHer} still quails in embarrassment as the fears and insecurities {heShe} was once able to hide are all dragged out into the open.");
				} else if (dignity > 33) {
					append(w, t, "By this point, {heShe} doesn't really have any secrets left, but the bystanders still enjoy seeing proof once again that {heShe} was never particularly strong after all.");
				} else {
					append(w, t, "{HeShe} lets {hisHer} thoughts flow freely, knowing that it's pointless to try to hide {hisHer} weaknesses after they've already been exposed so thoroughly.");
				}
			}
			w.append(t, "\n\n");
			if (currentTraining[6] && consenting) {
				append(w, t, "{HeShe} knows that {hisHer} current position where {heShe}'s in control of the Thralls can't possibly last long.  {HeShe}'s afraid that it will end soon, hesitant to push {hisHer} dominance too far for fear that it will only make things worse when they get a chance for revenge.  The Thralls pick up on that and start insulting {himHer}, knowing that {heShe} won't retaliate.  ");
			} else if (stimulated) {
				append(w, t, "{HisHer} humiliation is only enhanced by the fact that the Thralls can tell exactly how much pleasure {heShe}'s feeling from this.  They laugh and taunt {himHer}, calling {himHer} a slut and worse.  ");
			} else if (pained) {
				append(w, t, "The Thralls can tell exactly how much discomfort {heShe}'s in.  Regardless of whether {heShe} begs out loud, they can hear {hisHer} inner voice wailing and praying for it to end.  Even so, they mock {himHer}, shouting that {heShe} deserves even worse.  ");
			} else {
				append(w, t, "{HeShe} knows that {heShe}'s at the Demon Lord's mercy, and {heShe} mentally flinches away from every possible sign that there's more punishment in store.  {HisHer} anticipation encourages the surrounding people to stick around in hopes of being able to participate in whatever you're planning.  ");
			}
			if (hostility < 40) {
				if (morality > 66) {
					append(w, t, "{mainName} is shocked by their lack of emapthy, even when they have the deepest possible understanding of {hisHer} true feelings.  On some level, {heShe} had always believed that all cruelty only came about as a result of misunderstandings.");
				} else if (morality > 33) {
					append(w, t, "A part of {mainName} had hoped that knowledge of {hisHer} own weakness would make others sympathetic, but the men laugh openly at {hisHer} naivete.  {HisHer} fear only turns them on even more.");
				} else {
					append(w, t, "{mainName} had thought that {heShe} understood how cruel people could be, but the complete callous disregard these people show for {hisHer} suffering is still surprising.  {mainName} berates {himHer}self for not being even more harsh toward others, and that prompts another wave of laughter from those who hear {hisHer} thoughts.");
				}
				Project.changePortrait(gender, type, Project.displayedCivilians[0], true, w, Project.displayedNames, 0, Project.Emotion.SHAME, Project.Emotion.FEAR);
			} else {
				if (morality > 66) {
					append(w, t, "{mainName} barely even has the energy to get angry anymore.  {HeShe} had known that they would act this way, and only a lingering sense of betrayal remains to be broadcast to the surrounding people.");
				} else if (morality > 33) {
					append(w, t, "{mainName}'s thoughts grow increasingly angry, much to their amusement.  They begin deliberately trying to provoke {himHer}, competing to see who can get the biggest mental reaction.");
				} else {
					append(w, t, "{mainName} tries to threaten the surrounding people, imagining them all experiencing painful gorey deaths.  But they just laugh even harder, stoking {hisHer} anger even more.");
				}
				Project.changePortrait(gender, type, Project.displayedCivilians[0], true, w, Project.displayedNames, 0, Project.Emotion.ANGER, Project.Emotion.SHAME);
			}
			debased = true;
			...
			if (consenting) {
				if (bodyPresent) {
					append(w, t, "You bring a tentacle close to {mainName}'s mouth, and {heShe} obediently opens up and begins to suckle it.  ");
				} else if (currentTraining[11]) {
					append(w, t, "One of your Demons brings its tentacle close to {mainName}'s mouth, and {heShe} obediently opens up and begins to suckle it.  ");
				} else if (inPublic) {
					append(w, t, "One of the surrounding Thralls steps forward with a bottle of mysterious liquid.  {mainName} willingly drinks it.  ");
				} else {
					append(w, t, "You send a Thrall to meet with {mainName}, carrying with him a bottle of aphrodisiac extracted from your Demons.  The Thrall stays long enough to make sure that {mainName} drinks it, then reluctantly leaves.  ");
				}
			} else {
				if (bodyPresent) {
					append(w, t, "You force a tentacle into {mainName}'s mouth and begin pouring fluids directly into {hisHer} stomach.  ");
				} else if (currentTraining[11]) {
					append(w, t, "One of your Demons' tentacles forces its way into {mainName}'s mouth and begins pouring fluids directly into {hisHer} stomach.  ");
				} else if (inPublic) {
					append(w, t, "Your Thralls surge toward {mainName}, grabbing and immobilizing {himHer} long enough for one of the Thralls to bring a bottle of mysterious liquid to {hisHer} lips. The Thrall pinches {hisHer} nose to force {himHer} to open {hisHer} mouth, while the other Thralls hold {himHer} from behind, keeping {himHer} from even trying to escape.  Once the substance is in {hisHer} mouth, they stop {himHer} from spitting it out until they're certain that enough has been absorbed into {hisHer} mucous membranes.  ");
				} else if (helpless) {
					append(w, t, "You send one of your Thralls to where {mainName} currently lies helpless.  The Thrall brings a bottle filled with mysterious liquid to {mainName}'s lips, pinches {hisHer} nose to force {himHer} to open {hisHer} mouth, then stops {himHer} from spitting it out until enough of the substance has entered {hisHer} mucous membranes.  ");
				} else if (currentTypes > 0) {
					append(w, t, "Unbeknownst to {mainName}, you've been using {hisHer} distraction to flood the area with aphrodisiac gas.  When {heShe} tries to leave, {hisHer} head begins to feel fuzzy.  ");
				} else {
					append(w, t, "One of your Thralls slips some aphrodisiac extracted from your Demons into {mainName}'s food.  Within a few minutes of eating it, {heShe} starts to feel the effects and realizes what must have happened.  ");
				}
			}
			if (obedience < 30) {
				if (confidence > 66) {
					append(w, t, "{HeShe} resists the drug with all {hisHer} willpower, determined to remain {himHer}self, but it's no use.  Soon, {heShe} can think of nothing but {hisHer} own sexual desires.");
				} else if (confidence > 33) {
					append(w, t, "{HeShe} tries to resist the drug's effects, mentally fighting back out of pure unwillingness to become your toy.  That resolve isn't nearly strong enough, and {heShe} quickly starts panting like an animal in heat.");
				} else {
					append(w, t, "{HeShe}'s terrified of losing {himHer}self, stricken by panic as {heShe} feels {hisHer} body begin to heat up.  But {heShe} isn't strong-willed enough to resist for long, and it's not long before {hisHer} panic and all {hisHer} other doubts feel like someone else's thoughts, blurred by a haze of desire.");
				}
			} else {
				if (confidence > 66) {
					append(w, t, "{HeShe} still instinctively resists when the drug tries to take control of {hisHer} body, but even so, it makes no difference.  {HisHer} thoughts turn in increasingly lewd directions, and {heShe} only wants to think about what {heShe} can do to feel good.");
				} else if (confidence > 33) {
					append(w, t, "{HeShe} knows by now that there's no point in trying to retain {hisHer} sense of reason.  {HeShe} allows the drug's effects to dominate {hisHer} mind.");
				} else {
					append(w, t, "In {hisHer} heart, {heShe} welcomes the excuse to stop thinking and just feel pleasure.  {HisHer} mind quickly surrenders.");
				}
			}
			w.append(t, "\n\n");
			if (servicing) {
				if (currentTraining[10]) {
					append(w, t, "Time seems to fly by, and {heShe} loses track of how many cocks {heShe} services in {hisHer} drugged bliss.  ");
				} else if (consenting) {
					append(w, t, "{mainName} wears an ecstatic expression as {heShe} eagerly services every cock within reach, forgetting all lingering doubts and fatigue.  ");
				} else {
					append(w, t, "{mainName} knows {heShe} should be trying to escape, but the intense pleasure felt by {hisHer} body affects {hisHer} mental state as well.  ");
				}
			} else if (currentTraining[0] || (currentTraining[6] && (consenting == false || orgasmsGiven >= 1000)) || currentTraining[10]) {
				servicing = true;
				if (timesHadSex > 0) {
				...
				if (vaginal || anal) {
					append(w, t, "Before {heShe} even realizes it, {mainName} is eagerly arching {hisHer} back, presenting {hisHer} bottom to the Thralls.  The closest one wastes no time before thrusting ");
					if (vaginal) {
						w.append(t, "inside.  ");
					} else {
						append(w, t, "into {hisHer} asshole.  ");
					}
				} else if (oral) {
					append(w, t, "Although {heShe} still tries to cover {hisHer} lower body by reflex, {heShe} finds {himHer} self opening {hisHer} mouth wide, panting with desire that only intensifies as one of the Thralls brings his cock up close and finally rests it on {hisHer} tongue.  Before {heShe} realizes it, {heShe}'s eagerly sucking.  ");
				} else {
					append(w, t, "Through tremendous force of will, {mainName} refuses to allow {himHer}self to be penetrated, but {heShe} can't fend off all the cocks being thrust at {himHer}.  {HeShe}'s become so sensitive that every shaft rubbing up against {hisHer} face, {hisHer} armpits, and especially {hisHer} nipples causes {himHer} to spasm with pleasure.  ");
				}
			} else if (tied) {
				append(w, t, "{HeShe} yearns to touch {himHer}self, but bound as {heShe} is, {heShe} can't quite manage it.  {HeShe}'s reduced to moaning pitifully, struggling in {hisHer} bonds as {heShe} tries to rub {hisHer} crotch against something.  ");
			} else if (inPublic) {
				append(w, t, "{HeShe} starts to uncontrollably play with {himHer}self, rubbing frantically between {hisHer} legs, unable to spare a thought to the spectators laughing and jeering as they watch {himHer}.  ");
			} else {
				append(w, t, "{HeShe} starts to frantically play with {himHer}self, pinching one nipple as {heShe} rubs {hisHer} " + organ + ", biting {hisHer} lip in order to avoid making any sounds that might alert anyone nearby as to what exactly {heShe}'s doing.  ");
			}
			if (currentTraining[8]) {
				if (servicing) {
					append(w, t, "The effects of the aphrodisiac aren't limited to {mainName}, as {hisHer} telepathic broadcast causes those using {hisHer} body to also feel unusually lustful, and their movements are especially passionate.  ");
				} else {
					append(w, t, "The show {heShe} puts on is especially compelling because {hisHer} telepathic link with the bystanders lets them know just what {heShe}'s feeling.  ");
				}
			}
			if (timesOrgasmed == 0) {
				append(w, t, "Several intense shivers run through {hisHer} body, causing {hisHer} eyes to roll into the back of {hisHer} head.  But {hisHer} body is still inexperienced with orgasm, and the peak isn't as high as it should be - {heShe}'s left whining and unsatisfied.");
			} else if (deviancy < 35) {
				if (innocence > 66) {
					append(w, t, "The orgasm is so intense that it scares {himHer}, leaving {himHer} sobbing even as {heShe} craves another one.");
				} else if (innocence > 33) {
					append(w, t, "When {heShe} cums, there's a brief moment when all {hisHer} doubts and past suffering are forgotten.  {HeShe}'s left blinking and shuddering, wishing {heShe} could go back to it.");
				} else {
					append(w, t, "For a moment, {heShe}'s afraid that {heShe}'ll lose {hisHer} mind, and the first climax is almost intense enough to do just that - but the clarity that comes afterward is just enough to let {himHer} cling to sanity.");
				}
				timesOrgasmed += 2 + (int)(Math.random()*2);
			} else {
				if (innocence > 66) {
					append(w, t, "{HeShe} eagerly gives in to the urge to cum, moaning wordlessly as {hisHer} mind goes completely blank.  A delirious smile spreads across {hisHer} face.");
				} else if (innocence > 33) {
					append(w, t, "It feels so good to cum that {heShe} almost passes out.  But the desire to cum again motivates {himHer} to stay awake, gyrating {hisHer} body as {heShe} works toward the next orgasm.");
				} else {
					append(w, t, "When {heShe} cums, {heShe} feels truly satisfied - but only for a moment.  Before the doubts have a chance to return to {himHer}, {heShe} desperately starts pursuing orgasm once again, afraid of the brief clarity that sometimes comes after the pleasure.");
				}
				timesOrgasmed += 3 + (int)(Math.random()*3);
			}
			Project.changePortrait(gender, type, Project.displayedCivilians[0], true, w, Project.displayedNames, 0, Project.Emotion.LEWD, Project.Emotion.STRUGGLE);
			...
			if (consenting) {
				if (currentTraining[6]) {
					append(w, t, "You let all your Thralls know that those who report to {mainName} will be dominated and forced to cum until they don't even want to anymore.  ");
				} else if (currentTraining[8]) {
					append(w, t, "You tell {mainName} the details of how {heShe}'s going to be servicing the Thralls, and with how {heShe}'s being made to telepathically broadcast {hisHer} thoughts, it's as good as putting up billboards advertising {himHer} to every Thralll in the city.  ");
				} else {
					append(w, t, "You pass word to your Thralls that {mainName} will be sexually servicing everyone who comes to {himHer}, and soon there's a line stretching all around the block.  ");
				}
			} else {
				if (currentTraining[8]) {
					append(w, t, "You tell {mainName} the details of how you're going to give {hisHer} body to every Thrall who happens to wander by, and with how {hisHer} thoughts are being unwillingly broadcast to your minions, it's as good as putting up billboards advertising {himHer} to every Thralll in the city.  ");
				} else if (currentTraining[6]) {
					append(w, t, "You spread word among your Thralls that they're all to help punish the resisting {mainName}.  Even though {heShe} continues to kick and struggle, they're all eager to comply.  ");
				} else {
					append(w, t, "Your Thralls put up advertisements all across your domain that {mainName} is helpless, {hisHer} body is free for them to use, and soon there's a line stretching all around the block.  ");
				}
			}
			if (deviancy < 40) {
				if (innocence > 66) {
					append(w, t, "{mainName} is disbelieving, unable to comprehend just how many cocks {heShe}'s about to suck.  It's only when {heShe} sees the size of the crowd that {heShe} begins to pale.");
				} else if (innocence > 33) {
					append(w, t, "{mainName} looks sick when {heShe} sees the huge crowd, many of them exposing themselves and already masturbating in {hisHer} direction.  {HeShe} averts {hisHer} eyes, trying to think about anything other than what's about to happen.");
				} else {
					append(w, t, "When {mainName} sees the size of the crowd, {heShe} begins to run the numbers in {hisHer} head, estimating how many people are there and how long {heShe}'ll have to spend satisfying each one.  When {heShe} reaches {hisHer} answer, {heShe} shudders and closes {hisHer} eyes.");
				}
			} else {
				if (innocence > 66) {
					append(w, t, "{mainName}'s eyes light up when {heShe} sees how many cocks have come to {himHer} to be sucked.  {HeShe} practically lunges for them, mouth wide open, having forgotten about everything else.");
				} else if (innocence > 33) {
					append(w, t, "{mainName} feels a bit proud of {himHer}self for the size of the crowd that showed up.  {HisHer} breathing quickens, and {heShe} licks {hisHer} lips, craving the taste of their cum.");
				} else {
					append(w, t, "{mainName} decides to take the opportunity to practice {hisHer} technique and see how quickly {heShe} can satisfy them all.  However, a part of {himHer} regrets that this will mean {heShe} won't be able to take {hisHer} time and truly enjoy it.");
				}
			}
			w.append(t, "\n\n");
			if (disgrace < 30) {
				if (dignity > 66) {
					append(w, t, "At first, many of the Thralls are afraid to approach such a notoriously powerful Chosen, but they soon see that there's nothing to fear now that {heShe}'s become one of the Forsaken.  ");
				} else if (dignity > 33) {
					append(w, t, "The Thralls arrive in droves, not only due to their lust, but also due to their curiosity as to how low the powerful {mainName} has been brought.  They aren't disappointed.  ");
				} else {
					append(w, t, "Many of the Thralls show up expecting some sort of prank, but when they see that {mainName} is in fact being used as a public cum receptacle, they're happy to join in.  ");
				}
			} else {
				if (dignity > 66) {
					append(w, t, "The Thralls are especially eager to degrade someone who had been as powerful and respected as {mainName}.  Even those who aren't feeling particularly lustful still want to participate out of pure spite.  ");
				} else if (dignity > 33) {
					append(w, t, "By this point, chances to humiliate {mainName} aren't even viewed as particularly special.  The Thralls each approach in turn, expecting their cocks to be sucked and knowing that {mainName} won't be able to resist.  ");
				} else {
					append(w, t, "Even though {heShe} was once considered to be something of a celebrity, the novelty of violating {mainName} has long since worn off.  The Thralls now just view {himHer} as another interchangeable piece of meat.  ");
				}
			}
			if (timesHadSex > 0) {
				if (gender == Gender.MALE) {
			...
			if (vaginal && anal) {
				append(w, t, "All three of {hisHer} holes end up being used, and {heShe} can hardly be seen in between the men vigorously thrusting into {himHer}.");
			} else if (vaginal) {
				append(w, t, "Eventually, two orderly lines form on either side of {mainName}, one full of Thralls who prefer to use {hisHer} mouth, and the other with Thralls who prefer to use {hisHer} experienced pussy.");
			} else if (anal) {
				if (gender == Gender.MALE) {
					append(w, t, "Eventually, two orderly lines form on either side of {mainName}, one full of Thralls who prefer to use {hisHer} mouth, and the other with Thralls who prefer to use {hisHer} thoroughly-loosened asshole.");
				} else {
					if (currentTraining[3]) {
						append(w, t, "{HisHer} pussy is blocked by the toys being used to stimulate {hisHer} " + organ);
					} else {
						append(w, t, "A sticker covering {hisHer} pussy designates that hole as reserved for later use by order of the Demon Lord");
					}
					append(w, t, ", but {heShe} still has two other holes to use.  By the time the crowd begins to thin, both {hisHer} mouth and {hisHer} anus are overflowing with cum.");
				}
			} else {
				if (currentTraining[3]) {
					append(w, t, "{HisHer} crotch is blocked by the toys being used to stimulate {himHer}");
				} else if (gender == Gender.MALE) {
					append(w, t, "A sticker covering {hisHer} asshole designates that hole as reserved for later use by order of the Demon Lord");
				} else {
					append(w, t, "Stickers covering {hisHer} pussy and asshole designate those holes as reserved for later use by order of the Demon Lord");
				}
				append(w, t, ", which means that the Thralls must content themselves by using {hisHer} mouth, hands, hair, feet, armpits, and whatever other parts of {hisHer} body they feel like rubbing themselves against.  Soon, {heShe}'s covered in cum and coughing up the copious amounts {heShe} swallowed as well.");
			}
			if ((vaginal || anal) && timesOrgasmed > 0) {
				timesOrgasmed += 10 + (int)(Math.random()*10);
			}
			lustful = true;
			orgasmsGiven += 1000 + (int)(Math.random()*200);
			Project.changePortrait(gender, type, Project.displayedCivilians[0], true, w, Project.displayedNames, 0, Project.Emotion.LEWD, Project.Emotion.STRUGGLE);
		} else if (nextTraining == 11) {
			if (bodyPresent) {
				append(w, t, "More tentacled Demons join your Commander body, surrounding {mainName} on all sides.  You direct one of them to take charge of {himHer} for the time being.  ");
			} else if (consenting) {
				if (inPublic) {
					append(w, t, "A huge, tentacled Demon parts the crowd around {mainName}.  At first, many of the spectators are frightened, but {mainName} reassures them that this will just be a little combat show for their benefit.  ");
				} else if (tied) {
					append(w, t, "You loosen {mainName}'s bonds enough that {heShe} can hobble over to the makeshift arena where a huge Demon and a crowd of spectators are already waiting.  It's obvious that this won't be a fair fight, but that just makes the show more enticing.  ");
				} else if (helpless) {
					append(w, t, "{mainName} staggers over to the makeshift arena you prepared beforehand, obviously in no state to actually fight.  But the crowd of spectators waiting there is still excited, and they raise a cheer when the Demon {heShe}'ll be fighting makes its entrance.  ");
				} else {
					append(w, t, "You gather your Thralls around a makeshift arena and send one of your Demons down to its center.  A great cheer rises up as {mainName} makes {hisHer} appearance and steps forward, ready to fight for the crowd's amusement.  ");
				}
			} else {
				if (inPublic) {
					append(w, t, "The crowd around {mainName} parts, and a huge Demon covered in tentacles steps toward {himHer}.  {HeShe} tries to steady {himHer}self, but the Demon is upon {himHer} before {heShe} can do anything.  ");
				} else if (helpless) {
					append(w, t, "You send a huge Demon to {mainName}'s location, and it wraps a tentacle around {himHer} and drags {himHer} in front of a waiting crowd.  In {hisHer} current state, {heShe}'ll obviously be unable to fight properly, but that just makes the show more enticing.  ");
				} else {
					append(w, t, "You send a large crowd of Thralls accompanied by a tentacled Demon to ambush {mainName}, just like how you'd attack {himHer} back when {heShe} was one of the Chosen.  ");
				}
			}
			if (disgrace < 35) {
				if (currentTraining[8]) {
					append(w, t, "{HeShe}'s overcome even worse odds as one of the Chosen, which means that the spectators are shocked when they hear {hisHer} despairing thoughts.  ");
				} else if (dignity > 66) {
					append(w, t, "It isn't unheard of for one of the Chosen to triumph even against such odds, and with {mainName}'s former fame, many of the Thralls are willing to believe that {heShe} might actually win.");
				} else if (dignity > 33) {
					append(w, t, "There's some hesitation and uncertainty among the Thralls, as some aren't entirely convinced that {heShe}'ll lose.");
				} else {
					append(w, t, "{mainName} isn't known for being especially strong, but {heShe} isn't known as a weakling either, and many of the Thralls who haven't seen {himHer} fight before are curious as to how this will turn out.");
				}
			} else {
				if (dignity > 66) {
					append(w, t, "Even now, after {mainName}'s reputation has been driven into the ground, there are still plenty of Thralls who never tire of seeing the former celebrity get humiliated.");
				} else if (dignity > 33) {
					append(w, t, "The crowd is smaller now than it would have been in the past, because for many of the Thralls, seeing {mainName} humiliated has lost its novelty.");
				} else {
					append(w, t, "The atmosphere is lighthearted and jovial, as the Thralls know that {heShe} won't be able to put up much of a fight.");
				}
				if (currentTraining[8]) {
					append(w, t, "  {HisHer} broadcast thoughts, which quickly turn toward thinking about how {heShe} has no chance, only confirm their expectations.");
				}
			}
			w.append(t, "\n\n");
			if (obedience < 40) {
				if (confidence > 66) {
					append(w, t, "The outcome is clear from the beginning, as the Demon overpowers {mainName} and starts ");
					if (w.tickle()) {
						append(w, t, "tickling {himHer} as {heShe}'s pinned to the ground");
					} else {
						append(w, t, "repeatedly bashing {hisHer} face into the ground");
					}
					append(w, t, ", but it still takes several minutes of punishment before {heShe} finally murmurs out a delirious surrender.  ");
				} else if (confidence > 33) {
					append(w, t, "{mainName} fights hard, but the Demon's power is overwhelming, and it's not too long before {heShe}'s gasping for mercy.  ");
				} else {
					append(w, t, "{mainName} goes down quickly, sinking to the ground and putting all {hisHer} energy into covering {hisHer} vitals, but a last bit of stubbornness causes {himHer} to hold back {hisHer} cry of surrender until a few moments later.  ");
				}
			} else {
				if (confidence > 66) {
					append(w, t, "For the sake of {hisHer} own pride, {mainName} tries to make a show of it, but now {heShe} knows better than to actually aim for victory.  {HeShe} collapses to the ground, and the Demon picks {himHer}up.  ");
				} else if (confidence > 33) {
					append(w, t, "Almost before it's had a chance to start, the fight is over.  {mainName} surrenders to the Demon, but of course the crowd won't be satisfied with just that.  ");
				} else {
					append(w, t, "The fight is humiliatingly brief, and then {mainName} is squealing helplessly as {heShe}'s subdued.  ");
				}
			}
			if (currentTraining[0] || (currentTraining[6] && (consenting == false || orgasmsGiven >= 1000)) || currentTraining[10]) {
				if (timesHadSex > 0) {
				...
				if (vaginal || anal) {
					append(w, t, "{HisHer} cries take on a sharper, more desperate tone ");
					if (vaginal && anal) {
						append(w, t, "when the tentacles ram themselves into {hisHer} pussy and anus both at once");
					} else if (vaginal) {
						append(w, t, "when one of the tentacles starts roughly penetrating {himHer} down below");
					} else {
						append(w, t, "when one of the tentacles forces its way past {hisHer} anus and deep into {hisHer} belly");
					}
					append(w, t, ", and as soon as the Demon is done with {himHer}, the Thralls rush forward so that their cocks can take its place.");
				} else {
					append(w, t, "As soon as everyone has heard {himHer} give up, the Demon immediately plugs {hisHer} mouth with a tentacle that starts to violently rape {hisHer} throat.  ");
					if (oral) {
						append(w, t, "When it pulls out, {mainName} tries to gasp for breath, only to choke on the cock of the quickest Thrall to run forward and make use of the newly-freed hole.  Plenty more are lined up behind him.");
					} else {
						append(w, t, "After it's done with {himHer}, the Demon pulls out in order to offer {hisHer} mouth to the lustful crowd.  However, after seeing the way that {mainName} kept biting down on the Demon's appendage, they unanimously decide to keep their distance, masturbating and ejaculating at {hisHer} feet and onto {hisHer} body.");
					}
				}
				if (currentTraining[10]) {
				...
				append(w, t, "{mainName} is dangled by {hisHer} wrists above the crowd, which pelts {himHer} with rocks and bits of garbage.");
				Project.changePortrait(gender, type, Project.displayedCivilians[0], true, w, Project.displayedNames, 0, Project.Emotion.SHAME, Project.Emotion.STRUGGLE);
			}
			meek = true;
		} else if (nextTraining == 12) {
			if (currentTraining[16]) {
			...
			} else if (currentTraining[14] || currentTraining[15] || currentTraining[17]) {
				...
				append(w, t, "You deploy one of your Commander bodies to {mainName}'s location, a quadrupedal body with a huge penis slung between its back legs.  ");
			}
			if (consenting) {
				if (tied) {
					append(w, t, "{mainName} does {hisHer} best to raise {hisHer} hips and present {himHer}self to you, though {hisHer} bindings make it awkward.  ");
				} else if (inPublic) {
					append(w, t, "The crowd raises a cheer as {mainName} gets on {hisHer} hands and knees, raising {hisHer} bare bottom and presenting it to you.  ");
				} else {
					append(w, t, "{mainName} gets on {hisHer} hands and knees, raising {hisHer} bare bottom and presenting it to you.  ");
				}
			} else {
				if (inPublic) {
					append(w, t, "The crowd holds {mainName} still for you, bending {himHer} over and pulling {hisHer} legs apart.  ");
				} else {
					append(w, t, "{mainName} struggles and squirms, trying to crawl away, but you easily pin {himHer} down and line your member up between {hisHer} legs.  ");
				}
			}
			...
			if (gender == Gender.MALE) {
				append(w, t, "up {hisHer} ass");
				if (timesHadSex == 0) {
					append(w, t, ", forcing a pitiful squeal from {hisHer} throat as {heShe} learns how it feels to be fucked like a woman.");
				} else {
					append(w, t, ", filling {himHer} completely and putting immense pressure on {hisHer} prostate.");
				}
			} else {
				append(w, t, "inside {himHer}");
				if (timesHadSex == 0) {
					append(w, t, ", spillling a trickle of blood as you take {hisHer} virginity.");
				} else {
					append(w, t, ", creating a slight bulge in {hisHer} lower tummy as you press against {hisHer} cervix.");
				}
			}
			w.append(t, "\n\n");
			if (hostility < 50) {
				if (morality > 66) {
					append(w, t, "A part of {mainName} seems to be horrified that {heShe}'s having sex with a Demon Lord, the embodiment of human evil.  ");
				} else if (morality > 33) {
					append(w, t, "{mainName} tries not to think about the fact that the cock inside {himHer} belongs to the enemy {heShe} had sworn to try to defeat.  ");
				} else {
					append(w, t, "At first, {mainName} doesn't feel any pleasure at all, and {heShe}'s reminded of the tortures {heShe} first endured when {heShe} fought you.  ");
				}
				w.append(t, "However, ");
			} else {
				if (morality > 66) {
					append(w, t, "{mainName} no longer feels any guilt whatsoever about doing this sort of thing with the supposed enemy of humanity.  ");
				} else if (morality > 33) {
					append(w, t, "Even if the cock inside {himHer} is much longer, {mainName} doesn't feel any differently about having sex with the Demon Lord than {heShe} does about doing similar activities with the Thralls.  ");
				} else {
					append(w, t, "{mainName} can't help but relish the fact that {heShe}'s become the Demon Lord's concubine, placing {himHer} in direct opposition to the human public {heShe}'s come to hate so much.  ");
				}
				w.append(t, "And ");
			}
			if (deviancy < 50) {
				append(w, t, "your shaft begins spurting potent psychoactive slime into {himHer} even as it continues thrusting in and out.  ");
				if (timesOrgasmed == 0) {
					append(w, t, "{HeShe} moans and shudders, {hisHer} whole body seizing up in an almost-orgasm, as strong a pleasure as any {hisHer} inexperienced body has ever felt.");
				} else if (innocence > 66) {
					append(w, t, "{HeShe} whines and tosses {hisHer} head back and forth, {hisHer} body still instinctively resisting the pleasure, but {heShe} still cums in the end, squeezing {hisHer} eyes shut and thrashing wildly before going limp.");
				} else if (innocence > 33) {
					append(w, t, "The pleasure mounts higher and higher, overwhelming {himHer}, and finally {heShe} cums, moaning as {hisHer} whole body shudders.");
				} else {
					append(w, t, "{HeShe} knows exactly what will happen next, and fear briefly fills {hisHer} eyes before they glaze over with pleasure.  {HeShe} experiences a particularly intense orgasm that leaves {hisHer} mind pleasantly blank.");
				}
				if (timesOrgasmed > 0) {
					timesOrgasmed++;
				}
			} else {
				append(w, t, "{hisHer} well-trained body responds even before your shaft begins spurting your psychoactive slime inside {himHer}.  ");
				if (timesOrgasmed > 0) {
					append(w, t, "{HisHer} eyes roll upward and {hisHer} mouth gapes open, and {heShe} mindlessly bucks {hisHer} hips against you, enjoying the intense pleasure radiating from {hisHer} deepest places.");
				} else if (innocence > 66) {
					append(w, t, "{HisHer} eyes roll upward and {hisHer} mouth gapes open, and {heShe} mindlessly bucks {hisHer} hips against you, cumming constantly with every impact of your tip deep inside {himHer}.");
				} else if (innocence > 33) {
					append(w, t, "{HeShe} cums instantly, but your body isn't limited by a human's stamina, and you keep penetrating {himHer} until {heShe} cums again, {hisHer} insides spasming wildly around you as you push {himHer} further and further.");
				} else {
					append(w, t, "{HeShe} surrenders to the pleasure, happily letting {hisHer} mind go blank as {heShe} cums, the waves of pleasure washing away {hisHer} rationality until {heShe}'s nothing more than a grunting animal being bred by your cock.");
				}
				if (timesOrgasmed > 0) {
					timesOrgasmed += 2 + (int)(Math.random()*3);
				}
			}
			append(w, t, "  When you finally pull out, {hisHer} hole is gushing a steady stream of white slime.");
			timesHadSex++;
			Project.changePortrait(gender, type, Project.displayedCivilians[0], true, w, Project.displayedNames, 0, Project.Emotion.LEWD, Project.Emotion.STRUGGLE);
		} else if (nextTraining == 13) {
			if (inPublic) {
				append(w, t, "Black energy crackles through the crowd surrounding {mainName}, and your Thralls' bodies begin to subtly shift and grow.  ");
			} else if (helpless) {
				append(w, t, "Several of your Thralls arrive, looming over {mainName}, their bodies crackling with a black energy that begins to transform them.  ");
			} else if (consenting) {
				append(w, t, "Obeying your instructions, {mainName} goes out to meet a group of Thralls.  As soon as {heShe} arrives, their bodies begin to crackle with black energy and transform.  ");
			} else {
				append(w, t, "A crowd of Thralls chases down {mainName}, and while {heShe}'s able to outrun them at first, their bodies begin to crackle with black energy that transforms them and grants them the power to catch and subdue {himHer}.  ");
			}
			w.append(t, "They grow taller, their muscles bulge, and their eyes glow with malevolent light.  ");
			if (obedience < 50) {
				if (consenting) {
					if (confidence > 66) {
						append(w, t, "{HeShe} dares them to do their worst, and they're happy to oblige.");
					} else if (confidence > 33) {
						append(w, t, "{HeShe} warns them not to go too far, but they just chuckle and close in around {himHer}.");
					} else {
						append(w, t, "{HisHer} eyes go wide with fear, but {heShe} decides that {heShe} has no choice but to face them head-on.");
					}
				} else {
					if (confidence > 66) {
						append(w, t, "{HeShe} tries with all {hisHer} might to fight back, but with your power infusing them, any one of the Thralls is more than a match for {himHer}.  They handle {himHer} like a child, lifting {himHer} up off the ground while {heShe} kicks wildly at the air.");
					} else if (confidence > 33) {
						append(w, t, "{HisHer} efforts to fight back are met with derisive laughter, the Thralls not even seeming to feel {hisHer} attempts to kick and punch at them, and {heShe} soon decides that it's better to conserve {hisHer} strength.");
					} else {
						append(w, t, "{HeShe} tries to squirm away, eyes wide with fear, but they pin {himHer} down and close in from all sides, leaving no opportunity to escape.");
					}
				}
			} else {
				if (confidence > 66) {
					append(w, t, "Once, {heShe} would have fought back or at least sneered at them for ganging up on {himHer}, but now {heShe} recognizes them as instruments of your will and meekly submits.");
				} else if (confidence > 33) {
					append(w, t, "{HeShe} doesn't fight back or try to flee, humbly bowing {hisHer} head as {heShe} awaits their abuse.");
				} else {
					append(w, t, "{HeShe} gives them a tremulous smile, telling them that {heShe}'s ready to accept {hisHer} punishment.");
				}
			}
			w.append(t, "\n\n");
			if (currentTraining[0] || (currentTraining[6] && (consenting == false || orgasmsGiven >= 1000)) || currentTraining[10]) {
				...
				append(w, t, "Now that {mainName} can't fight back, the Thralls are merciless about satisfying their lust with {hisHer} body.  ");
				if (oral) {
					append(w, t, "One of them grabs {himHer} by the hair and stuffs his thick cock down {mainName}'s narrow throat, jackhammering in and out, suffocating {himHer} even as ");
				} else if (w.tickle()) {
					append(w, t, "Four of them pin {hisHer} limbs down, preventing {himHer} from protecting {himHer}self even a little from the others jabbing their fingers into {hisHer} sides and armpits in an especially sadistic bout of tickling.  Then, when {heShe}'s too exhausted to stand, ");
				} else {
					append(w, t, "They slap {himHer} around, taking sadistic glee out of punching {hisHer} belly and slamming {hisHer} head into the ground until {heShe} sees stars.  Then, when {heShe} can't stand up anymore, ");
				}
				if (vaginal && anal) {
					append(w, t, "the others take turns fucking {himHer} from both sides, stretching {hisHer} pussy and anus both as they savagely ram their inhumanly large shafts into {hisHer} deepest parts.  ");
				} else if (anal) {
					append(w, t, "the others take turns fucking {hisHer} ass, stretching it to the point of gaping as they savagely ram their inhumanly large shafts deep into {hisHer} bowels over and over again.  ");
				} else if (vaginal) {
					append(w, t, "the others take turns fucking {himHer}, stretching {hisHer} pussy to the point of gaping as they savagely ram their inhumanly large shafts deep enough to stretch {hisHer} cervix over and over again.  ");
				} else {
					append(w, t, "the others grind their shafts against {hisHer} body or masturbate in {hisHer} direction.  ");
				}
				append(w, t, "Finally, {heShe}'s left gasping for breath in a large pool of cum.  ");
				Project.changePortrait(gender, type, Project.displayedCivilians[0], true, w, Project.displayedNames, 0, Project.Emotion.STRUGGLE, Project.Emotion.LEWD);
			} else {
				append(w, t, "Two of the Thralls grab {hisHer} arms and pull them apart");
				if (w.tickle()) {
					append(w, t, " while another two grab {hisHer} legs, and the rest begin tickling {himHer} all over, digging into {hisHer} armpits and sides with superhuman strength.  Soon, {mainName} is outright crying at the borderline painful level of stimulation.  ");
				} else {
					append(w, t, " almost hard enough to dislocate them.  The others take turns punching {himHer} in the belly as hard as they can, testing out their superhuman strength.  {mainName} chokes and heaves, unable to breathe.  ");
				}
				Project.changePortrait(gender, type, Project.displayedCivilians[0], true, w, Project.displayedNames, 0, Project.Emotion.STRUGGLE, Project.Emotion.SHAME);
			}
			if (hostility < 60) {
				if (morality > 66) {
					append(w, t, "{HeShe} tells {himHer}self that this shouldn't surprise {himHer} after what {heShe} went through as one of the Chosen, but the sheer difference in power between {himHer} and them makes it feel even worse.");
				} else if (morality > 33) {
					append(w, t, "{HeShe} keeps expecting the Thralls to tire of hurting {himHer}, but the torment continues seemingly without end.");
				} else {
					append(w, t, "{HeShe}'s shocked and enraged by how far the Thralls are going with {himHer}, but the fury in {hisHer} eyes just makes them laugh and escalate even further.");
				}
			} else {
				if (morality > 66) {
					append(w, t, "{HeShe} soothes {himHer}self by planning out ways to get revenge on the Thralls after this is over.  No trace of {hisHer} compassionate nature remains.");
				} else if (morality > 33) {
					append(w, t, "{HeShe} can't even get angry anymore about this sort of treatment.  It's what {heShe}'s come to expect from people.");
				} else {
					append(w, t, "It's undeniably an unpleasant experience, but a part of {mainName} is contemptful of the Thralls for their pitifully simple efforts to make {himHer} suffer.");
				}
			}
		} else if (nextTraining == 14) {
			if (bodyPresent) {
				...
				append(w, t, "You deploy a specialized Commander body to {mainName}'s location, equipped with countless biomechanical tentacles.  ");
			}
			...
			if (consenting) {
				...
				append(w, t, "{mainName} smiles for the viewers who are suddenly seeing {himHer} on their screen, promising them a show they won't soon forget.  ");
				if (hostility < 70) {
					if (morality > 66) {
						append(w, t, "A part of {himHer} takes a masochistic joy out of being able to provide this sort of salacious entertainment to so many people.");
					} else if (morality > 33) {
						append(w, t, "{HeShe}'s decided to enjoy this as much as {heShe} can.");
					} else {
						append(w, t, "{HeShe} can't help but enjoy the prospect of causing so much chaos and confusion just by showing off a little.");
					}
				} else {
					if (morality > 66) {
						append(w, t, "The smile doesn't reach {hisHer} eyes, as {heShe} can't hide {hisHer} contempt for the people who enjoy this sort of show.");
					} else if (morality > 33) {
						append(w, t, "Although {hisHer} expression is bright, {hisHer} words are just the opposite, criticizing and cursing at everyone who's watching.");
					} else {
						append(w, t, "{HeShe} has nothing but hatred for the general public who will be seeing this, but {heShe} knows that the despair of seeing a former member of the Chosen in this situation will far outweigh the sexual pleasure they feel.");
					}
				}
			} else {
				append(w, t, "The unsuspecting viewers are shocked when their screens suddenly show {mainName} struggling in vain against the tentacles coiling around {himHer}.  ");
				if (hostility < 70) {
					if (morality > 66) {
						append(w, t, "{HeShe} knows that there are at least a few people out there who genuinely don't get any pleasure from seeing this, and {heShe} doesn't want it to be forced on them.");
					} else if (morality > 33) {
						append(w, t, "{HeShe} still isn't able to completely let go of {hisHer} desire to be viewed with respect.");
					} else {
						append(w, t, "{HeShe} has convinced {himHer}self that {heShe} doesn't really care what other people see or think, but {heShe} still feels a surge of spiteful stubbornness that motivates {himHer} to resist.");
					}
					Project.changePortrait(gender, type, Project.displayedCivilians[0], true, w, Project.displayedNames, 0, Project.Emotion.SHAME, Project.Emotion.STRUGGLE);
				} else {
					if (morality > 66) {
						append(w, t, "{HeShe} no longer has any complaints about producing propaganda to make the public despair, but {heShe} hates the thought of letting them enjoy {hisHer} predicament.");
					} else if (morality > 33) {
						append(w, t, "{HeShe}'s unable to retain any semblance of rational thought, and even though {heShe} knows it's hopeless, {heShe} still thrashes and curses at the viewers, unwittingly making the show even more memorable.");
					} else {
						append(w, t, "The fact that the human public {heShe} hates so strongly will be enjoying this show drives {himHer} insane with rage.");
					}
					Project.changePortrait(gender, type, Project.displayedCivilians[0], true, w, Project.displayedNames, 0, Project.Emotion.ANGER, Project.Emotion.STRUGGLE);
				}
			}
			w.append(t, "\n\n");
			if ((vaginal || anal) && (currentTraining[0] || (currentTraining[6] && (consenting == false || orgasmsGiven >= 1000)) || currentTraining[10])) {
				append(w, t, "After panning across {hisHer} face, the cameras focus on the spot where {heShe}'s joined to the Thrall");
				if (vaginal && anal) {
					append(w, t, "s fucking {himHer} in both holes.  After they finish, the viewers get a brief glimpse of both holes gaping open and overflowing with cum ");
				} else if (vaginal) {
					append(w, t, " fucking {himHer}.  After he finishes, the viewers get a brief glimpse of {hisHer} folds gaping open and overflowing with cum ");
				} else {
					append(w, t, " penetrating {hisHer} ass.  After he finishes, the viewers get a brief glimpse of {mainName}'s ass gaping open and overflowing with cum ");
				}
				append(w, t, "before another Thrall's cock comes into view, and then another, and then even more.  It's made clear that {mainName} has become little more than an outlet for men's desires.  ");
			} else {
				if (oral) {
					append(w, t, "A Thrall's cock comes into view and {mainName} is forced to suck it while the camera rotates around {himHer} and focuses on {hisHer} ass.  ");
				} else {
					append(w, t, "The cameras linger on {mainName}'s face long enough to make sure the viewers remember it, and then they start rotating around to show {hisHer} body.  ");
				}
				append(w, t, "More tendrils slither around {himHer}, tearing {hisHer} clothes and pulling them out of the way.  {HisHer} legs are pulled apart, and then ");
				if (gender == Gender.MALE) {
					append(w, t, "{hisHer} anus is spread painfully wide");
				} else {
					append(w, t, "{hisHer} pussy is spread open as well");
					if (gender == Gender.FUTANARI) {
						append(w, t, ", {hisHer} penis in plain view");
					}
				}
				append(w, t, ", a gesture clearly meant to humiliate {mainName}, with no other purpose.  ");
			}
			if (disgrace < 70) {
				if (dignity > 66) {
					append(w, t, "As one of the Chosen, {originalName} was a worldwide celebrity recognized on sight, and most of the viewers are shocked at seeing {himHer} like this.");
				} else if (dignity > 33) {
					append(w, t, "{originalName} had a decently-sized fanbase as one of the Chosen, but with this, {heShe}'s at risk of becoming even more infamous as one of the Forsaken.");
				} else {
					append(w, t, "{originalName} was never a particularly well-known member of the Chosen, and for those seeing {himHer} for the first time, this is an exceptionally poor first impression.");
				}
			} else {
				if (dignity > 66) {
					append(w, t, "Rumors of {originalName}'s humiliating defeat had already been spreading, but for many viewers, this is the moment in which they truly start believing them.");
				} else if (dignity > 33) {
					append(w, t, "Many of the viewers aren't convinced that {mainName} was ever one of the Chosen.");
				} else {
					append(w, t, "Footage of {mainName} being humiliated during {hisHer} time as one of the Chosen is already popular on certain websites, and many of the viewers eagerly save this new video to add it to their existing collection of {mainName} porn.");
				}
			}
			int views = 300000 + (int)(Math.random()*300000);
			timesExposed += views;
			if (consenting) {
			...
			if (bodyPresent) {
				append(w, t, "More and more tentacles continue to erupt from your body, wrapping around {mainName}'s limbs, stroking {himHer} all over, and tweaking {hisHer} nipples and " + organ + ".  ");
			} else {
				append(w, t, "You deploy a Commander body to {mainName}'s location covered in various types of tentacles, some thick enough to grab and subdue, others fine and specialized for sexual stimulation.  ");
			}
			if (consenting) {
				append(w, t, "{mainName} doesn't resist, allowing you to spread {hisHer} legs wide apart.  ");
				if (flavorObedience() < 60) {
					if (confidence > 66) {
						append(w, t, "{HeShe} shows {hisHer} bravado by warning you not to go too far, but there's a tremor in {hisHer} voice.");
					} else if (confidence > 33) {
						append(w, t, "{HeShe} gulps, wondering whether {heShe}'ll really be able to keep {hisHer} sanity after what's coming next.");
					} else {
						append(w, t, "{HeShe}'s plainly terrified, but {heShe} keeps control of {himHer}self with visible effort.");
					}
				} else {
					if (confidence > 66) {
						append(w, t, "{HeShe} grins with anticipation.");
					} else if (confidence > 33) {
						append(w, t, "{HeShe} looks back at you with an affectionate glance.");
					} else {
						append(w, t, "A smile of mingled fear and eagerness is frozen on {hisHer} face.");
					}
				}
			} else {
				if (inPublic) {
					append(w, t, "The crowd hands {himHer} off to you, then backs away to watch what will happen next, many of them masturbating as you force {hisHer} legs wide apart and show them everything.  ");
				} else {
					append(w, t, "{HeShe} tries to resist, but {heShe}'s near strong enough to stop you from binding {hisHer} arms behind {hisHer} back and spreading {hisHer} legs wide apart.  ");
				}
				if (obedience < 60) {
					if (confidence > 66) {
						append(w, t, "Even as your tentacles converge on {hisHer} crotch, {heShe} squirms and struggles until the last moment, trying to buck {hisHer} hips away from the approaching appendages.");
					} else if (confidence > 33) {
						append(w, t, "{HeShe} closes {hisHer} eyes and clenches {hisHer} teeth, willing {himHer}self not to feel what comes next.");
					} else {
						append(w, t, "{HisHer} eyes go wide and {heShe} tries to shrink back as tentacles with suckers and pinchers and rough slimy surfaces converge on {himHer}.");
					}
				} else {
					if (confidence > 66) {
						append(w, t, "The knowledge of what's to come drives {himHer} into a panic, and {heShe} just barely clings to obedience, freezing up completely.");
					} else if (confidence > 33) {
						append(w, t, "{HeShe} shouts wordlessly and tries to claw {himHer}self free, acting on instinct and forgetting {hisHer} supposed loyalty to you.");
					} else {
						append(w, t, "{HeShe} panics, thrashing about and screaming in fear of the intense stimulation to come.");
					}
				}
			}
			append(w, t, "\n\nThe tentacles smear aphrodisiac fluid all over {hisHer} body, twining around {hisHer} nipples, ");
			if (gender == Gender.MALE) {
				append(w, t, "caressing {hisHer} balls");
			} else {
				append(w, t, "squeezing {hisHer} breasts");
			}
			if (timesTortured > 0 || enjoyedAnal > 0) {
				if (w.tickle()) {
					append(w, t, " and tickling {hisHer} feet and armpits");
				} else if (gender == Gender.MALE) {
					append(w, t, " and sliding up {hisHer} ass");
				} else {
					w.append(t, " before giving them a sharp squeeze, alternating between pain and pleasure");
				}
			} else {
				append(w, t, " and trailing up and down {hisHer} thighs, probing {hisHer} mouth and making {himHer} stick out {hisHer} tongue");
			}
			if (timesHadSex > 0) {
				if (gender == Gender.MALE) {
					append(w, t, ", then sliding up {hisHer} ass to grind against {hisHer} prostate");
				} else {
					append(w, t, ", then pushing deep into {hisHer} pussy to grind against {hisHer} g-spot");
				}
			} else {
				if (gender == Gender.MALE) {
					append(w, t, ", then gently teasing {hisHer} virgin anus");
				} else {
					append(w, t, ", then gently spreading {hisHer} lower lips and teasing {hisHer} virgin pussy");
				}
			}
			append(w, t, " while a set of fine tendrils repeatedly tugs {hisHer} " + organ + ".  The combined stimulation makes {himHer} cum instantly, ");
			if (timesOrgasmed == 0) {
				append(w, t, ", leaving {himHer} dazed and confused as {heShe} tries to understand these strange new feelings, ");
			} else if (strongestOrgasm < 200) {
				append(w, t, ", a stronger orgasm than {heShe}'s ever felt in {hisHer} entire life, ");
			}
			append(w, t, "but the aphrodisiacs have put {himHer} in an excited state that won't be ended by cumming just once.  {HeShe} cums again and again, the orgasms overlapping each other and leaving {himHer} wailing and shuddering helplessly with {hisHer} limbs splayed out.  ");
			if (deviancy < 70) {
				if (innocence > 66) {
					append(w, t, "{HeShe}'s practically catatonic, eyes wide and staring");
				} else if (innocence > 33) {
					append(w, t, "{HeShe} whimpers that {heShe} doesn't want to cum anymore");
				} else {
					append(w, t, "{HeShe} tries to talk, but {hisHer} words are slurred and unintelligible");
				}
			} else {
				if (innocence > 66) {
					append(w, t, "{HisHer} eyes roll and {hisHer} tongue hangs out in a ridiculous smile");
				} else if (innocence > 33) {
					append(w, t, "{HeShe} begs you not to stop, begs to be made to cum even more");
				} else {
					append(w, t, "{HeShe} continuously tries to hump {himHer}self against you, unable to think of anything beyond seeking even more pleasure");
				}
			}
			if (currentTraining[0] || (currentTraining[6] && (consenting == false || orgasmsGiven >= 1000)) || currentTraining[10]) {
				append(w, t, ", and {heShe}'s completely defenseless against the crowd that rushes forward again, eager to take your tentacles' place.");
			} else {
				append(w, t, ", and when you finally give {himHer} a chance to catch {hisHer} breath, {heShe}'s too weak to even stand on {hisHer} own.");
			}
			Project.changePortrait(gender, type, Project.displayedCivilians[0], true, w, Project.displayedNames, 0, Project.Emotion.LEWD, Project.Emotion.STRUGGLE);
			int newLength = 200 + (int)(Math.random()*200);
			if (newLength > strongestOrgasm) {
				strongestOrgasm = newLength;
			}
			timesOrgasmed += 10 + (int)(Math.random()*11);
		} else if (nextTraining == 16) {
			if (inPublic) {
				append(w, t, "The crowd around {mainName} becomes larger and larger, summoned by your telepathic announcement.  ");
			} else if (consenting) {
				if (helpless) {
					append(w, t, "You free {mainName} and allow {himHer} to recover enough to head out into public and gather a crowd before resuming the training.  ");
				} else {
					append(w, t, "{mainName} is instructed to head out into public and gather a crowd.  ");
				}
			} else {
				if (helpless) {
					append(w, t, "You use your telepathic abilities to summon a crowd of spectators to the helpless {mainName}'s location.  ");
				} else {
					append(w, t, "You use your telepathic abilities to summon a crowd of spectators to {mainName}'s location, and {hisHer} eyes dart back and forth as {heShe} looks for a way to escape.  ");
				}
			}
			if (bodyPresent) {
			...
			if (consenting) {
				if (deviancy < 60) {
					if (innocence > 66) {
						append(w, t, "{mainName} shifts in obvious discomfort as {heShe} looks at them all, unable to imagine what some of them are used for.");
					} else if (innocence > 33) {
						append(w, t, "{mainName} looks intimidated, wanting to back out, but it's too late now.");
					} else {
						append(w, t, "{mainName} frowns, imagining what each type of tentacle might be used for, and uncomfortable with some of the conclusions {heShe} reaches.");
					}
				} else {
					if (innocence > 66) {
						append(w, t, "{mainName} proudly announces to everyone what each of the tentacles will be doing to {himHer}, happy to show off the knowledge {heShe}'s built after repeated training.");
					} else if (innocence > 33) {
						append(w, t, "{mainName} smiles eagerly.");
					} else {
						append(w, t, "{mainName} opens {hisHer} mouth for one of the tentacles, encouraging you to get started as quickly as possible.");
					}
				}
			} else {
				if (deviancy < 60) {
					if (innocence > 66) {
						append(w, t, "{mainName} struggles wildly, screaming in fear and disgust, until you plug {hisHer} mouth with a thick tentacle that delves all the way into {hisHer} stomach.");
					} else if (innocence > 33) {
						append(w, t, "{mainName} tries to struggle, but this only gives you a chance to show off your tentacles' prowess, bending {himHer} over backwards so that {hisHer} ankles are locked together over {hisHer} head.");
					} else {
						append(w, t, "{mainName} shudders, but {heShe} doesn't bother resisting the tentacles that twine around {himHer}, preferring to preserve {hisHer} strength for what's to come.");
					}
				} else {
					if (innocence > 66) {
						append(w, t, "{mainName} shouts in protest, telling you to slow down, but {hisHer} body's reaction is completely different, {hisHer} back arching and {hisHer} legs spreading wide to welcome you.  {HeShe} doesn't even seem aware of the contradiction.");
					} else if (innocence > 33) {
						append(w, t, "{mainName} tries to scream, but it comes out as a lewd moan, and {hisHer} struggles are pitifully weak.  {HisHer} body has already been trained to accept you.");
					} else {
						append(w, t, "{mainName}'s expression flickers between fear and ecstasy, {hisHer} mind fracturing from the incompatibility between {hisHer} resistance and {hisHer} addiction to the pleasure only you can give.");
					}
				}
			}
			w.append(t, "\n\n");
			if (disgrace < 60) {
				if (dignity > 66) {
					append(w, t, "People have gathered from far and wide to see the world-famous {mainName}'s show, and they aren't disappointed.  ");
				} else if (dignity > 33) {
					append(w, t, "For the next hour, the spectators are shocked and awed, not only by the things you're doing to {mainName}'s body, but also by the fact that this is happening to a former member of one of the supposedly-inviolable Chosen.  ");
				} else {
					append(w, t, "{mainName} proves to be a useful subject for demonstrating your techniques, as {heShe} doesn't hold back with {hisHer} wailing and thrashing, ensuring that everyone who watches understands just how intense the stimulation must be.  ");
				}
			} else {
				if (dignity > 66) {
					append(w, t, "Even though {mainName} has long since lost {hisHer} indomitable reputation, {heShe} still has appeal as a former worldwide celebrity, and the crowd is dense with people jostling for a better look.  ");
				} else if (dignity > 33) {
					append(w, t, "The novelty of seeing {mainName} abused has started to wear off, and the watching crowd shouts out some especially extreme suggestions on what you should do to {himHer} in order to keep things fresh.  ");
				} else {
					append(w, t, "By this point, {mainName} has earned a reputation for {himHer}self as an entertaining subject in this kind of show, and today is no exception.  ");
				}
			}
			if (gender != Gender.MALE) {
				if (currentTraining[12]) {
					append(w, t, "You resume fucking {himHer}, although this time, your shaft is even larger and slightly prehensile, capable of curving inside {hisHer} vaginal walls to put pressure on {hisHer} clitoris from the inside while your other tentacles fondle the outside.  The stimulation is intense enough to make {himHer} squeal into the tentacle going down {hisHer} throat.  ");
				} else if (timesHadSex > 0) {
					append(w, t, "A massive, studded tentacle stretches {hisHer} vagina as it forces its way in, then pushes further and further inside, penetrating {hisHer} cervix and filling {hisHer} womb so full that {hisHer} belly begins to bulge as {heShe} squeals into the tentacle going down {hisHer} throat.  ");
				} else {
					append(w, t, "Hooked tentacles latch onto {hisHer} lower lips, spreading them wider and wider, stretching {hisHer} tender flesh to its limit.  Then, another grabs {hisHer} " + organ + ", tugging and stretching, pulling harder and harder until {heShe}'s forced to push {hisHer} hips as far forward as they'll go, moaning into the tentacle going down {hisHer} throat.  ");
				}
			} else {
				if (currentTraining[17]) {
					if (w.tickle()) {
						append(w, t, "Ridged tentacles, tentacles with brushes, tentacles with spinning bristles, and vibrating tentacles all dig into {mainName}'s armpits, sides, the soles of {hisHer} feet, and everywhere else, tickling {himHer} until squealing laughter can be heard over the tentacle going down {hisHer} throat.  ");
					} else {
						append(w, t, "Tentacles with pinchers attack {hisHer} nipples, tentacles as strong as steel cable constrict {hisHer} penis and crush {hisHer} balls, and an especially fine tentacle penetrates {hisHer} urethra, sliding and squirming while {heShe} squeals around the tentacle going down {hisHer} throat.  ");
					}
				} else {
					append(w, t, "An unusual hollow tentacle, transparent to show what's happening, envelops {hisHer} penis and begins applying powerful suction, sliding up and down its length, persistently milking the shaft.  {mainName}'s eyes glaze over as {heShe} moans into the tentacle going down {hisHer} throat.  ");
				}
			}
			if ((gender != Gender.MALE && timesTortured > 0) || (gender == Gender.MALE && timesHadSex > 0)) {
				if (currentTraining[17]) {
					append(w, t, "Suddenly, {hisHer} eyes widen and bulge as that tentacle begins pushing further, going all the way through {hisHer} stomach and into {hisHer} bowels.  A chorus of 'ooh' and 'aah' comes from the crowd as they see the tentacle emerge from {hisHer} ass, penetrating {himHer} so deeply that it's come out the other side.  ");
				} else {
					append(w, t, "Another even larger tentacle, thicker than {hisHer} arm, pries {hisHer} ass open and begins fucking {himHer} in that hole as well.  ");
				}
			} else {
				append(w, t, "A torrent of slime is shot into {hisHer} belly, and it quickly overflows, streaming from {hisHer} mouth and nose.  {HeShe} chokes and gags, eyes rolling as {heShe} struggles for breath.  ");
			}
			if (currentTraining[15]) {
				Project.changePortrait(gender, type, Project.displayedCivilians[0], true, w, Project.displayedNames, 0, Project.Emotion.LEWD, Project.Emotion.STRUGGLE);
				append(w, t, "The extreme treatment would normally be nothing but unpleasant, but your aphrodisiac leaves {himHer} feeling nothing but pleasure, and {heShe} cums several times before you release {himHer}.");
				timesOrgasmed += 3 + (int)(Math.random()*4);
			} else if (timesOrgasmed > 0) {
				Project.changePortrait(gender, type, Project.displayedCivilians[0], true, w, Project.displayedNames, 0, Project.Emotion.LEWD, Project.Emotion.STRUGGLE);
				append(w, t, "For a regular human, it would be nothing but torture, but {mainName}'s Forsaken powers constantly preserve and repair {hisHer} body, allowing the extreme treatment to drive {himHer} to orgasm by the time you're done.");
				timesOrgasmed++;
			} else {
				Project.changePortrait(gender, type, Project.displayedCivilians[0], true, w, Project.displayedNames, 0, Project.Emotion.STRUGGLE, Project.Emotion.LEWD);
				append(w, t, "Every time {mainName} passes out from the extreme treatment, {hisHer} Forsaken powers revive {himHer}, and you're able to continue demonstrating the limits of {hisHer} body for quite awhile before releasing {himHer}.");
			}
		} else if (nextTraining == 17) {
			if (bodyPresent) {
				...
				if (inPublic) {
					append(w, t, "The crowd around {mainName} parts to let your Commander body through, an enormous mass of powerful tentacles that throb and pulse with unworldly strength.  ");
				} else if (consenting) {
					append(w, t, "You send a Commander body to {mainName}, an enormous mass of powerful tentacles that throb and pulse with unworldly strength.  A curious crowd trails behind, eager to see what such a terrifying body will be used for.  {mainName} waits for you, trembling, and then allows you to wrap those tentacles around {hisHer} much smaller form.  ");
				} else {
					append(w, t, "You send a Commander body to catch {mainName}, and while {heShe} tries to flee at first, this is a powerful combat-form with massive, powerful tentacles.  It's no trouble at all to pounce atop {himHer} and drag {himHer} out into public.  ");
				}
			}
			if (w.tickle()) {
				append(w, t, "The tentacles terminate in rigid, almost sharp, points, and you drive those points into {mainName}'s most sensitive places without mercy.  {HisHer} armpits and feet go without saying, but you even attack {hisHer} neck, {hisHer} thighs, and around {hisHer} crotch with brutal tickling.  ");
				if (obedience < 70) {
					if (confidence > 66) {
						append(w, t, "{HeShe} resists as long as {heShe} can, but it's only a matter of time until {heShe}'s screaming with laughter, tears running down {hisHer} face.");
					} else if (confidence > 33) {
						append(w, t, "{HeShe} tries to suppress {hisHer} laughter, but it comes out in shrieking bursts before {heShe} can regain control.  Each burst is longer than the last, and eventually {heShe}'s laughing constantly, having completely lost control of {himHer}self.");
					} else {
						append(w, t, "{HeShe} starts laughing immediately, hoping that you'll be satisfied with that much, but you only continue to tickle {himHer} even harder.  Soon, {heShe} couldn't stop even if {heShe} tried, and you force the laughter out of {himHer} until {heShe}'s wheezing for breath.");
					}
				} else {
					if (confidence > 66) {
						append(w, t, "{HeShe} holds {hisHer} laughter in as long as {heShe} can, not out of pride, but because {heShe}'s developed a masochistic enjoyment from encouraging you to escalate and tickle {himHer} even harder.  When {heShe} finally starts laughing, there's a true crazed joy in {hisHer} eyes.");
					} else if (confidence > 33) {
						append(w, t, "{HeShe} doesn't resist, immediately beginning to laugh, but in between {hisHer} wild cackling, {heShe} begs you to tickle {himHer} even harder.");
					} else {
						append(w, t, "{HisHer} well-trained body submits instantly, releasing screams of high-pitched laughter outside {hisHer} control, and {heShe} takes pride in the fact that {heShe}'s been shaped to your desires so thoroughly.");
					}
				}
			} else {
				append(w, t, "The tentacles pummel {himHer}, slap {himHer}, spank {himHer}, whip {himHer}.  They seize {hisHer} limbs, twisting, crushing, tugging.  ");
				if (gender == Gender.MALE) {
					append(w, t, "The one coiled around {hisHer} penis almost pulls it out by the root.  ");
				} else {
					append(w, t, "A tentacle with a slender tip but a base thicker than {hisHer} waist pushes into {hisHer} anus, and the further it goes in, the further the orifice is stretched.  A normal human would have {hisHer} organs crushed, but {mainName}'s belly just swells up from the impossible insertion, {hisHer} mouth gaping open as {heShe} struggles to breathe.  ");
				}
				if (obedience < 70) {
					if (confidence > 66) {
						append(w, t, "{HeShe} toughs it out as long as {heShe} can, but it's only a matter of time until {heShe}'s screaming and wailing, tears running down {hisHer} face.");
					} else if (confidence > 33) {
						append(w, t, "{HeShe} tries to hold back {hisHer} screams, but it still comes out in bursts of shrieking before {heShe} can regain control.  Each scream is longer than the last, and eventually {heShe}'s shrieking nonstop, having completely lost control of {himHer}self.");
					} else {
						append(w, t, "{HeShe} starts crying immediately, hoping that you'll be satisfied with that much, but you only continue to torture {himHer} without mercy.  Soon, {heShe} couldn't stop screaming even if {heShe} tried.");
					}
				} else {
					if (confidence > 66) {
						append(w, t, "{HeShe} holds in {hisHer} screams as long as {heShe} can, not out of pride, but because {heShe}'s developed a masochistic enjoyment from encouraging you to torture {himHer} even harder.  When {heShe} finally breaks, there's a true crazed joy in {hisHer} eyes.");
					} else if (confidence > 33) {
						append(w, t, "{HeShe} doesn't resist, even as {heShe} screams in pain, but in between {hisHer} pitiful squeals, {heShe} begs you to torture {himHer} even harder.");
					} else {
						append(w, t, "{HeShe} submits completely, not even trying to defend {himHer}self, and there's a glint of pride in {hisHer} eyes at being able to overcome {hisHer} fear and submit {himHer}self even to a torture like this.");
					}
				}
			}
			w.append(t, "\n\n");
			if (disgrace < 50) {
				if (dignity > 66) {
					append(w, t, "For some of the watchers, those who still thought of {mainName} as powerful and dignified, the show comes as an unpleasant shock.  But for most, it's just an erotic show.  ");
				} else if (dignity > 33) {
					append(w, t, "None of the watchers can blame {mainName} for being unable to hold back {hisHer} voice, but the show still leaves an impression on them.  ");
				} else {
					append(w, t, "{mainName}'s loud voice draws even more spectators, a growing crowd of people come to see the former hero brought low.  ");
				}
			} else {
				if (dignity > 66) {
					append(w, t, "Even back when {heShe} was a hero with worldwide renown, {mainName} wouldn't have been able to endure this kind of treatment with a stoic face.  But from the outside, it looks like {heShe}'s fallen far.  ");
				} else if (dignity > 33) {
					append(w, t, "No one is surprised by the display.  By this point, it's expected that {mainName} won't even be able to put up the slightest bit of token resistance.  ");
				} else {
					append(w, t, "The sound of {mainName}'s howling carries far and wide, and while that once would have drawn an extra-large crowd of spectators, many of the people who hear just continue to go about their business, recognizing the sound of one of {hisHer} routine humiliations.  ");
				}
			}
			if (currentTraining[0] || (currentTraining[6] && (consenting == false || orgasmsGiven >= 1000)) || currentTraining[10]) {
				append(w, t, "When you release {himHer} to fall limp on the ground, everyone surges forward to satisfy their lust without hesitation.");
			} else {
				append(w, t, "The end of the show is met with cries for an encore, and as {mainName} is released to fall limp onto the ground, {heShe}'s pelted by thrown garbage by the jeering crowd.");
			}
			Project.changePortrait(gender, type, Project.displayedCivilians[0], true, w, Project.displayedNames, 0, Project.Emotion.SWOON, Project.Emotion.STRUGGLE);
			timesTortured++;
		}
		w.append(t, "\n\n");
		grantExpertise(t, w, nextTraining % 6, gainedExpertise);
		currentTraining[nextTraining] = true;
		JButton Continue = new JButton("Continue");
		Continue.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
	...
	public void chooseCombatStyle() {
		...
		int choice = 0;
		long hateTotal = hateExp*(200-morality);
		long pleaTotal = pleaExp*(200-innocence);
		long injuTotal = injuExp*(200-confidence);
		long expoTotal = expoExp*(200-dignity);
		long[] weights = new long[15];
		weights[1] = (hateTotal*2700/(pleaTotal+injuTotal+expoTotal));
		weights[2] = (pleaTotal*2700/(hateTotal+injuTotal+expoTotal));
		weights[3] = (injuTotal*2700/(hateTotal+pleaTotal+expoTotal));
		weights[4] = (expoTotal*2700/(hateTotal+pleaTotal+injuTotal));
		weights[5] = (hateTotal+pleaTotal)*1000/(injuTotal+expoTotal);
		weights[6] = (hateTotal+injuTotal)*1000/(pleaTotal+expoTotal);
		weights[7] = (hateTotal+expoTotal)*1000/(pleaTotal+injuTotal);
		weights[8] = (pleaTotal+injuTotal)*1000/(hateTotal+expoTotal);
		weights[9] = (pleaTotal+expoTotal)*1000/(hateTotal+injuTotal);
		weights[10] = (injuTotal+expoTotal)*1000/(hateTotal+pleaTotal);
		weights[11] = (hateTotal+pleaTotal+injuTotal)*1000/(expoTotal*3);
		weights[12] = (hateTotal+pleaTotal+expoTotal)*1000/(injuTotal*3);
		weights[13] = (hateTotal+injuTotal+expoTotal)*1000/(pleaTotal*3);
		weights[14] = (pleaTotal+injuTotal+expoTotal)*1000/(hateTotal*3);
		if (combatStyle >=0 && combatStyle < 15) {
			weights[combatStyle] = weights[combatStyle]*11/10;
		}
	...
	public void grantExpertise(JTextPane t, WorldState w, int pattern, long amount) {
		long[] expertises = {hateExp, pleaExp, injuExp, expoExp};
		for (int i = 0; i < 3; i++) {
		...
		long moved = 0;
		long reductions = expertises[0]*5/amount - 100;
		while (reductions > 0) {
			long difference = amount*23/1000;
			amount -= difference;
			moved += difference;
			reductions--;
		}
		...
		if (pattern == 0) {
			...
			expoExp -= moved;
			if (moved > 0) {
				w.append(t, INJU + "/EXPO -> " + condensedFormat(moved) + " Exp -> HATE/PLEA\n");
			}
			w.append(t, "+" + condensedFormat(amount) + " Exp HATE/PLEA");
		} else if (pattern == 1) {
			hateExp += originalAmount;
			...
			expoExp -= moved;
			if (moved > 0) {
				w.append(t, "PLEA/EXPO -> " + condensedFormat(moved) + " Exp -> HATE/" + INJU + "\n");
			}
			w.append(t, "+" + condensedFormat(amount) + " Exp HATE/" + INJU);
		} else if (pattern == 2) {
			hateExp += originalAmount;
			...
			injuExp -= moved;
			if (moved > 0) {
				w.append(t, "PLEA/" + INJU + " -> " + condensedFormat(moved) + " Exp -> HATE/EXPO\n");
			}
			w.append(t, "+" + condensedFormat(amount) + " Exp HATE/EXPO");
		} else if (pattern == 3) {
			pleaExp += originalAmount;
			injuExp += originalAmount;
			hateExp -= moved;
			expoExp -= moved;
			if (moved > 0) {
				w.append(t, "HATE/EXPO -> " + condensedFormat(moved) + " Exp -> PLEA/" + INJU + "\n");
			}
			w.append(t, "+" + condensedFormat(amount) + " Exp PLEA/" + INJU);
		} else if (pattern == 4) {
			pleaExp += originalAmount;
			expoExp += originalAmount;
			hateExp -= moved;
			injuExp -= moved;
			if (moved > 0) {
				w.append(t, "HATE/" + INJU + " -> " + condensedFormat(moved) + " Exp -> PLEA/EXPO\n");
			}
			w.append(t, "+" + condensedFormat(amount) + " Exp PLEA/EXPO");
		} else {
			injuExp += originalAmount;
			expoExp += originalAmount;
			hateExp -= moved;
			pleaExp -= moved;
			if (moved > 0) {
				w.append(t, "HATE/PLEA -> " + condensedFormat(moved) + " Exp -> " + INJU + "/EXPO\n");
			}
			w.append(t, "+" + condensedFormat(amount) + " Exp " + INJU + "/EXPO");
		}
		if (hateExp < 1) {
			hateExp = 1;
		}
		...
		if (combatStorage != combatStyle) {
			w.append(t, "\n\nNew " + describeCombatStyle(w, true));
		}
	}
	...
	public void captureChosen(JTextPane t, JPanel p, JFrame f, WorldState w, Chosen c) {
		...
		int[] baseDamage = new int[]{powerConstant, powerConstant, powerConstant, powerConstant, 0, 0, 0, 0};
		...
		styleDamage[0] = styleDamage[0]*(int)(expMultiplier(hateExp)/1000);
		styleDamage[1] = styleDamage[1]*(int)(expMultiplier(pleaExp)/1000);
		styleDamage[2] = styleDamage[2]*(int)(expMultiplier(injuExp)/1000);
		styleDamage[3] = styleDamage[3]*(int)(expMultiplier(expoExp)/1000);
		if (defeatType == 5 && obedience < 40) {
		...
		if (c.usingDetonate && c.captureProgression + c.getINJULevel() + 1 >= w.getCaptureDuration() && w.adaptationsDisabled() == false) {
			breakCapture = true;
			if (c.captureProgression == 0) {
			...
			for (int i = 0; i < 3; i++) {
				if (w.getCombatants()[i] != null) {
					if (w.getCombatants()[i] != c) {
						if (w.getCombatants()[i].isSurrounded() == false && w.getCombatants()[i].isCaptured() == false && w.getCombatants()[i].alive && w.getCombatants()[i].resolve > 0 && w.getCombatants()[i].getConfidence() < c.confidence) {
							otherKiller = true;
			...
			if (otherKiller && otherTarget == false) {
				w.readyToEnd = false;
			}
			if (c.knownForsaken == null) {
				c.knownForsaken = new Forsaken[]{this};
			} else {
		...
		if (c.captureProgression >= w.getCaptureDuration() || breakCapture) {
			...
			if (breakCapture == false) {
				if (confidence > 66) {
					append(w, t, c, "Finally, {c:mainName} lands a solid hit on {mainName}, and {mainName} is forced to retreat.  {c:mainName} returns to the battle.");
				} else if (confidence > 33) {
					append(w, t, c, "A stampede of fleeing Thralls rushes past the two combatants, and when it passes, {mainName} has lost track of {c:mainName}.");
				} else {
					append(w, t, c, "Finally, {c:mainName} manages to successfully flee {mainName} and return to the main battlefield.");
				}
				w.append(t, "\n\n");
			...
			if (firstMeeting) {
				...
				if (disgrace > 66) {
					if (c.getMorality() > 66) {
						...
						Project.changePortrait(c.convertGender(), c.type, false, false, w, w.nameCombatants(), c.combatantNumber(w), Project.Emotion.NEUTRAL, Project.Emotion.NEUTRAL);
						if (confidence > 66) {
							c.say(t, "Alright, I've heard that " + mainName + " is all bark and no bite...");
						} else if (confidence > 33) {
							c.say(t, "If this is " + mainName + ", then I should be able to handle " + himHer() + "...");
						} else {
							c.say(t, "Wait, isn't this " + mainName + "?  Why would they send out one of the weakest Forsaken?");
						}
						c.say(t, "\"\n\n");
						say(t, "\"");
						if (hostility > 66) {
	...
	public void chosenLine(JTextPane t, WorldState w, Chosen c, int[] styleDamage) {
			...
			if (styleDamage[0] > 0) {
				if (styleDamage[1] > 0) {
					if (deviancy > 66) {
						...
						if (c.confidence > 66) {
							...
							c.say(t, "\"\n\n");
							say(t, "\"");
							Project.changePortrait(gender, type, false, true, w, w.nameCombatants(), 3, Project.Emotion.JOY, Project.Emotion.ANGER);
							if (confidence > 66) {
				...
				if (disgrace > 66) {
					...
					if (c.getConfidence() > 66) {
						...
						Project.changePortrait(c.convertGender(), c.type, false, false, w, w.nameCombatants(), c.combatantNumber(w), Project.Emotion.FEAR, Project.Emotion.SHAME);
						if (c.getMorality() > 66) {
							c.say(t, "I d-don't want to fight " + himHer() + "...");
						} else if (c.getMorality() > 33) {
		...
		} else if (c.captureProgression % 6 == 1) {
			if (styleDamage[1] > 0) {
				if (styleDamage[2] > 0) {
					if (disgrace > 66) {
						c.say(t, "\"");
					...
					if (deviancy > 66) {
						...
						if (c.getDignity() > 66) {
							...
							say(t, "\"\n\n");
							c.say(t, "\"");
							Project.changePortrait(c.convertGender(), c.type, false, false, w, w.nameCombatants(), c.combatantNumber(w), Project.Emotion.SHAME, Project.Emotion.LEWD);
							if (hostility > 66) {
				...
				if (disgrace > 66) {
					...
					if (c.getInnocence() > 66) {
						...
						Project.changePortrait(c.convertGender(), c.type, false, false, w, w.nameCombatants(), c.combatantNumber(w), Project.Emotion.FEAR, Project.Emotion.SHAME);
						if (innocence > 66) {
							c.say(t, "I need to outsmart " + himHer() + " somehow...!");
						} else if (innocence > 33) {
		...
		} else if (c.captureProgression % 6 == 2) {
			if (styleDamage[2] > 0) {
				if (styleDamage[3] > 0) {
					if (disgrace > 66) {
						c.say(t, "\"");
				...
				if (deviancy > 66) {
					...
					if (c.getConfidence() > 66) {
						...
						if (disgrace > 66) {
							...
							c.say(t, "If I can find the others before " + heShe() + " catches me...");
						} else {
							c.say(t, "I can't let " + himHer() + " catch me...!");
						}
						c.say(t, "\"\n\n");
						say(t, "\"");
						Project.changePortrait(gender, type, false, true, w, w.nameCombatants(), 3, Project.Emotion.ANGER, Project.Emotion.FOCUS);
				...
				if (hostility > 66) {
					...
					if (c.getMorality() > 66) {
						Project.changePortrait(c.convertGender(), c.type, false, false, w, w.nameCombatants(), c.combatantNumber(w), Project.Emotion.SHAME, Project.Emotion.STRUGGLE);
						if (deviancy > 66) {
							c.say(t, "I know you're in there somewhere, " + originalName + "!  Wake up!");
						} else if (deviancy > 33) {
							c.say(t, "There's no way you actually enjoy doing this, " + mainName + "!  You can stop anytime!");
						} else {
							c.say(t, "I can tell that it's hurting you to do this, " + mainName + "!  You belong on the side of Good!");
						}
						c.say(t, "\"\n\n");
						say(t, "\"");
						if (flavorObedience() > 66) {
		...
		} else if (c.captureProgression % 6 == 3) {
			if (styleDamage[1] > 0) {
				if (styleDamage[3] > 0) {
					if (disgrace > 66) {
						say(t, "\"");
					...
					if (disgrace > 66) {
						...
						if (c.getConfidence() > 66) {
							...
							Project.changePortrait(c.convertGender(), c.type, false, false, w, w.nameCombatants(), c.combatantNumber(w), Project.Emotion.FEAR, Project.Emotion.SHAME);
							if (confidence > 66) {
								c.say(t, "When " + heShe() + " looks at me like that...");
							} else if (confidence > 33) {
				...
				if (disgrace > 66) {
					...
					if (c.getConfidence() > 66) {
						...
						if (dignity > 66) {
							...
							c.say(t, "Hard to follow " + hisHer() + " attacks...!");
						}
					} else {
						if (deviancy > 66) {
							Project.changePortrait(gender, type, false, true, w, w.nameCombatants(), 3, Project.Emotion.LEWD, Project.Emotion.JOY);
		...
		} else if (c.captureProgression % 6 == 4) {
			if (styleDamage[0] > 0) {
				if (styleDamage[3] > 0) {
					if (hostility > 66) {
						say(t, "\"");
						...
						if (c.getDignity() > 66) {
							...
							Project.changePortrait(c.convertGender(), c.type, false, false, w, w.nameCombatants(), c.combatantNumber(w), Project.Emotion.ANGER, Project.Emotion.STRUGGLE);
							if (flavorObedience() > 66) {
								c.say(t, "That must have been what " + heShe() + " was after all along...!");
							} else if (flavorObedience() > 33) {
							...
							if (disgrace > 66) {
								...
								c.say(t, "I've got " + himHer() + " on the run...!");
							} else {
								c.say(t, "I can't let " + himHer() + " out of my sight...");
							}
							c.say(t, "\"\n\n");
							say(t, "\"");
							Project.changePortrait(gender, type, false, true, w, w.nameCombatants(), 3, Project.Emotion.FOCUS, Project.Emotion.JOY);
					...
					if (hostility > 66) {
						...
						if (c.getInnocence() > 66) {
							...
							if (flavorObedience() > 66) {
								...
								c.say(t, "This is what the Demons have done to " + himHer() + "...");
							} else {
								c.say(t, "I think... the Demon Lord is actually holding " + himHer() + " back...");
							}
							c.say(t, "\"\n\n");
							say(t, "\"");
							Project.changePortrait(gender, type, false, true, w, w.nameCombatants(), 3, Project.Emotion.ANGER, Project.Emotion.STRUGGLE);
							...
							Project.changePortrait(c.convertGender(), c.type, false, false, w, w.nameCombatants(), c.combatantNumber(w), Project.Emotion.FOCUS, Project.Emotion.STRUGGLE);
							if (disgrace > 66) {
								c.say(t, "I must not let " + himHer() + " escape.");
							} else if (disgrace > 33) {
								c.say(t, "Where...?");
							} else {
								c.say(t, "Now, while " + heShe() + "'s distracted...!");
							}
							c.say(t, "\"\n\n");
							say(t, "\"");
							Project.changePortrait(gender, type, false, true, w, w.nameCombatants(), 3, Project.Emotion.FOCUS, Project.Emotion.JOY);
				...
				if (deviancy > 66) {
					...
					if (c.getInnocence() > 66) {
						...
						Project.changePortrait(c.convertGender(), c.type, false, false, w, w.nameCombatants(), c.combatantNumber(w), Project.Emotion.FEAR, Project.Emotion.STRUGGLE);
						if (hostility > 66) {
							c.say(t, "There's no way " + heShe() + " can keep this up forever, right?");
						} else if (hostility > 33) {
							c.say(t, "Ow!");
						} else {
							c.say(t, "Close one...!");
						...
						Project.changePortrait(c.convertGender(), c.type, false, false, w, w.nameCombatants(), c.combatantNumber(w), Project.Emotion.NEUTRAL, Project.Emotion.ANGER);
						if (innocence > 66) {
							c.say(t, "Did " + heShe() + " outmaneuver me?  No, " + heShe() + "'s being directed...");
						} else if (innocence > 33) {
							c.say(t, "Damn it.  Nothing that can be done now.");
						} else {
							c.say(t, "If I let my guard down, " + heShe() + " will exploit it ruthlessly.");
						}
						c.say(t, "\"\n\n");
						say(t, "\"");
						Project.changePortrait(gender, type, false, true, w, w.nameCombatants(), 3, Project.Emotion.NEUTRAL, Project.Emotion.FOCUS);
		...
		} else if (c.captureProgression % 6 == 5) {
			if (styleDamage[0] > 0) {
				if (styleDamage[2] > 0) {
					if (hostility > 66) {
						say(t, "\"");
						...
						if (c.getConfidence() > 66) {
							...
							if (innocence > 66) {
								...
								c.say(t, "I c-can't fight these and " + himHer() + " at once...!");
							} else {
					...
					if (hostility > 66) {
						...
						if (c.getInnocence() > 66) {
							...
							Project.changePortrait(c.convertGender(), c.type, false, false, w, w.nameCombatants(), c.combatantNumber(w), Project.Emotion.ANGER, Project.Emotion.NEUTRAL);
							if (confidence > 66) {
								c.say(t, "I didn't think " + heShe() + "'d hide...");
							} else if (confidence > 33) {
						...
						if (c.getConfidence() > 66) {
							...
							if (confidence > 66) {
								...
								c.say(t, "I'm trying to ignore " + himHer() + ", but...");
							} else {
								c.say(t, "I won't let " + himHer() + "...!");
							}
						}
						c.say(t, "\"");
					} else {
	...
	public void captureChosenFlavor(JTextPane t, WorldState w, Chosen c, int[] styleDamage, String bottomDesc, String topDesc, String organ, String lowerOrgan) {
		if (c.captureProgression % 6 == 0) {
			if (styleDamage[0] > 0) {
				if (styleDamage[1] > 0) {
					if (deviancy > 66) {
						if (c.innocence > 66) {
							append(w, t, c, "{mainName} tackles " + c.mainName + " from behind, releasing {hisHer} overflowing lust on " + reference(c) + ".  {c:mainName} is momentarily paralyzed by the sensation of {mainName}'s body grinding against {c:himHer}");
							if (innocence > 66) {
								append(w, t, ", helpless against {mainName}'s singleminded lust.");
							} else if (innocence > 33) {
								append(w, t, c, ", and by the time {c:heShe} recovers, {mainName} has pinned {c:himHer} down.");
							} else {
								append(w, t, c, ", exactly as {mainName} knew {c:heShe} would be.");
							}
						} else if (c.innocence > 33) {
							if (innocence > 66) {
							...
							append(w, t, c, "giving {himHer} an easy opportunity to grab " + reference(c) + " from behind and begin grinding against {c:himHer} with wild abandon.");
						} else {
							...
							if (innocence > 66) {
								append(w, t, "barely even noticing the pain as {heShe} ");
							} else if (innocence > 33) {
								append(w, t, "getting turned on by the pain even as {heShe} ");
							} else {
								append(w, t, "refusing to be distracted from {hisHer} lustful objectives as {heShe} ");
							}
							append(w, t, c, "slams into {c:mainName} and begins humping {c:himHer} wildly.");
						}
					} else if (deviancy > 33) {
						if (c.innocence > 66) {
							if (innocence > 66) {
							...
							if (innocence > 66) {
								...
								append(w, t, c, "Soon, {c:mainName}'s resistance is reduced to trying desperately to buck {mainName} off as {heShe} lustfully gropes {c:himHer} and humps {c:hisHer} thigh.");
							} else {
							...
							if (innocence > 66) {
								...
								append(w, t, c, "Then, {heShe} positions {himHer}self between {c:mainName}'s legs, smirking with feigned innocence as " + reference(c) + "'s struggles stimulate them both.");
							} else {
								append(w, t, c, "This would normally be a poor strategy against one of the Chosen, but {mainName} has cunningly positioned their crotches against each other, and {c:mainName} has trouble keeping {c:hisHer} composure as {c:heShe} unavoidably stimulates {c:himHer}self as {c:heShe} tries to escape.");
							}
						}
					} else {
						if (c.innocence > 66) {
							if (innocence > 66) {
								append(w, t, c, "Giggling madly, {mainName} ambushes {c:mainName} from behind, ");
							} else if (innocence > 33) {
								append(w, t, c, "{c:name} is caught completely by surprise when {mainName} rushes forward and grabs {c:himHer}, ");
							} else {
								append(w, t, c, "{mainName} briefly conceals {himHer}self behind a crowd of Thralls, then springs out to grab {c:mainName}, ");
							}
							append(w, t, c, "forcing out a squeak as {c:heShe}'s roughly groped by " + c.reference(this) + ".");
						} else if (c.innocence > 33) {
							append(w, t, c, "{c:name} stops a huge Demon's attack with {c:hisHer} bare hands, but that leaves {c:himHer} completely open for ");
							if (innocence > 66) {
								append(w, t, c, "{mainName} to come up behind {c:himHer} and start running {c:hisHer} hands over " + reference(c) + "'s body before abruptly grabbing {c:hisHer} chest with both hands.");
							} else if (innocence > 33) {
								append(w, t, c, "{mainName} to embrace {c:himHer} from behind, planting a mockingly gentle kiss on {c:hisHer} lips before {hisHer} hands begin to wander all over " + reference(c) + "'s body.");
							} else {
								append(w, t, c, "{mainName} to seize the opportunity to grab {c:himHer} from behind, abruptly stimulating " + reference(c) + "'s most sensitive parts with {hisHer} hands.");
							}
						} else {
							append(w, t, c, "{c:name} and {mainName} meet in battle, but while {c:mainName} tries to fight conventionally, {mainName} ");
							if (innocence > 66) {
								append(w, t, c, "throws {c:hisHer} mind into complete disarray by embracing {c:himHer} and kissing {c:himHer} on the lips, then groping between {c:hisHer} legs before {c:heShe} can collect {c:himHer}self.");
							} else if (innocence > 33) {
								append(w, t, c, "simply throws {c:himHer} off-balance by pinching and groping {c:himHer} before falling back to defend {c:himHer}self.");
							} else {
								append(w, t, c, "is completely focused on feeling up {c:mainName}'s body, a strategy {c:heShe}'s unprepared to fight against.");
							}
						}
					}
				} else {
					if (hostility > 66) {
						if (c.getConfidence() > 66) {
							if (innocence > 66) {
								append(w, t, c, "When {c:heShe} sees {mainName}, giggling and covered in blood among the corpses of the countless fleeing civilians {heShe} decided to kill, {c:heShe} ");
							} else if (innocence > 33) {
								append(w, t, c, "{c:name} doesn't take notice of {mainName} quickly enough for the latter's liking, and so {mainName} decides to draw {c:hisHer} attention with the screams of murdered civilians.  {c:mainName} turns {c:hisHer} head, notices the slaughter, and ");
							} else {
								append(w, t, c, "{mainName} decides to draw {c:mainName}'s attention by killing several of the civilians {c:heShe} had just led to safety, and when{c:mainName} notices, {c:heShe} ");
							}
							w.append(t, "roars with rage and charges forward.");
						} else if (c.getConfidence() > 33) {
							append(w, t, "A blast of malevolent energy rips through one edge of the battlefield, tearing apart Demons, Thralls, and innocent civilians alike.  When the dust clears, {mainName} is standing in the epicenter, ");
							if (innocence > 66) {
								append(w, t, c, "wearing a bloodstained deranged grin as {heShe} stares at the nearby {c:mainName}.");
							} else if (innocence > 33) {
								append(w, t, c, "having used it to clear a path to {c:mainName}.");
							} else {
								append(w, t, c, "coldly striding through the carnage toward {c:mainName}.");
							}
						} else {
							if (innocence > 66) {
								append(w, t, c, "{mainName} walks slowly toward {c:mainName}, giggling and covered in the blood of {hisHer} civilian victims, ");
							} else if (innocence > 33) {
								append(w, t, c, "{mainName} carves a bloody swath through the civilians remaining on the battlefield as {heShe} heads directly toward {c:mainName}, ");
							} else {
								append(w, t, c, "When {mainName} sees that {c:mainName} is looking at {himHer}, {heShe} grabs a random fleeing civilian and kills him in front of " + reference(c) + "'s eyes, ");
							}
							append(w, t, c, "and it's all {c:mainName} can do to gasp and back away in horror.");
						}
					} else if (hostility > 33) {
						if (c.getConfidence() > 66) {
							...
							if (innocence > 66) {
								...
								append(w, t, c, "{mainName} refuses to engage {c:himHer}, preferring to stay just close enough to shout insults at {c:himHer}.");
							} else {
								append(w, t, c, "{mainName} has carefully positioned {himHer}self so that {heShe} has an easy escape route, and {heShe} leads {c:mainName} on a chase, cruelly insulting {c:himHer} as {heShe} does so.");
							}
						} else if (c.getConfidence() > 33) {
							append(w, t, c, "{mainName} perches on a rooftop near where {c:mainName} is fighting, shouting out unhelpful advice and demoralizing insults.  When {c:mainName} takes the bait and starts trying to give chase, ");
							if (innocence > 66) {
								append(w, t, c, "{mainName} laughs and dashes off, continuing to taunt {c:himHer} as {heShe} runs.");
							} else if (innocence > 33) {
							...
							if (innocence > 66) {
								...
								append(w, t, c, "forces {c:mainName} to shy away with a barrage of cruel words, giving {c:himHer} no respite.");
							} else {
								append(w, t, c, "opts to strike at {c:mainName}'s morale instead, making cutting remarks carefully picked to target " + reference(c) + "'s insecurities.");
							}
						}
					} else {
						if (c.getConfidence() > 66) {
							...
							if (disgrace < 34) {
								...
								append(w, t, c, "{mainName} fires an enormous burst of dark energy from {hisHer} " + weapon + ", but it dissipates the instant before it reaches {c:mainName}, revealing {c:hisHer} pitifully-flinching form.  ");
							} else {
								append(w, t, c, "{mainName} and {c:mainName} briefly meet in battle, exchanging several attacks.  However, {c:mainName}'s heart clearly isn't in it, and the two soon separate to face each other down.  ");
							}
				...
				if (deviancy > 66) {
					if (c.innocence > 66) {
						append(w, t, c, "{mainName} tackles " + c.mainName + " from behind, releasing {hisHer} overflowing lust on " + reference(c) + ".  {c:mainName} is momentarily paralyzed by the sensation of {mainName}'s skillful hands seeming to pinch and stroke a dozen places at once");
						if (innocence > 66) {
							append(w, t, ", helpless against {mainName}'s singleminded lust.");
						} else if (innocence > 33) {
							append(w, t, c, ", and by the time {c:heShe} recovers, {mainName} has pinned {c:himHer} down.");
						...
						append(w, t, c, "giving {himHer} an easy opportunity to grab " + reference(c) + " from behind and begin groping {c:himHer} with wild abandon.");
					} else {
						w.append(t, c.mainName + " catches sight of " + mainName + " charging at " + c.himHer() + ", but although " + c.heShe() + " launches an attack to fend the Forsaken off, " + mainName + " takes it head-on, ");
						if (innocence > 66) {
							append(w, t, "barely even noticing the pain as {heShe} ");
						...
						append(w, t, c, "slams into {c:mainName} and begins feverishly molesting {c:himHer}.");
					}
				} else if (deviancy > 33) {
					if (c.innocence > 66) {
						if (innocence > 66) {
						...
						if (innocence > 66) {
							...
							append(w, t, c, "Soon, {c:mainName}'s resistance is reduced to trying desperately to buck {mainName} off as {heShe} forcefully gropes {c:hisHer} chest and rubs between {c:hisHer} legs.");
						} else {
						...
						if (innocence > 66) {
							...
							append(w, t, c, "Then, {heShe} slowly pries {c:mainName}'s legs apart, and uses {hisHer} foot to stimulate " + reference(c) + ".");
						} else {
							append(w, t, c, "This would normally be a poor strategy against one of the Chosen, but {mainName} has cunningly positioned {c:mainName}'s crotch against the corner of a piece of rubble, and {c:heShe} has trouble keeping {c:hisHer} composure as {c:heShe} unavoidably stimulates {c:himHer}self as {c:heShe} tries to escape.");
						}
					}
				} else {
					if (c.innocence > 66) {
				...
				if (disgrace > 66) {
					if (c.getConfidence() > 66) {
						...
						if (confidence > 66) {
							...
							append(w, t, c, "and although {c:mainName} has the clear upper hand, it still distracts {c:himHer} from the Demons and forces {c:himHer} to spend {c:hisHer} valuable energy.");
						} else {
						...
						if (confidence > 66) {
							...
							w.append(t, "then falls back into a defensive stance when " + reference(c) + " responds.");
						} else {
						...
						if (confidence > 66) {
							...
							append(w, t, c, "and although {mainName} isn't very strong for one of the Forsaken, {c:mainName}'s lack of self-confidence causes {c:himHer} to fall back.");
						} else {
					...
					if (c.getConfidence() > 66) {
						...
						if (confidence > 66) {
							...
							append(w, t, "and {mainName} takes a defensive stance as " + reference(c) + " charges toward {himHer}.");
						} else {
						...
						if (confidence > 66) {
							...
							append(w, t, "{mainName} fights with a ferocity far beyond what {hisHer} timid demeanor would suggest.");
						}
					} else {
						append(w, t, c, "A nearby blast of dark psychic energy startles {c:mainName}, and {c:heShe}'s retreating before {c:heShe} realizes it, ");
						if (confidence > 66) {
							append(w, t, "slowly pursued by {mainName}'s menacing form.");
						} else if (confidence > 33) {
							append(w, t, "intimidated by {mainName}'s show of confidence.");
						} else {
							append(w, t, "much to the relief of {mainName}, who hoped to use intimidation to avoid an all-out fight.");
						}
					}
				} else {
					if (c.getConfidence() > 66) {
						...
						append(w, t, c, "{mainName} charges at {c:mainName}, smashing apart any buildings and vehicles in {hisHer} path.  Although {c:mainName} tries to stop {himHer}, ");
						if (confidence > 66) {
							append(w, t, c, "{mainName} smacks aside {c:hisHer} attacks with contemptuous ease.");
						} else if (confidence > 33) {
							append(w, t, c, "{mainName} takes {c:hisHer} attacks head-on without any visible reaction.");
						} else {
							append(w, t, "{mainName}'s frantic strength is unstoppable.");
						}
					} else {
						append(w, t, c, "The Demons switch tactics, forming a wall around {c:mainName} so that {c:heShe}'s trapped in the same area as {mainName}, unable to escape or get help.  {c:HeShe} quails as {c:heShe} sees " + mainName);
						if (confidence > 66) {
							w.append(t, "'s merciless expression.");
						} else if (confidence > 33) {
							w.append(t, " slowly advancing forward, pulsing with dark energy.");
						} else {
							append(w, t, c, "'s wide eyes fixed on {c:himHer}.");
						}
					}
				}
			}
			...
			if (styleDamage[1] > 0) {
				if (styleDamage[2] > 0) {
					if (disgrace > 66) {
						if (c.getConfidence() > 66) {
							append(w, t, c, "Even though {c:mainName} is far stronger than {mainName}, " + c.reference(this));
							if (innocence > 66) {
								append(w, t, c, "'s frantic efforts to get at {c:hisHer} " + lowerOrgan + " even as {c:heShe} tries to fight {himHer} off proves difficult to deal with.");
							} else if (innocence > 33) {
								append(w, t, c, " fights dirty, kicking {c:himHer} in the " + lowerOrgan + " and then molesting {c:himHer} for several moments before before forced back again.");
							} else {
								append(w, t, c, " makes cunning use of {c:mainName}'s reluctance to be stimulated against {c:hisHer} will, groping {c:hisHer} " + lowerOrgan + " and then taking advantage of {c:hisHer} surprise to press the advantage.");
							}
						} else if (c.getConfidence() > 33) {
							...
							if (deviancy > 66) {
								...
								append(w, t, "{mainName} won't be dissuaded");
							}
							append(w, t, c, ", and soon {c:mainName} is blushing and gasping from the repeated groping.");
						} else {
							append(w, t, c, "Despite {mainName}'s relative weakness, {c:mainName} lacks the willpower to pull free of " + c.reference(this) + "'s grip on {c:hisHer} " + lowerOrgan);
							if (innocence > 66) {
								append(w, t, ", helpless against the enthusiastic movements of {hisHer} fingers.");
							} else if (innocence > 33) {
								w.append(t, ".");
							} else {
								append(w, t, ", unused to being stimulated by someone as skillful as {mainName}.");
							}
						}
					} else if (disgrace > 33) {
						if (c.getConfidence() > 66) {
							append(w, t, c, "In a straight fistfight, {c:mainName} would easily beat {mainName}, but ");
							if (innocence > 66) {
								append(w, t, c, "{mainName} is unwittingly exploiting " + reference(c) + "'s reluctance to acknowledge unwilling pleasure by focusing more on groping {c:himHer} than on actually fighting.");
							} else if (innocence > 33) {
								append(w, t, c, "{mainName} is less interested in bruising " + reference(c) + " and more interested in feeling {c:himHer} up.  It proves difficult to stop.");
							} else {
								append(w, t, c, "{mainName} is well aware of that, and {heShe} refuses to have a fair fight, instead grabbing {c:mainName}'s " + lowerOrgan + " through {c:hisHer} " + bottomDesc + " and roughly tugging it whenever " + reference(c) + " tries to overpower {himHer}.");
							}
						} else if (c.getConfidence() > 33) {
							...
							if (hostility > 66) {
								...
								append(w, t, c, "but while {c:mainName} only knows how to fight conventionally, {mainName} is ruthless about kneeing and grinding against {c:mainName}'s sensitive " + organ + ".");
							} else {
								append(w, t, "but {mainName}'s surprisingly gentle hands on " + reference(c) + "'s " + organ + " and " + lowerOrgan + " make it hard to focus on the fight.");
							}
						} else {
							if (deviancy > 66) {
								w.append(t, "Driven wild by " + reference(c) + "'s meek, vulnerable manner, ");
							} else if (deviancy > 33) {
						...
						if (c.getConfidence() > 66) {
							append(w, t, c, "{c:name} struggles as hard as {c:heShe} can, but {mainName}'s iron grip holds {c:himHer} in place");
							if (innocence > 66) {
								w.append(t, ", " + c.reference(this) + "'s overly enthusiastic stimulation causing as much discomfort as pleasure.");
							} else if (innocence > 33) {
								append(w, t, c, " so that {c:heShe} can't escape the rough rubbing against {c:hisHer} " + organ + ".");
							} else {
								w.append(t, ", and every time escape seems imminent, ");
								if (w.tickle()) {
									append(w, t, c, "{mainName}'s fingers against {c:hisHer} armpits cause {c:himHer} to jerk and spasm, losing leverage.");
								} else {
									append(w, t, c, "{mainName} harshly twists {c:hisHer} limbs to keep {c:himHer} from getting away.");
								}
							}
						} else if (c.getConfidence() > 33) {
							append(w, t, c, "{mainName} slams {c:mainName} halfway through a concrete wall so that {c:hisHer} butt is hanging out, then rips open {c:mainName}'s " + bottomDesc);
							if (deviancy > 66) {
								append(w, t, c, " and starts eagerly licking {c:hisHer} " + organ + ".");
							} else if (deviancy > 33) {
								append(w, t, c, ", plants a foot on {c:hisHer} organ, and starts moving it back and forth.");
							} else {
								append(w, t, c, " and uses one hand to firmly stimulate {c:hisHer} " + organ + ".");
							}
						} else {
							...
							if (hostility > 66) {
								...
								append(w, t, c, "looking down on {c:himHer} contemptfully");
							} else {
								append(w, t, "shaking {hisHer} head with disappointment");
							}
							append(w, t, c, " when {c:mainName} can only squirm and whine in response.");
						}
					}
				} else {
					if (deviancy > 66) {
						if (c.getConfidence() > 66) {
							append(w, t, c, "Overwhelmed by the sensation of {mainName}'s hands all over {c:hisHer} body, {c:mainName} finally manages to deliver a mighty wallop to " + c.reference(this) + "'s head.  {c:mainName} manages to escape and get some distance, ");
							if (hostility > 66) {
								append(w, t, "but the murderous sadism in {mainName}'s eyes as {heShe} recovers promises even worse torment to come.");
							} else if (hostility > 33) {
								append(w, t, c, "but {mainName} wastes no time in immediately starting to pursue {c:himHer} again.");
							} else {
								append(w, t, "and {mainName} even briefly looks apologetic as {heShe} recovers, but then {hisHer} expression again melts into mindless lust.");
							}
							
						} else if (c.getConfidence() > 33) {
							append(w, t, "{mainName} ");
							if (c.gender.equals("male")) {
								append(w, t, c, "runs {hisHer} hands all over {c:mainName}'s chest");
							} else {
								append(w, t, c, "eagerly gropes {c:mainName}'s breasts");
							}
							append(w, t, c, ", pushing {c:himHer} down onto {c:hisHer} knees in order to molest {c:himHer} from a better angle, but ");
							if (innocence > 66) {
								w.append(t, "this proves too greedy, and " + reference(c) + " manages to use the opportunity to escape.");
							} else if (innocence > 33) {
								append(w, t, c, "while it's more satisfying to hold " + reference(c) + " like this, it's also more difficult, and {c:mainName} manages to buck {himHer} off and clamber to {hisHer} feet.");
							} else {
								append(w, t, c, "{heShe} gets distracted by the view of " + reference(c) + "'s ass and allows {c:mainName} to crawl away and escape.");
							}
						} else {
							if (innocence > 66) {
								append(w, t, c, "{c:name}'s squirming attempts to escape {mainName} are inadvertently causing {c:himHer} to grind against {mainName} in turn, and {c:heShe} doesn't even realize that ");
							} else if (innocence > 33) {
								append(w, t, c, "{c:name} notices that {mainName}'s groping grows weaker when {c:heShe} knees the Forsaken in a particular spot, but {c:heShe} only belatedly realizes that ");
							} else {
								append(w, t, c, "The grinding of {mainName}'s body against {c:mainName}'s grows more and more frantic, and {c:mainName} realizes that {c:heShe} will have an easy opportunity to escape while ");
							}
						...
						if (c.getConfidence() > 66) {
							append(w, t, c, "{c:name} starts lashing out with superhuman strength, and {mainName} stops molesting {c:himHer} and backs off");
							if (disgrace > 66) {
								append(w, t, ", knowing that taking a direct hit could end {hisHer} playtime with " + reference(c) + " sooner than {heShe}'d prefer.");
							} else if (disgrace > 33) {
								w.append(t, " reluctantly.");
							} else {
								append(w, t, c, ", happily letting {c:mainName} exhaust {c:himHer}self.");
							}
						} else if (c.getConfidence() > 33) {
							append(w, t, c, "{c:name} finally manages to disentangle {c:himHer}self from {mainName}'s lustful hands and shove " + c.reference(this) + " away");
							if (disgrace > 66) {
								append(w, t, ", and while {mainName} is far from satisfied, {heShe} knows {heShe} needs to pace {himHer}self if {heShe} wants to continue enjoying {hisHer} stronger opponent's body.");
							} else if (disgrace > 33) {
								append(w, t, c, ".  {mainName} immediately tries to grab {c:himHer} again, but {c:mainName} is just barely able to avoid {mainName}'s eager groping.");
							} else {
								append(w, t, c, ", but {mainName} is only allowing {c:himHer} to think {c:heShe} has any control over the situation in order to crush {c:hisHer} spirit later on.");
							}
						} else {
						...
						if (c.getDignity() > 66) {
							...
							if (disgrace > 66) {
								...
								append(w, t, c, "{c:name} takes a breath to compose {c:himHer}self, then prepares to fight again.");
							} else {
								append(w, t, c, "{c:name} is privately relieved that {c:heShe} can pretend not to be feeling any pleasure.");
							}
						} else if (c.getDignity() > 33) {
							append(w, t, c, "Made desperate by the quickly mounting pleasure of {mainName}'s hand on {c:hisHer} " + organ + ", {c:mainName} swings a blind punch at " + c.reference(this) + ", but ");
							if (innocence > 66) {
							...
							if (innocence > 66) {
								append(w, t, ", causing {mainName} to reflexively let go and back off, wiping {hisHer} palm on {hisHer} clothes.");
							} else if (innocence > 33) {
								append(w, t, ", but {mainName} refuses to go any further, uncomfortable with what {heShe}'s doing, and backs away to fighting distance.");
							} else {
								append(w, t, ", and {mainName} backs off and readies {hisHer} " + weapon + " with a slight expression of disgust on {hisHer} face.");
							}
						} 
					}
				}
				...
				if (disgrace > 66) {
					if (c.getInnocence() > 66) {
						if (confidence > 66) {
							append(w, t, "Refusing to acknowledge {hisHer} own weakness, ");
						} else if (confidence > 33) {
						...
						if (w.tickle()) {
							append(w, t, c, "and lunging forward to start tickling {c:himHer}.");
						} else {
							append(w, t, "and landing several solid hits with {hisHer} " + weapon + ".");
						}
					} else if (c.getInnocence() > 33) {
						if (dignity > 66) {
							append(w, t, c, "{mainName} forces {himHer}self to ignore {c:mainName}'s attacks, and ");
						} else if (dignity > 33) {
							append(w, t, c, "{mainName} charges right through a flurry of attacks from {c:mainName}'s " + weapon + ", and ");
						} else {
							append(w, t, c, "{mainName} cries out in pain as {heShe}'s struck by {c:mainName}'s " + weapon + ", but ");
						}
						append(w, t, c, "{heShe} soon gets close enough to start grappling with {c:himHer} directly");
						if (w.tickle()) {
							w.append(t, ", trying to get some tickling in");
						} else {
							append(w, t, ", trying to deal some damage with {hisHer} bare hands");
						}
						...
						if (w.tickle()) {
							append(w, t, c, "trying to tickle {c:himHer}");
						} else {
							append(w, t, "attacking with {hisHer} weapon");
						}
						w.append(t, ", ");
						if (innocence > 66) {
							append(w, t, "figuring that a situation where neither of them has time to think will work out in {hisHer} favor.");
						} else if (innocence > 33) {
							append(w, t, "knowing that {heShe} doesn't have any chance of victory if " + reference(c) + " is given any opportunity to use {hisHer} formidable strategic abilities.");
						} else {
							w.append(t, "planning around the expectation that " + reference(c) + " is not as experienced at fighting under this kind of pressure.");
						}
					}
				} else if (disgrace > 33) {
					if (c.getMorality() > 66) {
						append(w, t, c, "{c:name} tries to draw {mainName} away to an area further from any civilians, ");
						if (hostility > 66) {
							append(w, t, c, "but {mainName} just grins and starts heading straight for the evacuation route, forcing {c:mainName} to tackle {himHer} and open {himHer}self up to a punishing counterattack.");
						} else if (hostility > 33) {
							append(w, t, "and {mainName} takes advantage by attacking " + reference(c) + " from behind with {hisHer} " + weapon + ".");
						} else {
							append(w, t, c, "and {mainName} is content to oblige, soon catching up to {c:himHer} and engaging {c:himHer} in a close-range grapple.");
						}
					} else if (c.getMorality() > 33) {
						...
						if (innocence > 66) {
							...
							append(w, t, " as {mainName} skillfully deflects " + reference(c) + "'s attacks before responding with {hisHer} own.");
						}
					} else {
						append(w, t, c, "{c:name} charges directly at {mainName}, intent on striking {himHer} down first, ");
						if (confidence > 66) {
							append(w, t, c, "and {mainName} is happy to meet {c:himHer} head-on, {hisHer} " + weapon + " at the ready.");
						} else if (confidence > 33) {
							append(w, t, "but {mainName} refuses to be intimidated, and {heShe} even takes the chance to land a quick attack with {hisHer} " + weapon + ".");
						} else {
							append(w, t, c, "and while {mainName} initially flees, {heShe} fights ferociously once cornered, rounding on {c:mainName} to ");
							if (w.tickle()) {
								append(w, t, c, "desperately try to tickle {c:himHer}.");
							} else {
								append(w, t, c, "kick, punch, and bite {c:himHer}.");
							}
						}
					}
				} else {
					if (c.getInnocence() > 66) {
						append(w, t, c, "{c:name} launches an all-out attack at {mainName}, ");
						if (dignity > 66) {
							append(w, t, c, "but {mainName} is able to pretend that it didn't hurt at all, walking calmly up to " + reference(c) + " to grab {c:himHer} by the throat");
						} else if (dignity > 33) {
							append(w, t, c, "but it isn't enough to stop {himHer} from charging forward to grab {c:mainName}");
						} else {
							append(w, t, c, "but it just annoys {mainName}, who grits {hisHer} teeth and rushes forward to tackle {c:himHer}");
						}
						if (w.tickle()) {
							append(w, t, c, " and start mercilessly tickling {c:himHer}.");
						} else {
							append(w, t, c, " and begin crushing the life out of {c:himHer}.");
						}
					} else if (c.getInnocence() > 33) {
						if (confidence > 66) {
							append(w, t, "Intimidated by {mainName}'s clear confidence");
						} else if (confidence > 33) {
							append(w, t, c, "Wanting {c:hisHer} team's help for this fight");
						} else {
							append(w, t, "Hoping to take advantage of {mainName}'s hesitation");
						}
						append(w, t, c, ", {c:mainName} turns to flee, but {mainName} is fast enough to immediately catch up and slam {c:himHer} down into the concrete so hard that it leaves a crater.");
					} else {
						if (hostility > 66) {
							append(w, t, "{mainName} fires an overwhelming blast of energy at a building with some civilians still inside");
						} else if (hostility > 33) {
							append(w, t, "{mainName} fires indiscriminate blasts of energy at the surrounding buildings");
						} else {
							append(w, t, "{mainName} fires a few blasts of energy at nearby abandoned buildings");
						}
						append(w, t, c, " in order to block {c:mainName}'s only possible escape routes.  {c:mainName} tries to think of another way out, but before {c:heShe} can do anything, {mainName} corners {c:himHer} ");
						if (w.tickle()) {
							append(w, t, c, "and begins to tickle {c:himHer} with overwhelming strength.");
						} else {
							append(w, t, c, "and begins to ruthlessly pummel {c:hisHer} body.");
						}
					}
				}
			} else {
				if (disgrace > 66) {
					if (c.getConfidence() > 66) {
						append(w, t, c, "{c:name} chases after {mainName}, launching attack after attack with {c:hisHer} " + c.weapon + ", but ");
						if (innocence > 66) {
							append(w, t, "{mainName} keeps just barely managing to scramble away.");
						} else if (innocence > 33) {
							append(w, t, "{mainName} flees like {hisHer} life depends on it, hoping to tire " + reference(c) + " out.");
						} else {
							append(w, t, "{mainName} knows {heShe} stands no chance in a direct fight, so {heShe} makes sure that " + reference(c) + " doesn't get any opportunity to actually catch {himHer}.");
						}
					} else if (c.getConfidence() > 33) {
						append(w, t, "{mainName} fights defensively, not even using {hisHer} " + weapon);
						if (hostility > 66) {
							append(w, t, c, ", laughing and cruelly taunting {c:mainName} in lieu of actually trying to attack.");
						} else if (hostility > 33) {
							append(w, t, c, ", smirking with satisfaction at being able to hold {hisHer} ground against the more powerful {c:mainName}.");
						} else {
						...
						append(w, t, c, "{mainName} and {c:mainName} have a tense standoff.  {c:mainName} lacks the confidence to actively attack one of the Forsaken, while " + mainName);
						if (morality > 66) {
					...
					if (c.getMorality() > 66) {
						...
						append(w, t, c, "avoid taking any serious hits from {c:hisHer} " + c.weapon + ".");
					}
				} else {
					if (c.getInnocence() > 66) {
						append(w, t, c, "{c:name} cries out with futile effort as {c:heShe} launches constant desperate attacks with {c:hisHer} " + weapon);
						if (morality > 66) {
							append(w, t, c, ", but {mainName} just continues to stare back at {c:himHer} impassively.");
						} else if (morality > 33) {
							append(w, t, ", but {mainName} is completely unharmed.");
						} else {
							append(w, t, c, ", but {c:heShe} only succeeds in annoying {mainName}.");
						}
					} else if (c.getInnocence() > 33) {
						if (confidence > 66) {
							append(w, t, c, "{mainName} smirks with self-satisfaction as {heShe} weathers {c:mainName}'s attacks, and ");
						} else if (confidence > 33) {
						...
						append(w, t, c, "After a few attacks, {c:mainName} recognizes the futility of trying to fight " + mainName);
						if (hostility > 66) {
							append(w, t, c, ", and {c:hisHer} blood runs cold as {c:heShe} realizes that {c:heShe}'s completely at " + c.reference(this) + "'s mercy.");
						} else if (hostility > 33) {
							append(w, t, c, ", and {c:heShe} can only back off warily, alarmed at the palpable hatred pouring off " + c.reference(this) + ".");
						} else {
							append(w, t, c, ", but {c:heShe}'s grateful for the fact that " + c.reference(this) + " doesn't seem to intend to hurt {c:himHer}.");
						}
					}
				}
			}
			...
			if (styleDamage[2] > 0) {
				if (styleDamage[3] > 0) {
					if (disgrace > 66) {
						if (c.getDignity() > 66) {
							if (innocence > 66) {
								append(w, t, c, "Even {mainName} has noticed how much {c:mainName} seems to care about {c:hisHer} image, and {heShe} ");
							} else if (innocence > 33) {
							...
							append(w, t, c, "grabs " + reference(c) + "'s " + bottomDesc + ", distracting {c:himHer} enough that {mainName} can ");
							if (w.tickle()) {
								append(w, t, c, "tickle {c:hisHer} belly.");
							} else {
								w.append(t, "get a couple of good punches in.");
							}
						} else if (c.getDignity() > 33) {
							append(w, t, c, "{mainName} is on the losing end of {hisHer} grapple with {c:mainName}, but ");
							if (innocence > 66) {
								append(w, t, "{hisHer} desperate squirming still manages to tug " + reference(c) + "'s " + bottomDesc + " further and further to the side");
							} else if (innocence > 33) {
								append(w, t, c, "{c:name}'s victory comes at the expense of {c:hisHer} " + bottomDesc + " getting torn further and further.");
							} else {
								append(w, t, "{heShe} still manages to achieve {hisHer} secondary objective of tearing at " + reference(c) + "'s " + bottomDesc);
							}
						} else {
							append(w, t, c, "{mainName} and {c:mainName} roll around on the ground together, wrestling in a very undignified manner.  {c:mainName}'s clothes are too fragile to handle this kind of fight, ");
							if (deviancy > 66) {
								append(w, t, "and the distraction of seeing more and more of " + reference(c) + "'s skin slowly exposed makes {mainName} perform even more poorly.");
							} else if (deviancy > 33) {
								w.append(t, "and " + c.reference(this) + " enjoys the view.");
							} else {
								append(w, t, "but when " + c.reference(this) + " points it out, {mainName} just makes an apathetic noise.");
							}
						}
					} else if (disgrace > 33) {
						if (c.getConfidence() > 66) {
							append(w, t, c, "The flurry of blows exchanged between {mainName} and {c:mainName} is so intense that it sends scraps of their clothes flying");
							if (deviancy > 66) {
								append(w, t, " - and {mainName}'s increasing distraction as {heShe} sees more and more of " + reference(c) + "'s bare skin eventually causes {himHer} to take a punch right in the jaw.");
							} else if (deviancy > 33) {
								append(w, t, ", much to {mainName}'s amusement.");
							} else {
							...
							if (morality > 66) {
								...
								append(w, t, ", but {mainName} wants to win, ");
							} else {
								append(w, t, ", but {mainName} loves underhanded tactics, ");
							}
							append(w, t, "so {heShe} starts using {hisHer} " + weapon + " to directly attack " + reference(c) + "'s " + bottomDesc + ".");
						} else {
							append(w, t, c, "{c:name} tries to pull away from {mainName}'s ");
							if (w.tickle()) {
								w.append(t, "tickling, ");
							} else {
							...
							if (innocence > 66) {
								append(w, t, c, "and while {mainName}'s decision to hold {c:himHer} by {c:hisHer} " + bottomDesc + " is a poor strategy, it does mean that " + reference(c) + " ends up practically tearing {c:hisHer} own clothes off in {c:hisHer} haste to escape.");
							} else if (innocence > 33) {
								append(w, t, c, "but only succeeds in tearing apart {c:hisHer} own clothes as " + c.reference(this) + " clings to every dangling scrap in order to keep {c:himHer} there.");
							} else {
								append(w, t, c, "but {mainName} grabs {c:himHer} by the " + bottomDesc + ", using " + reference(c) + "'s struggling to help tear through the durable material.");
							}
						}
					} else {
						if (c.getInnocence() > 66) {
							if (confidence > 66) {
								append(w, t, c, "Completely helpless with {mainName} brutally holding {c:himHer} down, ");
							} else if (confidence > 33) {
								append(w, t, "Held in {mainName}'s iron grip, ");
							} else {
								append(w, t, "Completely trapped by {mainName}'s surprisingly strong grip, ");
							}
							append(w, t, c, "{c:name} panics and thrashes wildly, ");
							if (w.tickle()) {
								w.append(t, "hurting");
							} else {
								w.append(t, "exhausting");
							}
							append(w, t, c, " {c:himHer}self and tearing {c:hisHer} own clothes apart with {c:hisHer} frantic motions.");
						} else if (c.getInnocence() > 33) {
							if (w.tickle()) {
							...
							if (hostility > 66) {
								...
								append(w, t, ", though {heShe} doesn't seem to actually enjoy tormenting " + reference(c) + ".");
							}
						} else {
							append(w, t, "In order to escape {mainName}'s ");
							if (w.tickle()) {
								w.append(t, "tickling");
							} else {
								w.append(t, "pummeling");
							}
							append(w, t, c, ", {c:mainName} ");
							if (deviancy > 66) {
								append(w, t, c, "abruptly kisses " + c.reference(this) + " on the lips, and then when {mainName} starts to instinctively respond, {c:mainName} ");
							} else if (deviancy > 33) {
								append(w, t, c, "starts to spread {c:hisHer} legs, and when " + c.reference(this) + " lustfully looks downward, {c:mainName} ");
							} else {
								append(w, t, "waits for " + c.reference(this) + " to let {hisHer} guard down, then ");
							}
							append(w, t, c, "starts fighting with all {c:hisHer} might.  {c:HeShe}'s close to escaping, although the frantic struggle is enough to tear {c:hisHer} clothes apart.");
						}
					}
				} else {
					if (disgrace > 66) {
						if (c.getConfidence() > 66) {
							if (morality > 66) {
								append(w, t, "No matter how determined {heShe} is, ");
							} else if (morality > 33) {
								append(w, t, "With {hisHer} power so much weaker than it used to be, ");
							} else {
							...
							if (confidence > 66) {
								append(w, t, "as much as {heShe} wants to deny it");
							} else if (confidence > 33) {
								w.append(t, "with no other option available");
							} else {
								w.append(t, "out of fear of being truly injured or even killed");
							}
							append(w, t, c, ", {heShe} retreats, giving {c:mainName} some breathing room.");
						} else {
							if (innocence > 66) {
								append(w, t, c, "{c:name} sees a chance to get some distance between {c:himHer}self and {c:hisHer} opponent, and with a carefully-placed kick, {heShe} ");
							} else if (innocence > 33) {
								append(w, t, c, "{c:name} finally summons the willpower to fight back in earnest, and {c:heShe} ");
							} else {
								append(w, t, c, "{c:name}'s fear of further abuse finally gives {c:himHer} the strength to overcome {mainName}'s combat techniques, and {c:heShe} ");
							}
							append(w, t, "pushes {mainName} away.  The impact is surprisingly strong, and {mainName} takes long enough to recover that " + reference(c) + " is able to earn a significant respite from {hisHer} ");
							if (w.tickle()) {
								w.append(t, "tickling.");
							} else {
								w.append(t, "pummeling.");
						...
						if (c.getInnocence() > 66) {
							append(w, t, c, "Between {c:mainName}'s singlemindedness and ");
							if (deviancy > 66) {
								append(w, t, "{mainName}'s overwhelming lust for " + reference(c));
							} else if (deviancy > 33) {
								append(w, t, "{mainName}'s anger");
							} else {
								append(w, t, "{mainName}'s focused determination");
							}
							...
							if (innocence > 66) {
								append(w, t, c, "{c:name} knows that {mainName}'s energy reserves will likely outlast {c:hisHer} own, ");
							} else if (innocence > 33) {
								append(w, t, c, "{c:name} knows that it wouldn't be to {c:hisHer} advantage to waste {c:hisHer} energy trying to defeat {mainName}, ");
							} else {
								append(w, t, c, "{c:name} knows that {mainName} is trying to bait {c:himHer} into exhausting {c:himHer}self, ");
							}
							append(w, t, c, "so {c:heShe} backs off, seeing whether " + c.reference(this) + " will come to {himHer}.");
						}
					} else {
						if (c.getConfidence() > 66) {
							...
							append(w, t, c, "The tussle between {mainName} and {c:mainName} was brief, but {c:mainName} is still left limping away, ");
							if (w.tickle()) {
								append(w, t, c, "gasping for breath in the wake of {c:hisHer} tickling");
							} else {
								w.append(t, "bleeding from several new wounds");
							}
							if (hostility > 66) {
								append(w, t, " while {mainName} gleefully follows, shouting promises of more suffering to come.");
							} else if (hostility > 33) {
								append(w, t, c, " while {mainName} launches random blasts of energy until one of them destroys {c:mainName}'s cover.");
							} else {
								append(w, t, c, " while {mainName} grimly tracks {c:himHer} down.");
							}
						} else {
							append(w, t, c, "{c:name} lies motionless on the pavement, overwhelmed by {mainName}'s superior power.  ");
							if (hostility > 66) {
								append(w, t, "{mainName} licks {hisHer} lips, savoring the building dread within " + reference(c) + ".");
							} else if (hostility > 33) {
								append(w, t, c, "{mainName} stands nearby and looks down on {c:himHer}, taking {hisHer} time as {heShe} decides what to do next.");
							} else {
								append(w, t, "{mainName} sighs, clearly disappointed.");
							}
						}
					}
				}
				...
				if (deviancy > 66) {
					if (c.getDignity() > 66) {
						append(w, t, c, "Overwhelmed with lust, {mainName} charges at {c:mainName} with {hisHer} hands outstretched, ");
						if (disgrace > 66) {
							append(w, t, c, "and while {heShe}'s too weak to threaten {c:mainName} directly, {heShe}'s more than capable of tearing off {c:hisHer} clothes.");
						} else if (disgrace > 33) {
							append(w, t, c, "but while {c:mainName} is prepared to fend off a direct attack, {c:heShe} isn't able to stop {himHer} from grabbing {c:hisHer} clothes and starting to tear them off.");
						} else {
							append(w, t, c, "moving with such startling speed and strength that {c:mainName}'s clothes are being torn off almost before {c:heShe} realizes it.");
						}
						append(w, t, c, "  {c:mainName} is forced to grab " + c.reference(this) + " and try to wrestle {himHer} down just to stop {c:himHer}self from being stripped naked.");
					} else if (c.getDignity() > 33) {
						if (disgrace > 66) {
						...
						append(w, t, c, "as {hisHer} hit-and-run attacks against " + reference(c) + "'s clothes start to expose more and more skin, {hisHer} lust drives {himHer} to fight harder and harder, and {heShe} eventually ends up pouncing on {c:mainName} and tackling {c:himHer} to the ground.");
					} else {
						if (disgrace > 66) {
							append(w, t, c, "{mainName} has slipped away to ambush {c:mainName} again, and {c:mainName} knows that it would be bad for {c:himHer} to have to look over {c:hisHer} shoulder for a possible ambush for the rest of the battle.  ");
						} else if (disgrace > 33) {
							append(w, t, c, "{c:name} wants to decisively beat {mainName}, but {mainName} has been too focused on defending {himHer}self.  ");
						} else {
							append(w, t, c, "{c:name} reaches a good high ground position for seizing the advantage over the stronger {mainName}, but {mainName} isn't foolish enough to attack {c:himHer} there.  ");
						}
						append(w, t, c, "So, {c:heShe} decides to provoke " + c.reference(this) + " into attacking by acting like {c:heShe}'s pulling off {c:hisHer} own " + bottomDesc + ".  {mainName} is too blinded by lust to resist lunging forward to finish the job.");
					}
				} else if (deviancy > 33) {
					if (c.getDignity() > 66) {
						if (disgrace > 66) {
							append(w, t, c, "{c:name} isn't too worried about being hurt by {mainName}'s weak ranged attacks");
						} else if (disgrace > 33) {
							append(w, t, c, "{c:name} is feeling pretty good about {c:hisHer} performance as {c:heShe} duels {mainName} at range");
						} else {
							append(w, t, c, "{c:name} keeps having close calls with {mainName}'s powerful ranged attacks just barely missing {c:himHer}");
						}
						append(w, t, c, ", blasts of energy flying back and forth across the rooftops.  But then {c:heShe} realizes that " + c.reference(this) + " is deliberately trying to blast off {c:hisHer} clothes, and the only way to stop {himHer} is to close the distance and try to bring {himHer} down before the spectators notice that {c:heShe}'s being toyed with.");
					} else if (c.getDignity() > 33) {
						if (innocence > 66) {
							append(w, t, "{mainName} subconsciously ends up letting {hisHer} perversions get carried away");
						} else if (innocence > 33) {
							append(w, t, "{mainName} isn't interested in physically injuring {hisHer} opponent");
						} else {
							append(w, t, "{mainName} knows that tarnishing {hisHer} opponent's image will be more effective in the long term");
						}
						...
						if (flavorObedience() > 66) {
							...
							append(w, t, "in accordance with {hisHer} orders");
						} else {
							append(w, t, "in order to satisfy {hisHer} perversions");
						}
						append(w, t, c, ", {heShe} gets in close, grabs " + reference(c) + ", and starts trying to tear off as much of {c:hisHer} " + bottomDesc + " as possible before {heShe}'s stopped.");
					}
				} else {
					if (c.getConfidence() > 66) {
						if (disgrace > 66) {
						...
						append(w, t, c, "tear {c:mainName}'s clothes apart.");
					} else if (c.getConfidence() > 33) {
						append(w, t, c, "The fighting so far has kicked up a cloud of smoke, and {mainName} finds {himHer}self directly behind {c:mainName}, in the perfect position for a sneak attack.  ");
						if (hostility > 66) {
							append(w, t, c, "Knowing that {c:heShe}'ll suffer more if {c:hisHer} public image is destroyed, ");
						} else if (hostility > 33) {
							append(w, t, c, "More interested in humiliating {c:himHer} than in actually wounding {c:himHer}, ");
						} else {
							append(w, t, c, "Wanting to hinder {c:himHer} without wounding {c:himHer}, ");
						}
						append(w, t, "{mainName} starts tearing at " + reference(c) + "'s clothes with {hisHer} bare hands.");
					} else {
						append(w, t, c, "{c:name} falters and starts to flee, questioning whether {c:heShe} can really fight one of the Forsaken.  ");
						if (hostility > 66) {
							append(w, t, c, "{mainName} allows {c:himHer} to think {c:heShe}'s escaped before abruptly dropping down from a rooftop, pinning {c:himHer} to the ground and tearing at {c:hisHer} clothes in hopes of adding to the humiliation.");
						} else if (hostility > 33) {
							append(w, t, c, "{mainName} enjoys running {c:himHer} down, chasing {c:himHer} with blasts of energy which fry {c:hisHer} clothes before finally tackling {c:himHer} to the ground.");
						} else {
							append(w, t, c, "{mainName} tries to provoke {c:himHer} into fighting back by firing several warning blasts of energy at {c:hisHer} clothes, then finally sighs and starts chasing {c:himHer}, eventually managing to grab {c:himHer} by the wrist.");
						}
					}
				}
			} else {
				if (hostility > 66) {
					if (c.getConfidence() > 66) {
						if (flavorObedience() > 66) {
							append(w, t, c, "Furious over {c:mainName}'s continued defiance against the Demon Lord, ");
						} else if (flavorObedience() > 33) {
							w.append(t, "Full of directionless bloodlust, ");
						} else {
							append(w, t, c, "Annoyed about the fact that {c:mainName} is still standing up against {himHer}, ");
						}
						append(w, t, "{mainName} shouts insults at " + reference(c) + " from afar.  {HeShe} can't think about anything other than venting {hisHer} anger.");
					} else if (c.getConfidence() > 33) {
						if (disgrace > 66) {
							append(w, t, c, "{mainName} keeps getting beaten back every time {heShe} tries to press the attack against {c:mainName}, so {heShe} contents {himHer}self by ");
						} else if (disgrace > 33) {
						...
						append(w, t, "screaming barely-coherent threats as {heShe} pumps {himHer}self up for more fighting.");
					} else {
						append(w, t, c, "{c:name} backs away warily from {mainName}, who stalks after {c:himHer} and ");
						if (deviancy > 66) {
							append(w, t, "gibbers madly about all the sexual things {heShe} wants to do with " + reference(c) + ".");
						} else if (deviancy > 33) {
							append(w, t, "licks {hisHer} lips as {heShe} fantasizes about making " + reference(c) + " suffer.");
						} else {
							append(w, t, c, "grimly promises to kill {c:himHer} along with everyone {c:heShe} cares about.");
						}
					}
				} else if (hostility > 33) {
					if (c.getInnocence() > 66) {
						append(w, t, c, "As {mainName} and {c:mainName} circle each other and look for an opening, ");
						if (innocence > 66) {
							append(w, t, "{mainName} hurls childish insults at " + reference(c) + ", who is quickly goaded into responding in kind.");
						} else if (innocence > 33) {
							append(w, t, c, "{mainName} verbally abuses " + reference(c) + ", trying to unnerve {c:himHer} while also satisfying {hisHer} own sadism.");
						} else {
							append(w, t, "{mainName} whispers vicious, cruel insults selected to stab into " + reference(c) + "'s impressionable mind.");
						}
					} else if (c.getInnocence() > 33) {
						if (disgrace > 66) {
							append(w, t, c, "{mainName} knows that {heShe} will have a hard time against {c:mainName} in direct combat, so {heShe} ");
						} else if (disgrace > 33) {
							append(w, t, c, "Daring {c:mainName} to approach and put {c:himHer}self at a disadvantage, {mainName} ");
						} else {
							append(w, t, c, "Wanting to emphasize {c:mainName}'s weakness and inability to go on the offensive, {mainName} ");
						}
						w.append(t, "taunts " + reference(c) + " from a distance.");
					} else {
					...
					if (c.getMorality() > 66) {
						append(w, t, c, "{c:name} calls out after {mainName}, trying to convince {himHer} to stop fighting for the Demons");
						if (flavorObedience() > 66) {
							append(w, t, ", but {mainName}'s zealous reply leaves no doubt as to {hisHer} loyalty.");
						} else if (flavorObedience() > 33) {
							append(w, t, ".  It's enough to make {mainName} hesitate, but {heShe} still shakes {hisHer} head.");
						} else {
							append(w, t, ", but {mainName} can only reply with a sad, slightly twisted smile.");
						}
					} else if (c.getMorality() > 33) {
						if (disgrace > 66) {
						...
						append(w, t, "raises {hisHer} voice, questioning " + reference(c) + "'s reason for fighting.");
					} else {
						if (innocence > 66) {
							append(w, t, "Annoyed by {mainName}'s superficially innocent demeanor, ");
						} else if (innocence > 33) {
							append(w, t, c, "Angry and disturbed by the way that {mainName} doesn't even seem hostile toward {c:himHer}, ");
						} else {
							append(w, t, "Frustrated by {mainName}'s cunning tactics, ");
						}
						append(w, t, c, "{c:name} hurls verbal abuse at " + c.reference(this) + ", trying to hurt {himHer} emotionally in retaliation.");
					}
				}
			}
		} else if (c.captureProgression % 6 == 3) {
			if (styleDamage[1] > 0) {
				if (styleDamage[3] > 0) {
					if (disgrace > 66) {
						if (c.getDignity() > 66) {
							if (hostility > 66) {
								append(w, t, c, "Eager to humiliate {c:mainName}, ");
							} else if (hostility > 33) {
								append(w, t, c, "Exploiting {c:mainName}'s reliance on {c:hisHer} public image, ");
							} else {
								append(w, t, c, "Wanting to ensure that {c:mainName} will follow {himHer}, ");
							}
							append(w, t, c, "{mainName} disengages from their scuffle and starts running past spectators, shouting about how " + reference(c) + "'s body was reacting during its molestation.  {c:mainName} desperately gives chase, trying to stop {himHer}.");
						} else if (c.getDignity() > 33) {
							append(w, t, c, "{c:name} has {mainName} pinned and almost helpless, ");
							if (deviancy > 66) {
								append(w, t, "but the way {mainName} eagerly bucks {hisHer} hips and coaxes " + reference(c) + " to take {himHer} ");
							} else if (deviancy > 33) {
								append(w, t, c, "but {mainName}'s lewd squirming against {c:hisHer} body ");
							} else {
								append(w, t, "but {mainName}'s deliberate squirming to repeatedly put pressure against " + reference(c) + "'s " + organ + " ");
							}
							append(w, t, c, "causes {c:mainName} to gasp with pleasure and let {c:himHer} go, much to the confusion and suspicion of the spectators.");
						} else {
							append(w, t, c, "Every time {mainName} and {c:mainName} clash, {c:mainName} gets the better of the exchange, but {c:heShe}'s bad at hiding the pleasure " + c.reference(this) + "'s groping is inflicting on {c:himHer}, and ");
							if (flavorObedience() > 66) {
						...
						if (c.getConfidence() > 66) {
							append(w, t, c, "{c:name} slams {mainName} hard enough to send {himHer} tumbling down the street, but ");
							if (deviancy > 66) {
								append(w, t, "{mainName} very obviously considered it more than worthwhile for the chance to feel up " + reference(c) + ".");
							} else if (deviancy > 33) {
								append(w, t, c, "{mainName} still got the chance to grope {c:himHer} - and to announce it to the spectators.");
							} else {
								append(w, t, "as far as {mainName} is concerned, the important thing is that the spectators know that {heShe} got a chance to observe " + reference(c) + "'s body's reactions up close.");
							}
						} else if (c.getConfidence() > 33) {
							append(w, t, c, "As {mainName} and {c:mainName} scuffle, ");
							if (confidence > 66) {
								append(w, t, "{mainName}'s firm hands on " + reference(c) + "'s nipples and " + organ + " are starting to have an effect, ");
							} else if (confidence > 33) {
								append(w, t, "{mainName} keeps rubbing " + reference(c) + "'s " + organ + ", ");
							} else {
								append(w, t, c, "{mainName}'s desperate attempts to fend " + reference(c) + " off with a knee against {c:hisHer} " + lowerOrgan + " are becoming increasingly stimulating, ");
							}
							append(w, t, c, "and when they finally pull apart, {c:mainName} can't hide {c:hisHer} arousal from the spectators.");
						} else {
							append(w, t, c, "{mainName} squeezes {c:mainName}'s " + lowerOrgan + ", causing {c:mainName} to squeak and push {himHer} away with surprising force.  As {mainName} recovers, ");
							if (hostility > 66) {
								append(w, t, "{heShe} wears a cruel grin as {heShe} speaks.");
							} else if (hostility > 33) {
								append(w, t, "{heShe} masks {hisHer} annoyance and raises {hisHer} voice.");
							} else {
								append(w, t, "{heShe} looks disappointed with " + reference(c) + "'s reaction.");
							}
						}
					} else {
						if (c.getInnocence() > 66) {
							...
							if (deviancy > 66) {
								append(w, t, c, "then slowly licks the side of {c:hisHer} face before pushing {c:himHer} to the ground.");
							} else if (deviancy > 33) {
								append(w, t, c, "then pushes {c:himHer} away and leers at {c:himHer} on the ground.");
							} else {
								append(w, t, c, "then shoves {c:himHer} away with a huff of disgust.");
							}
						} else if (c.getInnocence() > 33) {
							...
							if (innocence > 66) {
								append(w, t, c, "{mainName} tilts {hisHer} head in mild puzzlement as {c:mainName} groans and squirms, ");
							} else if (innocence > 33) {
								append(w, t, c, "{mainName} shifts {hisHer} weight forward until {c:mainName} cries out, ");
							} else {
								append(w, t, "{mainName} moves {hisHer} foot precisely forward and back, at just the right level of pressure to straddle the line between pleasure and pain, ");
							}
							...
							if (hostility > 66) {
								append(w, t, c, "repeatedly stomping on {c:hisHer} " + lowerOrgan + ", but the sadistic pleasure causes {himHer} to let {hisHer} guard down, and {c:mainName} sees an opportunity to ");
							} else if (hostility > 33) {
								append(w, t, c, "cruelly pinching and twisting {c:hisHer} " + organ + ", but the fact that {heShe} only has one free hand to hold " + reference(c) + " in place means that {c:mainName} can ");
							} else {
								append(w, t, c, "gently stroking {c:hisHer} " + organ + " and " + lowerOrgan + ".  {HisHer} grip on " + reference(c) + " is light enough that eventually {c:mainName} is able to ");
							}
							append(w, t, c, "abruptly pull away - but only at the cost of leaving much of {c:hisHer} " + bottomDesc + " in {mainName}'s hands.");
						}
					}
				} else {
					if (disgrace > 66) {
						if (c.getConfidence() > 66) {
							append(w, t, c, "{c:name} chases down {mainName} and grabs {himHer} by the arm in order to stop {himHer} from escaping again, ");
							if (deviancy > 66) {
								append(w, t, c, "but {mainName} doesn't even try to get free, instead embracing " + reference(c) + " and eagerly running {hisHer} hands all over {c:hisHer} body.");
							} else if (deviancy > 33) {
								append(w, t, c, "and {mainName} retaliates by turning around and immediately trying to make out with " + reference(c) + ", using {c:mainName} preoccupation with holding {himHer} in order to easily grope {c:hisHer} " + lowerOrgan + ".");
							} else {
								append(w, t, c, "leaving {mainName} no choice but to try to distract " + reference(c) + " by acting in an overtly sexual manner again, reaching down and rubbing {c:hisHer} " + organ + ".");
							}
						} else if (c.getConfidence() > 33) {
							...
							if (confidence > 66) {
								...
								append(w, t, ", using {hisHer} other hand to squeeze " + reference(c) + "'s butt.");
							} else {
								append(w, t, c, ", begging " + reference(c) + " to ignore the fight for awhile and just let {c:himHer}self feel good.");
							}
						} else {
							append(w, t, c, "{c:name} had been chasing {mainName}, but when {mainName} turns around ");
							if (hostility > 66) {
								append(w, t, c, "and fixes {c:himHer} with a murderous glare");
							} else if (hostility > 33) {
								append(w, t, c, "and suddenly starts approaching {c:himHer}");
							} else {
								append(w, t, c, "and starts striding toward {c:himHer} with a sense of purpose");
							}
							append(w, t, c, ", {c:heShe} falters.  {mainName} pushes {c:himHer} up against the nearest wall and immediately starts grinding their bodies together.");
						}
					} else if (disgrace > 33) {
						if (c.getInnocence() > 66) {
							append(w, t, c, "When {mainName} rushes forward to launch some close-range attacks, {c:mainName} takes a defensive stance to protect {c:himHer}self, but {c:heShe}'s surprised and paralyzed by pleasurable sensations when ");
							if (deviancy > 66) {
								append(w, t, c, "{mainName} drops to {hisHer} knees and starts pawing at {c:hisHer} crotch, drooling over the thought of licking {c:himHer} down there.");
							} else if (deviancy > 33) {
								append(w, t, c, "{mainName} practically pounces on {c:himHer}, forcing {c:hisHer} legs apart and grinding their hips together.");
							} else {
								append(w, t, c, "{mainName} grapples {c:himHer} instead, distracting {c:himHer} with a hand rubbing between {c:hisHer} legs.");
							}
						} else if (c.getInnocence() > 33) {
							append(w, t, c, "{mainName} and {c:mainName} start to fight at closer range again, weapons clashing against each other.  However, ");
							if (innocence > 66) {
								append(w, t, c, "{mainName} is acting almost playfully, trying to grope " + reference(c) + " whenever {c:heShe}'s within arm's reach");
							} else if (innocence > 33) {
								append(w, t, c, "{mainName} remains focused on trying to inflict pleasure on " + reference(c) + ", and now that they're fighting within arm's length, there are plenty of opportunities to bump up against {c:hisHer} crotch, slap {c:hisHer} bottom, or briefly grab {c:hisHer} chest.");
							} else {
								append(w, t, c, "{mainName}'s strategy still relies on disrupting " + reference(c) + " with unwanted pleasure, and most of {hisHer} attacks are focused around cornering {c:mainName} until {mainName} can finally grab {c:himHer} with both hands and start stimulating {c:himHer} in earnest.");
							}
						} else {
							if (confidence > 66) {
								append(w, t, c, "{mainName} aggressively fights {hisHer} way toward {c:mainName}, ");
							} else if (confidence > 33) {
								append(w, t, c, "Leaving {c:himHer} no time to evade, {mainName} launches {himHer}self directly at {c:mainName} , ");
							} else {
							...
							append(w, t, c, "tackling {c:himHer} to the ground and trying to hold {c:himHer} there.  {c:mainName}'s determined struggling takes its toll on {mainName}'s body, but the way they grind against each other, almost as if they're having sex in the middle of the battlefield, also wears down {c:mainName}'s self-control.");
						}
					} else {
						if (c.getDignity() > 66) {
							append(w, t, c, "Caught in front of the spectators' cameras, {c:mainName} refuses to flee from the approaching {mainName}, ");
							if (hostility > 66) {
								append(w, t, c, "but {mainName} still doesn't show any mercy whatsoever as {heShe} throws " + reference(c) + " down to the pavement and starts stomping on {c:hisHer} " + lowerOrgan + ".");
							} else if (hostility > 33) {
								append(w, t, c, "but {mainName} still manages to humiliate " + reference(c) + " by grabbing {c:himHer} from behind and rubbing a palm against {c:hisHer} " + organ + " in front of everyone.");
							} else {
								append(w, t, c, "and {mainName} responds by being almost gentle as {heShe} pushes " + reference(c) + " onto {c:hisHer} back and starts stroking {c:hisHer} " + organ + ".");
							}
						} else if (c.getDignity() > 33) {
							if (deviancy > 66) {
								append(w, t, "Eagerly using {hisHer} overwhelming power to satisfy {himHer}self, ");
							} else if (deviancy > 33) {
								append(w, t, "Toying with {hisHer} weaker opponent, ");
							} else {
							...
							if (innocence > 66) {
								append(w, t, c, "{mainName} enthusiastically plays with {c:hisHer} " + organ + " and " + lowerOrgan + ".");
							} else if (innocence > 33) {
								append(w, t, c, "{mainName} holds {c:himHer} in place with a hand on {c:hisHer} " + lowerOrgan + ".");
							} else {
								append(w, t, c, "{mainName} pinches {c:hisHer} " + organ + " and allows {c:himHer} to stimulate {c:himHer}self with {c:hisHer} own reflexive struggling.");
							}
						}
					}
				}
				...
				if (hostility > 66) {
					if (c.getMorality() > 66) {
						if (disgrace > 66) {
							append(w, t, "In order to level the playing field with {hisHer} stronger opponent, ");
						} else if (disgrace > 33) {
						...
						append(w, t, c, "{c:name} won't be able to attack {himHer} there.  The terrified humans cry out for {c:mainName} to save them, but {c:heShe} clearly can't even do that.");
					} else {
						append(w, t, c, "{c:name} becomes more and more annoyed as {c:heShe} tries to chase and strike down {mainName}, whose ");
						if (confidence > 66) {
							append(w, t, c, "boastful, confident manner makes the spectators think {heShe}'s completely out of {c:mainName}'s league.");
						} else if (confidence > 33) {
							append(w, t, c, "showy evasive maneuvers catch the spectators' eyes, making {himHer} look like {heShe}'s easily beating {c:mainName} even without directly fighting {c:himHer}.");
						} else {
							append(w, t, c, "meek demeanor makes the spectators think it's completely ridiculous that {c:mainName} is still struggling to land a solid hit on {himHer}.");
						}
					}
				} else if (hostility > 33) {
					if (c.getInnocence() > 66) {
						...
						append(w, t, c, "and the more {c:heShe} gets frustrated and starts attacking " + c.reference(this) + " with wild, obviously futile attacks, the more pathetic {c:heShe} looks to the spectators.");
					} else if (c.getInnocence() > 33) {
						append(w, t, c, "As {c:mainName} pursues {mainName} and tries to land at least one decisive hit, ");
						if (dignity > 66) {
							append(w, t, "{mainName} flaunts {hisHer} moves for the spectators, performing elaborate dodges designed to make them respect {himHer} and question " + reference(c) + ".");
						} else if (dignity > 33) {
							append(w, t, c, "{mainName} refuses to let {c:himHer} do so.  The defensive fighting style doesn't accomplish much in the short term, but it does make everyone who sees it question {c:mainName}'s competence.");
						} else {
							append(w, t, c, "{mainName} just smirks and puts all {hisHer} effort into dodging.  For the spectators, who are used to flashier fights, it appears as though {c:mainName} is just weak.");
						}
					} else {
						append(w, t, c, "As their running battle reaches a high ground in view of several crowds of distant spectators across the city, {mainName} adopts a defensive posture.  Although {c:mainName} tries several possible strategies for getting around it, ");
						if (disgrace > 66) {
							append(w, t, "even the weaker {mainName} is able to avoid taking any hits as long as {heShe} doesn't try to accomplish anything else.");
						} else if (disgrace > 33) {
							append(w, t, "{mainName} doesn't take any risks that could be exploited.");
						} else {
					...
					if (c.getConfidence() > 66) {
						if (disgrace > 66) {
							append(w, t, c, "{mainName} focuses everything {heShe} has on avoiding {c:mainName}'s attacks, ");
						} else if (disgrace > 33) {
						...
						append(w, t, c, "lecturing {c:himHer} at the same time about how {c:hisHer} raw strength isn't enough on its own.");
					} else if (c.getConfidence() > 33) {
						append(w, t, "{mainName} spends some time focusing on talking rather than fighting, constantly dodging backward ");
						if (flavorObedience() > 66) {
							append(w, t, c, "as {heShe} tries to convince {c:mainName} to switch sides.");
						} else if (flavorObedience() > 33) {
							append(w, t, c, "and warning {c:mainName} that {c:hisHer} strength still isn't enough to stand up to the Demon Lord.");
						} else {
							append(w, t, c, "and giving {c:mainName} tips on how to fight more effectively.");
						}
					} else {
						append(w, t, c, "{c:name} hesitates, unable to approach {mainName} while ");
						if (deviancy > 66) {
							append(w, t, c, "{heShe} leers at {c:mainName} and voices {hisHer} perverted desires.");
						} else if (deviancy > 33) {
							append(w, t, c, "{heShe} belittles {c:mainName} and points out the futility of {hisHer} actions.");
						} else {
							append(w, t, c, "{heShe} calmly explains {c:mainName}'s failings.");
						}
					}
				}
			} else {
				if (disgrace > 66) {
					if (c.getMorality() > 66) {
						append(w, t, c, "{c:name}'s attempts to offer {mainName} a chance to surrender peacefully are interrupted when {mainName} abruptly charges at {c:himHer}, ");
						if (hostility > 66) {
							w.append(t, "too enraged to even consider " + reference(c) + "'s words.");
						} else if (hostility > 33) {
							w.append(t, "unwilling to accept " + reference(c) + "'s pity.");
						} else {
							append(w, t, "knowing that {heShe} has to take any opening {heShe} can.");
						}
					} else if (c.getMorality() > 33) {
						append(w, t, "Feeling the effects of exhaustion, {mainName} ");
						if (innocence > 66) {
							...
							append(w, t, c, "tries to take advantage of {c:mainName}'s distraction to slip away and catch {hisHer} breath, but ");
						}
						append(w, t, c, "{c:name} manages to lunge forward and grab {himHer} by the arm, preventing {hisHer} escape.");
					} else {
						append(w, t, c, "{c:name} senses {mainName}'s weakness and charges in for the kill, ");
						if (confidence > 66) {
							append(w, t, "but {mainName} fights back fiercely despite how far {hisHer} powers have declined.");
						} else if (confidence > 33) {
							append(w, t, c, "and {c:hisHer} aggression is rewarded as {mainName} quickly starts to falter under the assault.");
						} else {
							append(w, t, "causing {mainName} to shriek in pain and fear as {heShe} desperately defends {himHer}self.");
						}
					}
				} else if (disgrace > 33) {
					if (c.getConfidence() > 66) {
						if (confidence > 66) {
							append(w, t, "{mainName} never shies away from battle, but a moment of carelessness puts {himHer} on {hisHer} back foot, forcing {himHer} to retreat, and ");
						} else if (confidence > 33) {
							append(w, t, c, "In terms of raw strength, {mainName} is just a bit weaker than {c:mainName}.  {HeShe} tries to put some more distance between {himHer}self and " + reference(c) + ", but ");
						} else {
							append(w, t, "{mainName}'s Forsaken powers aren't weak, but {heShe} still finds it difficult to fight enemies head-on, and ");
						}
						append(w, t, c, "{c:name} quickly chases {himHer} down and tackles {himHer} to the ground, where they continue to scuffle.");
					} else if (c.getConfidence() > 33) {
						append(w, t, c, "As {mainName} and {c:mainName} quietly circle each other, ");
						if (innocence > 66) {
							append(w, t, c, "{mainName} gets distracted by a loud noise behind {himHer}, and {c:mainName} darts forward to take advantage of the opening!");
						} else if (innocence > 33) {
							...
							append(w, t, c, "{c:name} abruptly realizes that {c:heShe}'s been maneuvered into a corner, and an instant later, {mainName} darts directly at {himHer} in order to take advantage of the positional advantage!");
						}
					} else {
						if (deviancy > 66) {
							append(w, t, c, "Intimidated by {mainName} obvious desire to violate {c:himHer}, ");
						} else if (deviancy > 33) {
							append(w, t, c, "Uneasy over the way {mainName} leers at {c:himHer}, ");
						} else {
							append(w, t, c, "Insecure about {c:hisHer} own strength, ");
						}
						append(w, t, c, "{c:name} starts looking around for a way to escape {c:hisHer} confrontation with {mainName}, but that moment of hesitation is all the excuse {mainName} needs to pounce forward and press the attack against {c:himHer}.");
					}
				} else {
					if (c.getMorality() > 66) {
						append(w, t, c, "{c:name} has briefly managed to escape {c:hisHer} opponent, but when {c:heShe} realizes that there's no one else nearby to stop {mainName} from leading the Demons and breaching the evacuation perimeter, {c:heShe} reluctantly turns around and willingly faces " + c.reference(this) + ", ");
						if (hostility > 66) {
							append(w, t, c, "mentally bracing {c:himHer}self for the torture that {mainName} no doubt has in store.");
						} else if (hostility > 33) {
							append(w, t, "much to {mainName}'s amusement.");
						} else {
						...
						if (deviancy > 66) {
							...
							append(w, t, c, "enjoying the view of {c:hisHer} behind");
						} else {
							append(w, t, c, "allowing {c:himHer} to exhaust {c:himHer}self");
						}
						append(w, t, c, ", then abruptly picks up speed, grabbing " + reference(c) + " and pulling {c:himHer} down to the ground.");
					} else {
						if (innocence > 66) {
							append(w, t, c, "{c:name} briefly escapes {mainName}'s notice, but then ");
						} else if (innocence > 33) {
							append(w, t, c, "{c:name} has fled far enough from {mainName} that they can no longer see each other, but then ");
						} else {
			...
			if (styleDamage[0] > 0) {
				if (styleDamage[3] > 0) {
					if (hostility > 66) {
						if (c.getMorality() > 66) {
							if (confidence > 66) {
								append(w, t, "{mainName} sees that countless cameras are watching {hisHer} battle, and {heShe} exults in the chance to directly threaten the public with an excruciatingly painful death.  ");
							} else if (confidence > 33) {
								append(w, t, "As the spectators' cameras watch the battle, {mainName} rants and raves, shouting to anyone who will listen that death awaits them all.  ");
							} else {
								append(w, t, "{mainName} gives voice to {hisHer} despair, calling out to all the watching spectators about how humanity is doomed and the closest thing to salvation is a quick death.  ");
							}
							append(w, t, c, "For {c:mainName}, protecting the people's hope is just as important as striking down " + c.reference(this) + ", and {heShe} feels obligated to try to reassure everyone.");
						} else if (c.getMorality() > 33) {
							if (disgrace > 66) {
								append(w, t, "Even though {heShe} barely has the energy to stand, {mainName}'s hatred gives {himHer} the strength to keep fighting and ");
							} else if (disgrace > 33) {
								append(w, t, c, "They might be evenly-matched, but {mainName} is far more bloodthirsty than {c:mainName}, and {heShe} keeps ");
							} else {
								append(w, t, c, "The difference in strength is so great that {mainName} doesn't even need to worry about {c:mainName}'s attacks, leaving {himHer} free to continuously ");
							}
							...
							if (innocence > 66) {
								...
								append(w, t, c, "{mainName}'s experiences have given {himHer} a good idea of the psychological weaknesses of people like {c:mainName}.  {HeShe} uses {hisHer} words to exploit those weaknesses mercilessly, causing as much pain as possible, and ");
							}
							append(w, t, c, "{c:name}'s obvious impotent rage weakens both {c:hisHer} emotional stability and {c:hisHer} public image.");
						}
					} else if (hostility > 33) {
						if (c.getInnocence() > 66) {
							if (innocence > 66) {
								append(w, t, c, "In the chaotic melee, {mainName} and {c:mainName} lose track of each other, but {mainName} gets {c:hisHer} attention again by ");
							} else if (innocence > 33) {
							...
							if (deviancy > 66) {
								...
								append(w, t, c, "happily taking the opportunity to insult {c:mainName} and try to provoke {c:himHer}.  ");
							} else {
								append(w, t, "venting {hisHer} irritation by harshly insulting " + reference(c) + ".  ");
							}
							...
							if (disgrace > 66) {
								append(w, t, c, "{mainName} knows that {heShe} doesn't have much of a chance against {c:mainName}'s tactical acumen and superior power, so {heShe} continues backing off and ");
							} else if (disgrace > 33) {
								append(w, t, c, "{c:name}'s combat style is precise and methodical, but {mainName} manages to take advantage of it as {heShe} ");
							} else {
								append(w, t, c, "{c:name} is too smart to carelessly approach the stronger {mainName}, but {mainName} tries to force {c:hisHer} hand, ");
							}
							append(w, t, c, "addressing the filming spectators.  {HeShe} calls {c:mainName} a coward and a fake hero, watching closely for a reaction.");
						}
					} else {
						if (c.getDignity() > 66) {
							append(w, t, c, "A sudden flurry of camera flashes reminds {c:mainName} of how much damage {c:hisHer} clothes have taken over the course of the fight.  {c:HeShe} blushes and hurriedly rearranges them, then frowns at {mainName}, ");
							if (deviancy > 66) {
								append(w, t, c, "whose lustful urges have overflowed and caused {c:himHer} to make lewd comments.");
							} else if (deviancy > 33) {
								append(w, t, c, "who is very obviously leering at {c:himHer}.");
							} else {
								w.append(t, "who flatly comments on how much was showing.");
							}
						} else if (c.getDignity() > 33) {
							if (disgrace > 66) {
								append(w, t, c, "{c:name} isn't having much trouble fighting {mainName}, but ");
							} else if (disgrace > 33) {
								append(w, t, c, "{c:name}'s fight against {mainName} doesn't give {c:himHer} a chance to worry about secondary concerns, and ");
							} else {
							...
							if (flavorObedience() > 66) {
								...
								w.append(t, "following your orders to humiliate " + reference(c) + " as much as possible.");
							} else {
								append(w, t, c, "then turns around and angrily admonishes {c:mainName} for not covering {c:himHer}self up better.");
							}
						}
					}
				} else {
					if (hostility > 66) {
						if (c.getInnocence() > 66) {
							...
							if (deviancy > 66) {
								...
								append(w, t, c, "but {c:mainName} is disgusted by the way that {mainName} is clearly enjoing the contact with {c:hisHer} body.  {c:mainName} shifts {c:hisHer} efforts into getting away, opening up some distance between {c:himHer}self and " + c.reference(this) + ", then looks back at {himHer} with revulsion.");
							} else {
								append(w, t, c, "but the cold threats {mainName} whispers into {c:mainName}'s ear soon become too much to bear, and {c:mainName} recoils backward, trying to collect {c:himHer}self.");
							}
						} else if (c.getInnocence() > 66) {
							if (morality > 66) {
								append(w, t, "{mainName}'s sense of betrayal toward humanity and hatred toward {himHer}self grwos strong enough to take physical form around {himHer} as a dark aura of despair, and ");
							} else if (morality > 33) {
								append(w, t, "The battle causes {mainName}'s negative emotions to overflow, briefly coalescing around {himHer} in a visible whirlpool of blackness, and ");
							} else {
								append(w, t, "{mainName} is engulfed in cold flames that seem to drink the light from {hisHer} surroundings, a psychic manifestation of {hisHer} burning hatred toward humanity itself.  Although they're only a mental projection, they induce a very real reaction in other people, and ");
							}
							append(w, t, c, "{c:name} is forced to retreat as {c:heShe} fights off the effects on {c:hisHer} own mind.");
						} else {
							if (disgrace > 66) {
								append(w, t, c, "{c:name} struggles to lock {mainName} in a pin against the ground, and although {mainName} is struggling desperately, {heShe}'s distracted by the sight of a nearby civilian.  {mainName} tries to lunge after the bystander, consumed by thoughts of murdering him, and {c:mainName} takes the opportunity to seize control.");
							} else if (disgrace > 33) {
								append(w, t, c, "{c:name}'s and {mainName}'s battle is briefly interrupted as {mainName} spots a passing civilian and darts off to intercept him, bloodlust in {hisHer} eyes.");
							} else {
								append(w, t, c, "{c:name} is pinned on the ground, helpless underneath {mainName}, but {c:heShe}'s suddenly surprised to find that {mainName} has released {c:himHer}.  A small group of civilians who are late to be evacuated has wandered by, and {mainName} wants to kill them before returning to finish off " + reference(c) + ".");
							}
						}
					} else if (hostility > 33) {
						if (c.getConfidence() > 66) {
							append(w, t, "Wearing an infuriating smirk, {mainName} ");
							if (deviancy > 66) {
								append(w, t, c, "allows {c:mainName} to graze {himHer} with several attacks, moaning in sexual pleasure every time.  The sight unnverves {c:mainName} enough to slow {c:hisHer} movements, and {mainName} backs off to focus on taunting {c:himHer} even more.");
							} else if (deviancy > 33) {
								append(w, t, c, "deliberately makes {c:mainName} uncomfortable with constant sexual talk as they fight, using {c:hisHer} disgust to make {c:himHer} grow increasingly careless and allow {mainName} to easily evade {c:hisHer} attacks and get to a high ground position above {c:himHer}.");
							} else {
								append(w, t, c, "keeps making derogatory remarks about {c:mainName}, calling {c:himHer} weak and stupid, and claiming that {c:heShe}'s nowhere near strong enough to beat the Demon Lord.  {c:mainName}'s determination to prove {mainName} wrong causes {c:himHer} to become reckless, and {mainName} emphasizes the point by dodging around {c:himHer} and leading {c:himHer} on a chase.");
							}
						} else if (c.getConfidence() > 33) {
							if (disgrace > 66) {
								append(w, t, c, "Even as {c:mainName}'s attacks take their toll, ");
							} else if (disgrace > 33) {
								append(w, t, c, "As {heShe} exchanges blows with {c:mainName}, ");
							} else {
								append(w, t, c, "Shrugging off {c:mainName}'s attacks like they're nothing, ");
							}
							append(w, t, c, "{mainName} constantly insults {c:mainName}'s abilities.  Several Demons show up to occupy {c:mainName} so that {mainName} can continue the stream of cruel words uninterrupted.");
						} else {
							...
							if (dignity > 66) {
								...
								append(w, t, c, "insulting {c:himHer} again and again.  ");
							} else {
								w.append(t, "unleashing a torrent of raw hostility.  ");
							}
							append(w, t, c, "{c:name} ends up stumbling backwards, wiping tears out of {c:hisHer} eyes and trying to remind {c:himHer}self that there's no need to care about the words spoken by an enemy.");
						}
					} else {
						if (c.getMorality() > 66) {
							...
							if (flavorObedience() > 66) {
								...
								append(w, t, c, "{mainName} harshly criticizes what {heShe} sees as {c:mainName}'s failings.");
							}
						} else if (c.getMorality() > 33) {
							append(w, t, c, "A clash of energy between {mainName} and {c:mainName} starts to collapse the building whose rooftop they're fighting on.  ");
							if (deviancy > 66) {
								append(w, t, c, "As they both clamber to safety, {mainName} can't resist groping {c:mainName}, causing them both to fall and land some distance apart.");
							} else if (deviancy > 33) {
								append(w, t, c, "{mainName} reaches the ground first, and {heShe} can't resist making lewd comments as {heShe} looks up at where {c:mainName} is climbing down.");
							} else {
				...
				if (deviancy > 66) {
					if (c.getConfidence() > 66) {
						append(w, t, c, "{mainName} launches {himHer}self at {c:mainName}, wrapping {hisHer} limbs around {c:himHer}, and no matter how violently {c:mainName} tries to beat {himHer} away, ");
						if (innocence > 66) {
							append(w, t, "{mainName} doesn't even seem to care, ");
						} else if (innocence > 33) {
							append(w, t, "{mainName}'s lust gives {himHer} the strength to hold on, ");
						} else {
							append(w, t, "{mainName} squirms to avoid the worst of it, ");
						}
						w.append(t, "completely focused on tearing " + reference(c) + "'s clothes off.");
					} else if (c.getConfidence() > 33) {
						if (disgrace > 66) {
							append(w, t, c, "The fleeing {mainName} leads {c:mainName} into a central plaza in plain view of many distant cameras, then abruptly rounds on {c:himHer} and ");
						} else if (disgrace > 33) {
						...
						append(w, t, c, "starts tearing at " + reference(c) + "'s clothes, as turned on by the fact that they're being watched as {heShe} is by {c:mainName}'s increasingly exposed skin.");
					} else {
						if (hostility > 66) {
							append(w, t, c, "Determined to make the shy {c:mainName} suffer, {mainName} tackles {c:himHer} and ");
						} else if (hostility > 33) {
						...
						append(w, t, c, "starts pulling {c:hisHer} torn clothes out of the way for the benefit of the countless cameras watching the battle.");
					}
				} else if (deviancy > 33) {
					if (c.getDignity() > 66) {
						if (innocence > 66) {
							...
							append(w, t, c, "It hasn't escaped {mainName}'s notice that {c:mainName} gets more careful around cameras the more {c:hisHer} clothes are torn, ");
						}
						append(w, t, c, "and when {heShe} lunges at " + reference(c) + " to start trying to pull {c:hisHer} " + bottomDesc + " aside, {heShe} isn't disappointed with {c:mainName}'s squealing reaction.");
					} else if (c.getDignity() > 33) {
						append(w, t, c, "The more {c:mainName}'s clothes come off, the more perverted {mainName}'s expression gets.  {HeShe} runs at {c:mainName}, intent on tearing off even more of {c:hisHer} clothes");
						if (disgrace > 66) {
							append(w, t, " even though it opens {himHer} up to a punishing counterattack.");
						} else if (disgrace > 33) {
							append(w, t, ", now thinking less about the battle and more about {hisHer} own lustful desires.");
						} else {
							append(w, t, ", and " + reference(c) + " doesn't have the strength to resist {himHer}.");
						}
					} else {
						append(w, t, c, "No matter how much {c:hisHer} clothes get torn, {c:mainName} hardly seems to care.  {mainName} gets in close to help tear them off further, ");
						if (flavorObedience() > 66) {
							append(w, t, "happy that {heShe}'s found an opponent who doesn't realize how valuable it is for the Demons when the Chosen are made to look vulnerable to the public.");
						} else if (flavorObedience() > 33) {
							append(w, t, c, "relieved that {hisHer} orders to humiliate {c:mainName} will be relatively easy to execute.");
						} else {
							append(w, t, c, "enjoying feeling like {heShe}'s playing a prank on {c:himHer}.");
						}
					}
				} else {
					if (c.getInnocence() > 66) {
						...
						if (hostility > 66) {
							...
							append(w, t, c, "There's no anger behind {mainName}'s attacks, but {heShe} still intends to debilitate {c:mainName}.  {HisHer} strikes are precise, and even when {c:mainName} dodges, {c:hisHer} clothes are usually caught and partially torn away.");
						}
					} else {
						append(w, t, c, "{c:name}'s clothes have taken significant damage, and while {mainName} isn't interested in ogling {c:himHer}, ");
						if (flavorObedience() > 66) {
							append(w, t, "{heShe}'s eager to please the Demon Lord.  ");
						} else if (flavorObedience() > 33) {
							append(w, t, "{heShe} knows that the fight will become easier as the Chosen begin to look more exposed and vulnerable to the public and each other.  ");
						} else {
							append(w, t, c, "{heShe} knows that {c:mainName} doesn't want to be exposed.  ");
						}
						append(w, t, c, "{HeShe} charges forward, forcing {c:mainName} to choose between fighting effectively and keeping {c:hisHer} movements minimal in order to protect {c:hisHer} modesty in front of the watching cameras.");
					}
				}
			} else {
				if (disgrace > 66) {
					if (c.getConfidence() > 66) {
						if (innocence > 66) {
							append(w, t, c, "{c:name} can't help but underestimate {mainName}, and {c:heShe}'s too prideful to admit to {c:himHer}self that " + c.reference(this) + " might actually be able to wear {c:himHer} down.  ");
						} else if (innocence > 33) {
						...
						if (deviancy > 66) {
							...
							append(w, t, c, "{mainName} constantly squirms and contorts {hisHer} body, even enjoying the way it causes {himHer} to rub up against " + reference(c) + ", and {c:mainName} can't completely keep {hisHer} hands off {c:himHer}.");
						} else {
							append(w, t, "{mainName} is content to bide {hisHer} time and look for an opening to kick {hisHer} way free - which {heShe} eventually finds.");
						}
					} else {
						append(w, t, c, "{c:name} has tackled {mainName} to the ground, and by all rights, {c:mainName} should be able to neutralize {c:hisHer} weakened opponent right then and there.  ");
						if (confidence > 66) {
							append(w, t, "But {mainName}'s personality is intimidating even now, ");
						} else if (confidence > 33) {
							append(w, t, c, "But {c:mainName} lacks the willpower to follow through, ");
						} else {
							append(w, t, c, "But {mainName} struggles with a wild desperation that {c:mainName} isn't prepared for, ");
						}
						append(w, t, "and soon it's {mainName} that's back on top of " + reference(c) + ".");
					}
				} else if (disgrace > 33) {
					if (c.getInnocence() > 66) {
						append(w, t, c, "Neither {mainName} nor {c:mainName} is significantly stronger than the other, but ");
						if (confidence > 66) {
							append(w, t, c, "{mainName} is clearly so convinced of {hisHer} victory that {c:mainName} subconsciously assumes that there's no point in resisting as {c:heShe}'s pushed down to the ground.");
						} else if (confidence > 33) {
							append(w, t, c, "{c:name} is careless and easily distracted.  One inattentive moment is all it takes for {mainName} to push {c:himHer} to the ground.");
						} else {
							append(w, t, c, "while {c:mainName} doesn't fully understand the stakes of the battle, {mainName} is motivated by terror of the consequences if {heShe} fails.  Desperation fuels {mainName}'s strength, allowing {himHer} to overpower {c:mainName} and push {c:himHer} to the ground.");
						}
					} else if (c.getInnocence() > 33) {
						...
						if (flavorObedience() > 66) {
							...
							append(w, t, "grits {hisHer} teeth and reminds {himHer}self that {heShe} can't lose.");
						} else {
					...
					if (c.getMorality() > 66) {
						...
						append(w, t, c, "{mainName} easily seizes control in {hisHer} grapple with {c:mainName}, putting {c:himHer} on {c:hisHer} back and pinning {c:hisHer} hands over {c:hisHer} head");
						if (deviancy > 66) {
							append(w, t, c, " while practically drooling over the sight of {c:hisHer} body.");
						} else if (deviancy > 33) {
							append(w, t, c, ", unable to resist the urge to make some sexual comments while {c:heShe} does so.");
						} else {
							append(w, t, c, " in order to make sure {c:heShe} understands how helpless {c:heShe} is.");
						}
					} else {
						append(w, t, c, "Frustrated with {c:hisHer} inability to make a dent in {mainName}'s body, {c:mainName} gives in to {c:hisHer} anger, ");
						if (flavorObedience() > 66) {
							w.append(t, "insulting the Demon Lord");
						} else if (flavorObedience() > 33) {
							append(w, t, "calling {mainName} a coward");
						} else {
							append(w, t, c, "calling {mainName} every name {c:heShe} can think of");
						}
						append(w, t, c, " in hopes of at least inflicting some emotional damage.  Considering that {c:heShe}'s currently locked in a grapple with {mainName}, this proves to be a poor decision.");
					}
				}
			}
		} else if (c.captureProgression % 6 == 5) {
			if (styleDamage[0] > 0) {
				if (styleDamage[2] > 0) {
					if (hostility > 66) {
						...
						if (c.getMorality() > 66) {
							if (disgrace > 66) {
								append(w, t, "Desperate to buy {himHer}self some time and willing to do anything for it, ");
							} else if (disgrace > 33) {
							...
							append(w, t, c, "{mainName} begins drawing on as much Demonic energy as {heShe} can, and before {c:mainName} can stop {himHer}, an enormous blast consumes their side of the district, slaughtering Thralls along with any nearby civilians who couldn't evaucate.  As the light fades, ");
							if (deviancy > 66) {
							...
							if (innocence > 66) {
								append(w, t, "and {mainName} assumes that it's " + reference(c) + "'s fault, ");
							} else if (innocence > 33) {
								append(w, t, "and {mainName} doesn't take losing sight of the object of {hisHer} sadism well, ");
							} else {
								append(w, t, "and the disruption to {hisHer} plans causes {mainName} to abruptly snap, ");
							}
							append(w, t, c, "shrieking with incoherent rage at {c:mainName} and lashing out in all directions.");
						}
					} else if (hostility > 33) {
						//hiding behind Demons
						if (c.getConfidence() > 66) {
							append(w, t, c, "{c:name}'s fight with {mainName} takes them into the heart of the Demonic army, breaking their line of sight with each other.  {c:mainName} shouts angry accusations that {mainName} is hiding behind the Demons as {c:heShe} fends them off, ");
							if (disgrace > 66) {
								append(w, t, c, "and {c:heShe}'s not completely wrong.");
							} else if (disgrace > 33) {
								append(w, t, "although {mainName} is just as annoyed by the development.");
							} else {
								append(w, t, c, "even though {c:mainName} {c:himHer}self is the one who benefits more from the reprieve.");
							}
						} else if (c.getConfidence() > 33) {
							if (innocence > 66) {
								append(w, t, c, "A squad of Demons pushes past {mainName} on their way to {c:mainName}, disorienting {himHer} and causing {himHer} to briefly lose track of " + reference(c) + ".  ");
							} else if (innocence > 33) {
							...
							append(w, t, c, "A horde of Demons suddenly attacks {c:mainName}, and {c:heShe} has trouble fending them off.  ");
							if (deviancy > 66) {
								append(w, t, c, "{mainName} takes a seat on a nearby rooftop and starts masturbating as {heShe} watches, moaning loudly to make sure {c:mainName} notices {himHer}.");
							} else if (deviancy > 33) {
						...
						//separated by chance
						if (c.getInnocence() > 66) {
							append(w, t, c, "{c:name} falls off a rooftop as {c:heShe} tries to close the distance with {mainName}.  When {mainName} notices, {heShe} ");
							if (innocence > 66) {
								append(w, t, "looks down and giggles despite {himHer}self.");
							} else if (innocence > 33) {
								append(w, t, c, "angrily reminds {c:mainName} to take their fight seriously.");
							} else {
								w.append(t, "sighs and voices some biting criticism.");
							}
						} else if (c.getInnocence() > 33) {
							append(w, t, c, "The flow of the battle has put some distance between {c:mainName} and {mainName}, but when {c:mainName} tries to turn away and put {c:hisHer} focus back on fighting the Demons, {mainName} hits {c:himHer} in the back with a long-range blast of energy");
							if (deviancy > 66) {
								w.append(t, ", suddenly overcome by an irresistable urge to make " + reference(c) + " scream.");
							} else if (deviancy > 33) {
								append(w, t, c, ", smirking at the way {c:heShe} squeaks in indignation.");
							} else {
								append(w, t, c, ", reminding {c:himHer} not to let {c:hisHer} guard down around an opponent.");
							}
						} else {
							append(w, t, c, "{c:name} fights off some straggler Demons, keeping a wary eye on {mainName} at the same time.  They're too far from each other to fight now, but ");
							if (flavorObedience() > 66) {
								append(w, t, c, "{mainName} is determined to bring {c:himHer} down for the Demon Lord, ");
							} else if (flavorObedience() > 33) {
								append(w, t, "{mainName} knows that the Demon Lord won't be satisfied with this performance, ");
							} else {
								append(w, t, "{mainName} shows no signs of stopping, ");
							}
							append(w, t, c, "and soon enough {heShe} starts to move toward {c:mainName} again.");
						}
					}
				} else {
					if (hostility > 66) {
						if (c.getInnocence() > 66) {
							if (flavorObedience() > 66) {
								append(w, t, "{mainName} is willing to put {hisHer} murderous desires aside for the Demon Lord's sake, ");
							} else if (flavorObedience() > 33) {
								append(w, t, "{mainName}'s fear of the Demon Lord's wrath is greater than {hisHer} desire to kill, ");
							} else {
								append(w, t, "{mainName}'s hatred wouldn't be satisfied by giving {hisHer} oppoent a kick death, ");
							}
							append(w, t, c, "but {heShe} would still prefer to be directly attacking {c:mainName}.  However, {mainName} worries that {c:mainName}'s oblivious nature would lead {c:himHer} to get {c:himHer}self killed even by an attack that's not intended to be fatal, and so {mainName} contents {himHer}self with causing emotional pain.");
						} else if (c.getInnocence() > 33) {
							append(w, t, c, "{c:name} loses sight of {mainName} in the chaotic battlefield, ");
							if (innocence > 66) {
								append(w, t, "but {mainName} spoils the tactical advantage by continuing to shout furious insults, giving " + reference(c) + " hints as to where {heShe} is.");
							} else if (innocence > 33) {
								append(w, t, c, "and {mainName} stalks {c:himHer} from the shadows, whispering cruel threats just loudly enough to be heard before vanishing again.");
							} else {
								append(w, t, c, "and {mainName} decides to unnerve {c:himHer} by leaving a trail of civilian corpses leading into an ambush.");
							}
						} else {
							if (disgrace > 66) {
								append(w, t, "Despite all {hisHer} impotent rage, ");
							} else if (disgrace > 33) {
								append(w, t, c, "Even though {hisHer} desire to kill {c:mainName} is very sincere, ");
							} else {
								append(w, t, c, "Even though {heShe} might well be capable of killing {c:mainName}, ");
							}
							append(w, t, c, "{mainName} still hasn't inflicted even a scratch on {c:himHer}.  {c:mainName} can only conclude that it must be because of the Demon Lord's orders.");
						}
					} else if (hostility > 33) {
						if (c.getConfidence() > 66) {
							if (disgrace > 66) {
							...
							if (innocence > 66) {
								...
								append(w, t, c, "due to the Demons and piles of rubble between them, {c:mainName} has no choice but to grit {c:hisHer} teeth and take it.");
							} else {
								append(w, t, c, "with how much {heShe}'s come to understand " + reference(c) + ", {heShe} knows exactly what subjects will get the biggest reaction out of {c:himHer}.");
							}
						} else {
							append(w, t, c, "{c:name} finds {c:himHer}self briefly too intimidated to approach {mainName}, ");
							if (deviancy > 66) {
								append(w, t, c, "deeply disturbed by the way that " + c.reference(this) + " stares hungrily at {c:himHer} like {c:heShe}'s a delicious piece of meat.");
							} else if (deviancy > 33) {
								append(w, t, c, "and the cruel taunts and laughter coming {c:hisHer} way from " + c.reference(this) + " don't help matters.");
							} else {
								w.append(t, "sensing " + c.reference(this) + "'s sadistic nature.");
							}
						}
					} else {
						if (c.getMorality() > 66) {
							...
							if (flavorObedience() > 66) {
								append(w, t, c, "{c:name} sees {mainName} eagerly stunning the civilians who were too slow to evacuate and handing them over to the Demons, ");
							} else if (flavorObedience() > 33) {
							...
							append(w, t, "{mainName} has ended up some distance from {hisHer} opponent, ");
							if (deviancy > 66) {
								append(w, t, c, "masturbating and moaning {c:mainName}'s name out loud.  ");
							} else if (deviancy > 33) {
								append(w, t, c, "calling out lewd backhanded compliments at {c:mainName}.  ");
							} else {
								append(w, t, c, "coldly insulting {c:mainName}'s abilities.  ");
							}
							append(w, t, c, "{c:name} is angry, but there's not much {c:heShe} can do about it from down below.");
						} else {
							append(w, t, c, "{c:name} is eager to let out the frustration that's been building over the course of the fruitless battle, but ");
							if (disgrace > 66) {
								append(w, t, "{mainName} keeps running away");
							} else if (disgrace > 33) {
								append(w, t, "the flow of the battle has put {mainName} too far away");
							} else {
								append(w, t, "{mainName} is too dangerous to approach");
							}
							append(w, t, c, ", and {c:mainName}'s aggression continues to grow.");
						}
					}
				}
			} else if (styleDamage[2] > 0) {
				if (flavorObedience() > 66) {
					if (c.getMorality() > 66) {
						append(w, t, c, "{c:name} fights through waves of Demons to reach {mainName}, but {c:heShe} has mixed feelings when {c:heShe} sees that " + c.reference(this) + " is ");
						if (innocence > 66) {
						...
						append(w, t, c, "Several Demons rush in between {c:mainName} and {mainName}, fighting to protect the Forsaken.  ");
						if (confidence > 66) {
							append(w, t, "{mainName} mostly just looks annoyed at the interference.");
						} else if (confidence > 33) {
							append(w, t, "{mainName} angrily tells them not to throw their lives away, but they don't listen.");
						} else {
							append(w, t, "{mainName} looks grateful for the chance to catch {hisHer} breath.");
						}
					} else {
						if (disgrace > 66) {
							...
							append(w, t, c, "Deciding that {c:mainName} is beneath {hisHer} notice for the moment, {mainName} blasts a tunnel straight down through the ground so that the Demons can easily carry their captives to the hive.  ");
						}
						append(w, t, c, "{c:name} doesn't care enough to stop {himHer}, and {c:heShe}'s busy fighting off some Demons attacking {c:himHer} at the same time, so {c:heShe} lets it happen.");
					}
				} else if (flavorObedience() > 33) {
					if (c.getConfidence() > 66) {
						if (disgrace > 66) {
							append(w, t, c, "Desperate to delay the powerful {c:mainName}, {mainName} orders a swarm of Demons to attack {c:himHer}.  ");
						} else if (disgrace > 33) {
							append(w, t, c, "With the battle between {mainName} and {c:mainName} at a stalemate, {mainName} decides to tip the scales by ordering a large-scale Demon attack against {c:mainName}.  ");
						} else {
							append(w, t, c, "A swarm of Demons attacks {c:mainName}, and {mainName} decides to use the opportunity to check on how the rest of the battle is going.  ");
						}
						append(w, t, c, "{c:name}, for {c:hisHer} part, enjoys cutting down the weaker enemies.");
					} else if (c.getConfidence() > 33) {
						append(w, t, c, "A crowd of Demons has grown around the spot where {mainName} and {c:mainName} are fighting, forming into a ring around them.  {mainName} steps back, points at {c:mainName}, and begins shouting combat orders to the Demons, ");
						if (innocence > 66) {
							append(w, t, "although this generally amounts to just using random military jargon {heShe} picked up from the shows {heShe} watches.");
						} else if (innocence > 33) {
							w.append(t, "staying out of the way while they fight.");
						} else {
							append(w, t, c, "using {hisHer} keen tactical mind to let them fight at a greater effectiveness than {c:mainName} is used to.");
						}
					} else {
						if (deviancy > 66) {
					...
					if (c.getInnocence() > 66) {
						append(w, t, c, "A Demon attacks {c:mainName} from behind, knocking {c:himHer} down, but when it tries to hit {c:himHer} again, ");
						if (deviancy > 66) {
							append(w, t, "{mainName} flies into a rage, tearing it apart in an instant.");
						} else if (deviancy > 33) {
							append(w, t, "{mainName} runs by, killing the Demon with a smirk.");
						} else {
							append(w, t, "{mainName} coldly kills the Demon with a blast of energy.");
						}
					} else if (c.getInnocence() > 33) {
						append(w, t, c, "A huge wave of Demons crashes over {c:mainName}, and {c:heShe}'s suddenly fighting enemies in all directions.  However, before {c:heShe} can be overwhelmed, ");
						if (disgrace > 66) {
							append(w, t, c, "{mainName} takes down the Demon that was about to grab {c:himHer} from behind, and with " + c.reference(this) + "'s help, {c:mainName} is just barely able to hold out.");
						} else if (disgrace > 33) {
							append(w, t, c, "{mainName} starts fighting through the Demons as well, carving a path to {c:hisHer} side.  The two of them fight back-to-back, sending the Demons fleeing, before they both jump apart and resume staring each other down.");
						} else {
							append(w, t, "huge explosions start raining from the sky, annihilating the Demons.  {mainName} looks down on the scene, energy crackling around {hisHer} outstretched hand.");
						}
					} else {
						append(w, t, c, "As {mainName} and {c:mainName} fight, ");
						if (hostility > 66) {
							append(w, t, c, "some Demons try to swarm at {c:mainName}, only to be immediately struck down by {mainName} before they can lay a finger on " + reference(c) + ".");
						} else if (hostility > 33) {
							append(w, t, "some Demons come within view, but {mainName} instantly incinerates them before returning {hisHer} attention to " + reference(c) + ".");
						} else {
							append(w, t, "some Demons with captive civilians march by, but {mainName} attacks in order to free the humans, and soon both {heShe} and " + reference(c) + " are fighting the Demons off together.");
						}
					}
				}
			} else {
				if (deviancy > 66) {
					if (c.getDignity() > 66) {
						if (disgrace > 66) {
							...
							append(w, t, c, "{c:name} flees across the city, just ahead of {mainName}'s grasping hands, ");
						}
						append(w, t, c, "blushing deeply as {c:heShe} tries to come to terms with the fact that {c:heShe} doesn't entirely dislike " + c.reference(this) + "'s molestation.");
					} else if (c.getDignity() > 33) {
						append(w, t, c, "As {mainName} and {c:mainName} maneuver around each other, {mainName}'s movements become increasingly spastic");
						if (innocence > 66) {
							append(w, t, c, ", and while it's obvious to {c:mainName} that " + c.reference(this) + " is attempting to masturbate under {hisHer} clothes in the middle of their fight, {c:heShe} does {c:hisHer} best not to dwell on it.");
						} else if (innocence > 33) {
							append(w, t, " as {heShe} tries without much success to masturbate even while {heShe} fights.");
						} else {
							append(w, t, ", because despite {hisHer} expertise on both subjects, {heShe} finds it difficult to masturbate and fight at the same time.");
						}
					} else {
						if (hostility > 66) {
					...
					if (c.getConfidence() > 66) {
						...
						if (confidence > 66) {
							...
							append(w, t, c, "{mainName} comments on how much {heShe} admires {c:mainName}'s figure, disparaging {hisHer} own in the process.  ");
						}
						append(w, t, c, "{c:name} can't help but feel flattered.");
					} else {
						append(w, t, c, "{c:name} knows that {c:heShe}'s supposed to be fighting {mainName}, but {c:heShe} finds it harder and harder to do so, ");
						if (disgrace > 66) {
							append(w, t, c, "as {mainName}'s relative weakness and apparent unwillingness to actually harm {c:himHer} makes it difficult to get motivated.");
						} else if (disgrace > 33) {
							append(w, t, "as {mainName} hasn't actually been launching any dangerous attacks in return.");
						} else {
							append(w, t, c, "demotivated and confused by {mainName}'s unwillingness to actually harm {c:himHer} despite the vast difference in power between them.");
						}
						append(w, t, c, "  The molestation is unpleasant, but not to the point of provoking {c:himHer}.");
					}
				} else {
					if (c.getInnocence() > 66) {
						append(w, t, c, "{c:name} knows that {c:heShe}'s supposed to be fighting {mainName}, but {c:heShe} finds it harder and harder to do so, ");
						...
						if (hostility > 66) {
							append(w, t, c, "Given {mainName}'s obvious murderous desires, {heShe}'s showing a surprising amount of restraint against {c:mainName}.  ");
						} else if (hostility > 33) {
							append(w, t, c, "Although {mainName} clearly takes a sort of mischievous pleasure out of tormenting {c:mainName}, there's no actual malice behind it.  ");
						} else {
							append(w, t, "{mainName} seems to be doing the things {heShe}'s doing out of a sense of obligation, rather than for {hisHer} own personal pleasure.  ");
						}
						append(w, t, c, "{c:name} takes notice, hesitating as {c:heShe} decides how to proceed.");
					} else {
						append(w, t, c, "{c:name} is annoyed by the way that {mainName} is trying to molest {c:himHer}, but it's obvious that {mainName} isn't enjoying it ");
						if (flavorObedience() > 66) {
							w.append(t, " and is only doing it out of devotion to the Demon Lord, ");
						} else if (flavorObedience() > 33) {
							append(w, t, " and is only following {hisHer} orders, ");
						} else {
							w.append(t, " and is only using whichever tactic seems most effective, ");
						}
						append(w, t, c, "so {c:mainName} finds it impossible to actually hold it against {himHer}.");
					}
				}
			}
		}
	...
	public int intensity(int baseParameter, int currentParameter, int parameterCount) {
		...
		if (baseParameter > 0) {
			total += 5;
			if (total < baseParameter + 5*parameterCount) {
	...
	public int[] motivationDamage() {
		int[] damages = {(1000 - motivation)*2/3, 1000 - motivation, (1000 - motivation)*3/2};
		for (int i = 0; i < 3; i++) {
	...
	public int opinion(Forsaken x) {
		...
		if (x.equals(firstPartner)) {
			if (firstOriginalRelationship == 4) {
				sum = 1000;
			} else if (firstOriginalRelationship == 2) {
				sum = 300;
			} else if (firstOriginalRelationship >= 0) {
				sum = 100;
			} else if (firstOriginalRelationship == -2) {
				sum = -300;
			} else {
		...
		} else if (x.equals(secondPartner)) {
			if (secondOriginalRelationship == 4) {
				sum = 1000;
			} else if (secondOriginalRelationship == 2) {
				sum = 300;
			} else if (secondOriginalRelationship >= 0) {
				sum = 100;
			} else if (secondOriginalRelationship == -2) {
				sum = -300;
			} else {
		...
		if (x.kills != null) {
			if (x.kills.length > 0 && x.kills[0] != null) {
				if (firstFormerPartner != null && secondFormerPartner != null) {
					...
					} else if (x.kills[0].equals(secondFormerPartner)) {
						if (secondOriginalRelationship == 4) {
							sum -= 700;
						} else if (secondOriginalRelationship >= 0) {
							sum -= 500;
	...
	public String expMultiplierDisplay(long expDisplay) {
		...
		String displayed = (expDisplay/1000) + ".";
		expDisplay = expDisplay % 1000;
		if (expDisplay < 100) {
	...
	public String fixedFormat(long amount) {
		...
		if (power > 0) {
			format = format + (amount%100)/10;
		} else {
			format = format + " ";
		}
		if (power < 4) {
			format = format + amount%10;
		}
		if (power > 17) {
			format = format + "E";
		} else if (power > 14) {
	...
	public Boolean equals(Forsaken x) {
		if (this == null || x == null) {
			...
			return forsakenID == x.forsakenID;
		}
	}
